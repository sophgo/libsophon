#include "../include/bm_smi_display_util_detail.hpp"
...
static void bm_smi_get_attr(bm_handle_t handle, int bmctl_fd, int dev_id) {
    ...
    if (!status) {
        ...
        for (i = 0; i < 2; i++) {
            g_attr[dev_id].vpu_mem_total_size[i] = 0;
            g_attr[dev_id].vpu_mem_used_size[i] = 0;
            g_attr[dev_id].vpu_mem_free_size[i] = 0;
        }
	for (i = 0; i < MAX_NUM_VPU_CORE; i++) {
	    g_attr[dev_id].vpu_instant_usage[i] = 0;
	    }
        for (i = 0; i < VPP_CORE_MAX; i++) {
	    g_attr[dev_id].vpp_instant_usage[i] = 0;
            g_attr[dev_id].vpp_chronic_usage[i] = 0;
	    }
    }
#ifdef SOC_MODE
...
static void bm_smi_display_format(std::ofstream &file, bool save_file) {
    ...
    for (int i = 0; i < BM_SMI_FORMAT_HEIGHT_UTIL; i++) {
        switch (i) {
            case 0:
            ...
            case 6:
                snprintf(line_str, BUFFER_LEN,
                         "|                                       |"
                          "Vpp_Ins_Util0  Vpp_Ins_Util1  Vpp_Chr_Util0  Vpp_Chr_Util1|\n");
                break;
            case 7:
                snprintf(line_str, BUFFER_LEN,
                         "|                                       |"
        ...
        if (i == 0 || LINES >= BM_SMI_FORMAT_HEIGHT_UTIL) {
            printw("%s", line_str);
        }
    }
}
...
/* get vpu decode instant usage */
static void bm_smi_get_vpu_decode(int dev_id, char *s) {
#ifdef __linux__
#ifdef SOC_MODE
    snprintf(s, 5, " N/A ");
#else
    int vpu_decoder = 0;
    if(g_attr[dev_id].chip_id == 0x1686) {
        for (int i = 0; i < MAX_NUM_VPU_CORE_BM1686 - 1; i++) {
            vpu_decoder += g_attr[dev_id].vpu_instant_usage[i];
        }
        snprintf(s, 5, "%d%%", vpu_decoder / 2);
    } else {
        for (int i = 0; i < MAX_NUM_VPU_CORE - 1; i++) {
            vpu_decoder += g_attr[dev_id].vpu_instant_usage[i];
        }
        snprintf(s, 5, "%d%%", vpu_decoder / 4);
    }
...
/* get vpu encode instant usage */
static void bm_smi_get_vpu_encode(int dev_id, char *s) {
#ifdef __linux__
#ifdef SOC_MODE
    snprintf(s, 5, " N/A ");
#else
    if(g_attr[dev_id].chip_id == 0x1686) {
        snprintf(s, 5, "%d%%", g_attr[dev_id].vpu_instant_usage[MAX_NUM_VPU_CORE_BM1686 - 1]);
    } else {
        snprintf(s, 5, "%d%%", g_attr[dev_id].vpu_instant_usage[MAX_NUM_VPU_CORE - 1]);
    }
#endif
#else
    snprintf(s, 5, " N/A ");
...
/* get jpu decode*/
static void bm_smi_get_jpu_core_instace(int dev_id, char *s) {
#ifdef __linux__
#ifdef SOC_MODE
    snprintf(s, 5, " N/A ");
#else
    ...
    for (int i = 0; i < MAX_NUM_JPU_CORE; i++) {
        jpu_decoder += g_attr[dev_id].jpu_core_usage[i];
    }
    snprintf(s, 5, "%d%%", jpu_decoder / MAX_NUM_JPU_CORE);
#endif
#else
    snprintf(s, 5, " N/A ");
#endif
...
static void bm_smi_display_attr(int            dev_id,
    ...
    bm_smi_get_vpu_decode(dev_id, vpu_decode);
    bm_smi_get_vpu_encode(dev_id, vpu_encode);
    bm_smi_get_jpu_core_instace(dev_id, jpu_instance_core);

    attr_y = dis_slot * BM_SMI_DEVATTR_HEIGHT_UTIL;
    attr_y -= win_y_offset;

    for (int i = 0; i < BM_SMI_DEVATTR_HEIGHT_UTIL; i++, attr_y++) {
        switch (i) {
            case 0: {
            ...
            case 1:
                ...
                break;
            case 2: {
#if !defined(SOC_MODE) && defined(__linux__)
                snprintf(line_str, BUFFER_LEN,
                             "|                                       | %5d%%"
                             "         %5d%%         %5d%%         %5d%%      |\n",
                             g_attr[dev_id].vpp_instant_usage[0],
                             g_attr[dev_id].vpp_instant_usage[1],
                             g_attr[dev_id].vpp_chronic_usage[0],
                             g_attr[dev_id].vpp_chronic_usage[1]);
                snprintf(whole_str, BUFFER_LEN,
                             "|                                       | %5d%%"
                             "         %5d%%         %5d%%         %5d%%      |\n",
                             g_attr[dev_id].vpp_instant_usage[0],
                             g_attr[dev_id].vpp_instant_usage[1],
            ...
            case 3: {
                ...
                snprintf(line_str, BUFFER_LEN,
                             "|                                       |  %5s"
                             "              %5s              %5s             |\n", vpu_decode, vpu_encode, jpu_instance_core);
                snprintf(whole_str, BUFFER_LEN,
                             "|                                       |  %5s"
                             "              %5s              %5s             |\n", vpu_decode, vpu_encode, jpu_instance_core);
#else
        ...
        if (attr_y >= 0) {
            if (move(attr_y + BM_SMI_FORMAT_HEIGHT_UTIL, 0) == OK) {
                clrtoeol();
                attron(COLOR_PAIR(0));
                printw("%s", line_str);
...
#ifdef __linux__
    if (LINES >
        std::max((dev_cnt * BM_SMI_DEVATTR_HEIGHT_UTIL + BM_SMI_FORMAT_HEIGHT_UTIL +
                  BM_SMI_PROCHEADER_HEIGHT - win_y_offset),
                 BM_SMI_FORMAT_HEIGHT_UTIL + BM_SMI_PROCHEADER_HEIGHT)) {
#else
    if (LINES > max((dev_cnt * BM_SMI_DEVATTR_HEIGHT_UTIL + BM_SMI_FORMAT_HEIGHT_UTIL +
                     BM_SMI_PROCHEADER_HEIGHT - win_y_offset),
                    BM_SMI_FORMAT_HEIGHT_UTIL + BM_SMI_PROCHEADER_HEIGHT)) {
#endif
        proc_show = true;
        proc_y    = dev_cnt * BM_SMI_DEVATTR_HEIGHT_UTIL + BM_SMI_FORMAT_HEIGHT_UTIL +
...
static void bm_smi_display_all(std::ofstream &target_file,
    ...
    text_lines += BM_SMI_FORMAT_HEIGHT_UTIL;
    ...
    text_lines += BM_SMI_DEVATTR_HEIGHT_UTIL * dev_cnt;
...
bm_smi_display_util_detail::bm_smi_display_util_detail(bm_smi_cmdline &cmdline) : bm_smi_test(cmdline) {
...
bm_smi_display_util_detail::~bm_smi_display_util_detail() {}

int bm_smi_display_util_detail::validate_input_para() {
#ifndef SOC_MODE
...
int bm_smi_display_util_detail::run_opmode() {
    int  ch;
    bool loop      = true;
    ...
    while (loop) {
        ...
        switch (ch) {
            ...
            case KEY_NPAGE:  // scroll down one page
                ...
                break;
            case KEY_REFRESH:  // refresh window after resize detected
                refresh();
                clear();
                break;
            default:  // others exit
...
int bm_smi_display_util_detail::check_result() {
    return 0;
}

void bm_smi_display_util_detail::case_help() {
    printf("case_help\n");
}
