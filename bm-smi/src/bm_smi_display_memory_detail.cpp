#include "../include/bm_smi_display_memory_detail.hpp"
...
static void bm_smi_display_format(std::ofstream &file, bool save_file) {
    ...
    for (int i = 0; i < BM_SMI_FORMAT_HEIGHT_MEM; i++) {
        switch (i) {
            case 0:
        ...
        if (i == 0 || LINES >= BM_SMI_FORMAT_HEIGHT_MEM) {
            printw("%s", line_str);
        }
    }
}
...
static void bm_smi_maxclk_to_str(int dev_id, char *s) {
    if (g_attr[dev_id].tpu_max_clock == ATTR_NOTSUPPORTED_VALUE) {
        ...
        snprintf(s, 13, "%dM", g_attr[dev_id].tpu_max_clock);
    }
}
...
static void bm_smi_currclk_to_str(int dev_id, char *s) {
    if (g_attr[dev_id].tpu_current_clock == ATTR_NOTSUPPORTED_VALUE) {
        ...
        snprintf(s, 13, "%dM", g_attr[dev_id].tpu_current_clock);
    }
}
...
static void bm_smi_display_attr(int            dev_id,
    ...
    attr_y = dis_slot * BM_SMI_DEVATTR_HEIGHT_MEM;
    attr_y -= win_y_offset;

    for (int i = 0; i < BM_SMI_DEVATTR_HEIGHT_MEM; i++, attr_y++) {
        switch (i) {
            case 0: {
            ...
            case 2: {
                ...
                snprintf(line_str, BUFFER_LEN,
                             "|                                       |%4dMB/%4dMB"
                             " %4dMB/%4dMB %4dMB/%4dMB %5lldMB/%5lldMB |\n", (int)g_attr[dev_id].stat.heap_stat[0].mem_used,
                             (int)g_attr[dev_id].stat.heap_stat[0].mem_total, (int)g_attr[dev_id].stat.heap_stat[1].mem_used,
                             (int)g_attr[dev_id].stat.heap_stat[1].mem_total, (int)g_attr[dev_id].stat.heap_stat[2].mem_used,
                             (int)g_attr[dev_id].stat.heap_stat[2].mem_total,
                             (g_attr[dev_id].vpu_mem_used_size[0] / 1024 / 1024), (g_attr[dev_id].vpu_mem_total_size[0] / 1024 / 1024));
        ...
        if (attr_y >= 0) {
            if (move(attr_y + BM_SMI_FORMAT_HEIGHT_MEM, 0) == OK) {
                clrtoeol();
                attron(COLOR_PAIR(0));
                printw("%s", line_str);
...
#ifdef __linux__
    if (LINES >
        std::max((dev_cnt * BM_SMI_DEVATTR_HEIGHT_MEM + BM_SMI_FORMAT_HEIGHT_MEM +
                  BM_SMI_PROCHEADER_HEIGHT - win_y_offset),
                 BM_SMI_FORMAT_HEIGHT_MEM + BM_SMI_PROCHEADER_HEIGHT)) {
#else
    if (LINES > max((dev_cnt * BM_SMI_DEVATTR_HEIGHT_MEM + BM_SMI_FORMAT_HEIGHT_MEM +
                     BM_SMI_PROCHEADER_HEIGHT - win_y_offset),
                    BM_SMI_FORMAT_HEIGHT_MEM + BM_SMI_PROCHEADER_HEIGHT)) {
#endif
        proc_show = true;
        proc_y    = dev_cnt * BM_SMI_DEVATTR_HEIGHT_MEM + BM_SMI_FORMAT_HEIGHT_MEM +
...
static void bm_smi_display_all(std::ofstream &target_file,
    ...
    text_lines += BM_SMI_FORMAT_HEIGHT_MEM;
    ...
    text_lines += BM_SMI_DEVATTR_HEIGHT_MEM * dev_cnt;
...
bm_smi_display_memory_detail::bm_smi_display_memory_detail(bm_smi_cmdline &cmdline) : bm_smi_test(cmdline) {
...
bm_smi_display_memory_detail::~bm_smi_display_memory_detail() {}

int bm_smi_display_memory_detail::validate_input_para() {
#ifndef SOC_MODE
...
int bm_smi_display_memory_detail::run_opmode() {
    int  ch;
    bool loop      = true;
...
int bm_smi_display_memory_detail::check_result() {
    return 0;
}

void bm_smi_display_memory_detail::case_help() {
    printf("case_help\n");
}
