...
void BM1688JsonDumper::addDmaCmdInfoNode(nlohmann::ordered_json& engine_obj, const Engine& engine) {
...
void BM1688JsonDumper::addTiuCmdInfoNode(nlohmann::ordered_json& engine_obj, const Engine& engine) {
...
void BM1688JsonDumper::addDmaCtrlInfoNode(nlohmann::ordered_json& engine_obj, const Engine& engine) {
    nlohmann::ordered_json dma_obj;
    // 获取引擎结果
    const auto& results = engine.getResults();
    if (results.empty()) {
...
void BM1688JsonDumper::addTiuCtrlInfoNode(nlohmann::ordered_json& engine_obj, const Engine& engine) {
    nlohmann::ordered_json tiu_obj;
    // 获取引擎结果
    const auto& results = engine.getResults();
    if (results.empty()) {
