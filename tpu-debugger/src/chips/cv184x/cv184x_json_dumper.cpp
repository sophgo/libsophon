...
void CV184XJsonDumper::addDmaCmdInfoNode(nlohmann::ordered_json& engine_obj, const Engine& engine) {
...
void CV184XJsonDumper::addTiuCmdInfoNode(nlohmann::ordered_json& engine_obj, const Engine& engine) {
...
void CV184XJsonDumper::addDmaCtrlInfoNode(nlohmann::ordered_json& engine_obj, const Engine& engine) {
    nlohmann::ordered_json dma_obj;
    // 获取引擎结果
    const auto& results = engine.getResults();
    if (results.empty()) {
...
void CV184XJsonDumper::addTiuCtrlInfoNode(nlohmann::ordered_json& engine_obj, const Engine& engine) {
    nlohmann::ordered_json tiu_obj;
    // 获取引擎结果
    const auto& results = engine.getResults();
    if (results.empty()) {
