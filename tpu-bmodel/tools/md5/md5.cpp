...
std::string gen_md5_string(unsigned char *file_path) {
    unsigned char md5sum[MD5SUM_LEN] = {0};
    read_md5(file_path, md5sum);
    std::stringstream ss;
    for(int i=0; i<MD5SUM_LEN; ++i)
        ss << std::setfill('0') << std::setw(2) << std::hex << (int)md5sum[i];
    return ss.str();
}

std::string gen_md5_string(unsigned char *buffer, size_t size) {
    unsigned char md5sum[MD5SUM_LEN] = {0};
    calc_md5(buffer, size, md5sum);
    std::stringstream ss;
    for(int i=0; i<MD5SUM_LEN; ++i)
        ss << std::setfill('0') << std::setw(2) << std::hex << (int)md5sum[i];
    return ss.str();
