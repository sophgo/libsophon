...
{
    UNREFERENCED_PARAMETER(ddr_freq);
	ddr4_init(bmdi, 0);
	ddr4_init(bmdi, 1);
	return 0;
}
...
static int ddr_self_test_pattern(struct bm_device_info *bmdi, u32 ddr_ctl)
{
	...
	if ((data & ~0xfffffffd) == 0x02) {
        TraceEvents(TRACE_LEVEL_ERROR,
                    TRACE_DEVICE, "bmdrv: release SDII grant capability, FORCE_SDII_NO_GNT "
                    "should be 0!\n");
		return -1;
	}
	...
	if ((data & ~0xffffffef) == 0x10) {
		//TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"bmdrv: [ddr_%04x BIST] success!\n", ddr_ctl);
	} else {
		//TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"bmdrv: [ddr_%04x BIST] failed!\n", ddr_ctl);
		data = ddr_reg_read_enh(bmdi, ddr_ctl, 0x4);
...
int ddr_init(struct bm_device_info *bmdi, u32 freq)
{
    ...
    TraceEvents(
        TRACE_LEVEL_INFORMATION, TRACE_DEVICE, "bmdrv : ddrc0_init...\n");
	...
	if (ret) {
        TraceEvents(
            TRACE_LEVEL_ERROR, TRACE_DEVICE, "bmdrv: ddr self test failed!\n");
		return ret;
	}
	//INFO("ddrc0_init end...\n");
#ifdef DUMP_DDRC_REG
	...
	TraceEvents(
        TRACE_LEVEL_INFORMATION, TRACE_DEVICE, "bmdrv : ddrc1 init...\n");
...
int bm1682_ddr_top_init(struct bm_device_info *bmdi)
{
	...
	if (ret) {
        TraceEvents(
            TRACE_LEVEL_ERROR, TRACE_DEVICE, "bmdrv : init ddr failed\n");
	}
	return ret;
}
