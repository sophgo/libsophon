	...
	int with_cmd, int addr_bytes, int data_bytes)
{
	...
	if (data_bytes > 65535) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "SPI data in overflow, should be less than 65535 bytes(%d)\n", data_bytes);
		return -1;
	}
	...
	while ((spi_reg_read(bmdi, SPI_INT_STS) & BIT_SPI_INT_RD_FIFO) == 0 &&
		...
		bm_udelay(1);
	if (wait_cnt == 0) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "SPI read int timeout!\n");
		return -1;
	}
	...
	while (off < data_bytes) {
		...
		wait_cnt = 3000;
		while ((spi_reg_read(bmdi, SPI_FIFO_PT) & 0xF) != (u32)xfer_size && wait_cnt--)
			bm_udelay(1);
		if (wait_cnt == 0) {
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "SPI wait to read FIFO timeout!\n");
			return -1;
		}
	...
	while ((spi_reg_read(bmdi, SPI_INT_STS) & BIT_SPI_INT_TRAN_DONE) == 0 &&
		...
		bm_udelay(1);
	if (wait_cnt == 0) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "SPI wait tran timeout!\n");
		return -1;
	}
...
{
	if (bmdi->cinfo.bar_info.io_bar_vaddr.spi_bar_vaddr == NULL){
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[bm_spi_init] spi_bar_vaddr is = NULL\n");
	} else{
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "[bm_spi_init] spi_bar_vaddr is = %d \n", *bmdi->cinfo.bar_info.io_bar_vaddr.spi_bar_vaddr);
	}
	u32 tran_csr = 0;
	...
	u32 with_cmd, u32 addr_bytes)
{
	...
	if (addr_bytes > 3) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "non-data: addr bytes should be less than 3 (%d)\n", addr_bytes);
		return -1;
	}
	...
	u32 with_cmd, u32 addr_bytes, u32 data_bytes)
{
	...
	if (data_bytes > 65535) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "data out overflow, should be less than 65535 bytes(%d)\n", data_bytes);
		return -1;
	}
	...
	while ((spi_reg_read(bmdi, SPI_FIFO_PT) & 0xF) != 0 &&
		...
		bm_udelay(1);
	if (wait_cnt == 0) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "SPI issue tran timeout!\n");
		return -1;
	}
	...
	while (off < data_bytes) {
		...
		if (wait_cnt == 0) {
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "wait to write FIFO timeout!\n");
			return -1;
		}
	...
	u32 with_cmd, u32 addr_bytes, u32 send_bytes, u32 get_bytes)
{
	...
	if (send_bytes != get_bytes) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "data in&out: get_bytes should be the same as send_bytes\n");
		return -1;
	}

	if ((send_bytes > SPI_MAX_FIFO_DEPTH) || (get_bytes > SPI_MAX_FIFO_DEPTH)) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "data in&out: FIFO will overflow\n");
		return -1;
	}
...
int bm_spi_flash_erase_sector(struct bm_device_info *bmdi, u32 addr)
{
	...
	if ((spi_status & SPI_STATUS_WEL) == 0) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "write en failed, get status: 0x%x\n", spi_status);
		return -1;
	}
	...
	while (1) {
		...
		if (((spi_status & SPI_STATUS_WIP) == 0) || (wait_cnt > 30)) { // 3s, spec 0.15~1s
			//TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"sector erase done, get status: 0x%x, wait: %d\n", spi_status, wait_cnt);
			break;
		}
		wait_cnt++;
	}
...
int bm_spi_flash_program_sector(struct bm_device_info *bmdi, u32 addr)
{
	...
	spi_status = (u8)spi_read_status(bmdi);
	if (spi_status != 0x02) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "spi status check failed, get status: 0x%x\n", spi_status);
		return -1;
	}
	...
	while (1) {
		...
		if (((spi_status & SPI_STATUS_WIP) == 0) || (wait_cnt > 600)) { // 60ms, spec 120~2800us
			TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"sector prog done, get status: 0x%x\n", spi_status);
			break;
		}
		wait_cnt++;
	}
...
static int do_page_program(struct bm_device_info *bmdi, u8 *src_buf, u32 addr, u32 size)
{
	...
	if (size > SPI_FLASH_BLOCK_SIZE) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "size larger than a page\n");
		return -1;
	}
	if ((addr % SPI_FLASH_BLOCK_SIZE) != 0) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "addr not alignned to page\n");
		return -1;
	}
	...
	while (1) {
		...
		if (((spi_status & SPI_STATUS_WIP) == 0) || (wait_cnt > 600)) { // 60ms, spec 120~2800us
            TraceEvents(TRACE_LEVEL_INFORMATION,
                        TRACE_DEVICE,
                        "page prog done, get status: 0x%x\n",
                        spi_status);
			break;
		}
		wait_cnt++;
...
int bm_spi_flash_program(struct bm_device_info *bmdi, u8 *src_buf, u32 base, u32 size)
{
	...
	if (id == SPI_ID_M25P128)
		...
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "unrecognized flash ID 0x%x\n", id);
		return -1;
	}

	if ((base % sector_size) != 0) {
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "<flash offset addr> is not aligned erase sector size (0x%x)!\n", sector_size);
		return -1;
	}

	erased_sectors = (u8)((size + sector_size) / sector_size);
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"Start erasing %d sectors, each %d bytes...\n", erased_sectors, sector_size);

	for (i = 0; i < erased_sectors; i++)
		bm_spi_flash_erase_sector(bmdi, base + i * sector_size);

	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"--program boot fw, page size %d\n", SPI_FLASH_BLOCK_SIZE);
	...
	while (off < size) {
		...
		if (do_page_program(bmdi, src_buf + off, base + off, xfer_size) != 0) {
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "page prog failed @ 0x%x\n", base + off);
			return -1;
		}
		...
		if (cmp_ret != 0) {
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "memcmp failed\n");
			return cmp_ret;
		}
		off += xfer_size;
	...
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"--program boot fw success\n");
	return 0;
}
