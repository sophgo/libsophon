...
static void dwc_decode_major_msg(struct bm_device_info *bmdi, int msg)
{
	...
	if (msg == FW_COMPLET_FAILED) {
        TraceEvents(TRACE_LEVEL_INFORMATION,
                    TRACE_DEVICE,
                    "dwc phy message major message type: 0x%x, %s\n",
                    msg,
...
loop:
	...
	if (mail == 0xff) {
        TraceEvents(TRACE_LEVEL_ERROR,
                    TRACE_DEVICE,
                    "Lpddr train failed, ddr: %s, phase: %d\n",
...
static int ddr_training(struct bm_device_info *bmdi, u32 cfg_base, u32 train)
{
	...
	if (train == T1D) {
		...
		//3 set dfi clock
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"phy load dmem\n");
		...
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"phy run firmware\n");
...
static int ddr_phy_enter_mission(struct bm_device_info *bmdi, u32 cfg_base, u32 ddr_index)
{
	u32 i;
	u32 read_data;
    UNREFERENCED_PARAMETER(ddr_index);
	//5 set dfi clock highest
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"phy load pie\n");
	...
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"phy run firmware done\n");


	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"Wait for impedance calibration\n");
...
static int ddr_phy_init(struct bm_device_info *bmdi, u32 cfg_base, u32 ddr_index)
{
	...
	//DDR_PHY_ENABLE
	//0 pin swap
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"phy pin swap\n");
	dwc_phy_apb_write16(bmdi, cfg_base, MICRO_CONT_MUX_SEL, 0);
	if (bmdi->cinfo.chip_id == 0x1684){
		...
		if (ddr_index == 0) {
			for (i = 0; i < ARRAY_SIZE(pin_swap_1684x_0); i++)
				dwc_phy_apb_write16(bmdi, cfg_base, pin_swap_1684x_0[i].offset, pin_swap_1684x_0[i].value);
		} else if (ddr_index == 1) {
			for (i = 0; i < ARRAY_SIZE(pin_swap_1684x_1); i++)
				dwc_phy_apb_write16(bmdi, cfg_base, pin_swap_1684x_1[i].offset, pin_swap_1684x_1[i].value);
		} else if (ddr_index == 2) {
			for (i = 0; i < ARRAY_SIZE(pin_swap_1684x_2); i++)
				dwc_phy_apb_write16(bmdi, cfg_base, pin_swap_1684x_2[i].offset, pin_swap_1684x_2[i].value);
		} else if (ddr_index == 3) {
			for (i = 0; i < ARRAY_SIZE(pin_swap_1684x_3); i++)
				dwc_phy_apb_write16(bmdi, cfg_base, pin_swap_1684x_3[i].offset, pin_swap_1684x_3[i].value);
		}
	}
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"phy pll\n");

	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"phy initial\n");
	//1 phy initial
...
static  void pll_setting(struct bm_device_info *bmdi, u32 freq)
{
	...
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"No support freq\n");
	return;
...
static void dwc_pre_setting(struct bm_device_info *bmdi, u32 rank, u32 freq)
{
	...
	if (!cfg_freq || !cfg_rank)
        TraceEvents(TRACE_LEVEL_ERROR,
                    TRACE_DEVICE,
                    "lpddr4/4x can't get configuration\n");
	...
	if (bmdi->boot_info.ddr_ecc_enable == 0x1) {
		...
		if (bmdi->cinfo.chip_id == 0x1686){
			cfg_ecc = bm1684x_cfg_ecc_off;
			ecc_array_size = ARRAY_SIZE(bm1684x_cfg_ecc_off);
		} else{
			cfg_ecc = cfg_ecc_off;
...
static int ddr_memset_by_cdma(struct bm_device_info *bmdi, u64 start_addr, u64 end_addr)
{
    ...
    u32                    size        = memcpy_info->stagging_mem_size;
	int num = (int) ((end_addr - start_addr)/size);

	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"ecc memset start_addr = 0x%llx, end_addr = 0x%llx, num = 0x%x, size = 0x%x\n",
		start_addr, end_addr, num, size);
    ExAcquireFastMutex(&memcpy_info->s2dstag_Mutex);
    memset(memcpy_info->s2dstagging_mem_vaddr, 0, size);
	for (i = 0; i <= num; i++) {
        ...
        if (memcpy_info->bm_cdma_transfer(bmdi, &cdma_arg, TRUE, NULL)) {
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "ddr memeset src = 0x%llx, dst = 0x%llx fail\n",
			start_addr, end_addr);
...
int bm1684_lpddr4_init(struct bm_device_info *bmdi)
{
	...
	case 0x3:
		...
		break;
	default:
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "unknow rank type for DDR init\n");
		return -1;
	}
	...
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"LPDDR interleave disabled\n");
#endif
...
start:
	...
	if (bmdi->cinfo.ddr_failmap) {
		...
		fail_count++;
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "Lpddr train %d times still fail, retry!\n", fail_count);
		if (fail_count > 20) {
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "lpddr train fail\n");
			return -1;
		}
		goto start;
	}
	...
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"lpddr%s(rank: %d + %d, mode: %s) init done\n",
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION,
                TRACE_DEVICE,
                "bmdi->boot_info.ddr_ecc_enable =%d\n",
                bmdi->boot_info.ddr_ecc_enable);
	if (bmdi->boot_info.ddr_ecc_enable == 0x1) {
        TraceEvents(TRACE_LEVEL_INFORMATION,
                    TRACE_DEVICE,
                    "start cdma write ddr for ecc\n");
		...
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"cdma write ddr for ecc done\n");
	}
