...
void bm1684x_enable_ddr_interleave(struct bm_device_info *bmdi)
{
	int reg_val = 0;
...
void ddr_phy_reg_write16(struct bm_device_info *bmdi, u32 addr, u32 value)
{
	...
	bar4_addr =(u32*) bmdi->cinfo.bar_info.bar4_vaddr;
...
//	u64 ddr0_total_size = 0x200000000*7/8; //ONLY 7/8 NEED FORMAT
	...
	memset(memcpy_info->s2dstagging_mem_vaddr, 0xfe, realmem_size);
