...
函数原型：bm_status_t bm_malloc_device_mem(bm_handle_t handle,
unsigned long long \*paddr, int heap_id, unsigned long long size);
...
函数作用：为智能视觉深度学习处理器 fw log设置debug模式 备注：此函数SC3在使用
...
函数作用：获取当前板卡的名称，名称:处理器id-板卡类型（如：1684-SC5+）。
