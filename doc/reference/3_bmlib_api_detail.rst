...
函数作用：在指定的设备上创建handle，第一次创建handle的时候会创建bmcpu的线程，用于执行算子
...
函数原型：bm_status_t bm_memcpy_s2s(uint64_t u64PhyDst, uint64_t u64PhySrc, uint64_t u64Size);

函数作用：将指定的DDR物理地址长度为u64Size的数据从u64PhySrc拷贝到u64PhyDst，拷贝的长度单位是字节
...
| 参数名       | 输入/输出    | 说明                                  |
+==============+==============+=======================================+
| u64PhyDst    | 输入         | 目标内存的物理地址                    |
+--------------+--------------+---------------------------------------+
| u64PhySrc    | 输入         | 源内存的物理地址                      |
+--------------+--------------+---------------------------------------+
| u64Size      | 输入         | 拷贝的数据单位字节长度                |
+--------------+--------------+---------------------------------------+
...
函数原型：bm_status_t bm_memcpy_s2s_2d(sg_api_2d_memcpy_t \*api_mem_param);

函数作用：将矩阵数据进行部分拷贝，拷贝子矩阵(w_bytes*h)数据，从src_global_offset地址拷贝到dst_global_offset地址，支持2D拷贝
...
| 参数名       | 输入/输出    | 说明                                  |
+==============+==============+=======================================+
| api_mem_param| 输入         | 封装好原地址和目标地址等信息的结构体  |
+--------------+--------------+---------------------------------------+
