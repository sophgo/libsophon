...
bool BMProfileDevice::begin(net_ctx_t* net_ctx)
{
    ...
    if(enable_arm){
        ...
        ret = dynamic_cast<bmdnn_func_1684x*>(launcher.get())->_bmdnn_set_profile_enable_(handle, enable_func_id[0], true);
        CHECK_status(ret);
    }
    return true;
}
    ...
    if (enable_arm) {
        ...
        for(int i=0; i<2; i++){
            ...
            while(1){
                ...
                bm_status_t status = dynamic_cast<bmdnn_func_1684x*>(launcher.get())->_bmdnn_get_profile_data_(
                            handle,
                            get_func_id,
        ...
        bm_status_t status = dynamic_cast<bmdnn_func_1684x*>(launcher.get())->_bmdnn_set_profile_enable_(handle, enable_func_id, false);
        CHECK_status(status);
    }
    return true;
}
