...
bool BMProfileDevice::begin(net_ctx_t* net_ctx)
{
    ...
    for(size_t i=0; i<core_list.size(); i++){
        if(enable_bdc){
            ...
            bmfunc::bmdnn_mars3()->_bmdnn_set_engine_profile_param_(
        ...
        ret = bmfunc::bmdnn_mars3()->_bmdnn_set_profile_enable_(handle, core_list[i], enable_func_ids[i], enable_bits);
        CHECK_status(ret);
    }
    return true;
}
...
bool BMProfileDevice::end(net_ctx_t* net_ctx)
{
    ...
    for(size_t i=0; i<core_list.size(); i++){
        ...
        auto enable_func_ids = net_ctx->kernel_module_->get_enable_profile_func_id(core_list);
        bm_status_t status = bmfunc::bmdnn_mars3()->_bmdnn_set_profile_enable_(handle, core_list[0], enable_func_ids[0], 0);
    }
    return true;
}
