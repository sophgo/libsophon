...
void npy_save(std::string fname, const T *data, const std::vector<size_t> shape,
  ...
  std::vector<char> header =
      create_npy_header(true_data_shape, word_size, type);
  size_t nels =
...
template <typename T>
void npy_save(std::string fname, const std::vector<T> data, std::string mode) {
...
void npz_save(std::string zipname, std::string fname, const T *data,
  ...
  if (shape.size() != 0) {
    ...
    std::cerr << "[Warning] zip name: " << fname
              << " npz shape size is 0, skip it\n";
    fclose(fp);
    return;
  }
  ...
  */
  ...
  if (global_header_offset >= ZIP64_LIMIT) {
    ...
    zip64endrec_header += (uint64_t)global_header_offset + nbytes +
                          local_header.size(); // centDirOffset
    ...
    zip64locrec_header +=
        (uint64_t)global_header_offset + nbytes + local_header.size() +
  ...
  footer +=
      (global_header_offset >= ZIP64_LIMIT)
          ? (uint32_t)0xFFFFFFFF
...
template void npz_save<std::complex<double>>(std::string, std::string,
                                             ...
                                             const std::vector<size_t> &,
                                             std::string);
template void npz_save<double>(std::string, std::string, const double *,
                               const std::vector<size_t> &, std::string);
template void npz_save<char>(std::string, std::string, const char *,
                             const std::vector<size_t> &, std::string);
...
template <typename T>
void npz_save(std::string zipname, std::string fname, NpyArray &array,
              std::string mode) {
  npz_save<T>(zipname, fname, array.data<T>(), array.shape, mode);
}

template <typename T>
void npz_add_array(npz_t &map, std::string fname, const T *data,
...
template void npz_add_array<std::complex<double>>(npz_t &, std::string,
                                                  const std::complex<double> *,
                                                  const std::vector<size_t>);
template void npz_add_array<float>(npz_t &, std::string, const float *,
                                   const std::vector<size_t>);
template void npz_add_array<int8_t>(npz_t &, std::string, const int8_t *,
                                    const std::vector<size_t>);
template void npz_add_array<uint8_t>(npz_t &, std::string, const uint8_t *,
                                     const std::vector<size_t>);
template void npz_add_array<int16_t>(npz_t &, std::string, const int16_t *,
                                     const std::vector<size_t>);
template void npz_add_array<uint16_t>(npz_t &, std::string, const uint16_t *,
                                      const std::vector<size_t>);
template void npz_add_array<uint32_t>(npz_t &, std::string, const uint32_t *,
                                      const std::vector<size_t>);
template void npz_add_array<int32_t>(npz_t &, std::string, const int32_t *,
                                     const std::vector<size_t>);

template <typename T>
void npz_add_array(npz_t &map, std::string fname, const std::vector<T> &data) {
  std::vector<size_t> shape;
  shape.push_back(data.size());
...
template void
npz_add_array<std::complex<double>>(npz_t &, std::string,
...
void npz_save_all(std::string zipname, npz_t &map) {
  for (auto it = map.begin(); it != map.end(); it++) {
    ...
    if (arr.type == 'f') {
      ...
      // invalid type
      std::cout << "libcnpy error: invalid array type " << arr.type << ", for "
                << it->first << "\n";
      assert(0);
    }
  }
