#include "cpu_anakin_detect_out.h"
#include <algorithm>
...
{ \
  ...
  float time_spent = 1000.0f * (t1. tv_sec - t0.tv_sec) \
...
int cpu_anakin_detect_outlayer::process(void *param, int param_size)
{
#ifdef TIME_PROFILE
  ...
  Dtype* conf_data  = input_tensors_[1];
  const int num = input_shapes_[0][0];

  float* all_conf_scores = (float*)conf_data;
  int conf_stride = num_classes_;
  ...
  if(input_tensors_.size() >= 3) {
    ...
    if (input_tensors_.size() >= 5) {
      const Dtype* arm_conf_data  = input_tensors_[3];
      const Dtype* arm_loc_data   = input_tensors_[4];

      // Retrieve all confidences.
      TIMER_START;
      GetConfidenceScores_opt(conf_data, num, num_priors_, num_classes_,
                              all_conf_scores);
      for (int i = 0; i < num; i++) {
          float* label_scores = all_conf_scores + i * num_classes_ * num_priors_;
          for (int p = 0; p < num_priors_; p++) {
              if (arm_conf_data[p * 2 +1] < objectness_score_) {
                  label_scores[0 * num_priors_ + p] = 1.0;
                  for (int n = 1; n < num_classes_; n++) {
                      label_scores[n * num_priors_ + p] = 0.0;
                  }
              }
          }
      }
      conf_stride = 1;
      TIMER_END("GetConfidenceScores");

      TIMER_START;
      NormalizedBBoxOpt* arm_loc_preds = NULL;
      GetLocPredictions_opt(arm_loc_data, num, num_priors_, num_loc_classes_,
                        share_location_, arm_loc_preds);
      TIMER_END("GetLocPredictions");

      /* Following three lines seems unused, Commented temporarily */
      //const int prior_count = bmcpu_shape_count(input_shapes_[2]);
      //Dtype* arm_bbox_data = new Dtype[prior_count];
      //memcpy(arm_bbox_data,prior_data,prior_count*sizeof(float));

      TIMER_START;
      NormalizedBBoxOpt* arm_decode_bboxes = NULL;
      DecodeBBoxesAll_opt(arm_loc_preds, prior_bboxes, prior_variances, 1,
                      share_location_, num_priors_, 1,
                      background_label_id_, code_type_, variance_encoded_in_target_,
                      clip_bbox, arm_decode_bboxes, all_decode_bboxes_area);
      delete[] all_decode_bboxes_area;
      if(num_loc_classes_ != 1) delete[] arm_loc_preds;

      DecodeBBoxesAll_opt(all_loc_preds, (float*)arm_decode_bboxes, prior_variances,
                      num, share_location_, num_priors_, num_loc_classes_,
                      background_label_id_, code_type_, variance_encoded_in_target_,
                      clip_bbox, all_decode_bboxes, all_decode_bboxes_area);
      delete [] arm_decode_bboxes;
    ...
    if(num_loc_classes_ != 1) delete[] all_loc_preds;
  } else if(input_tensors_.size() == 2){
    // Retrieve all location predictions.
    TIMER_START;
    GetLocPredictions_opt(loc_data, num, num_priors_, num_loc_classes_,
                      share_location_, all_decode_bboxes);
    all_decode_bboxes_area = new float[num * num_priors_ * num_loc_classes_];
    for(int i = 0; i < num; ++i) {
      for(int c = 0; c < num_loc_classes_; ++c) {
        const NormalizedBBoxOpt* bboxes = all_decode_bboxes + + c * num_priors_ +
                                          i * num_priors_ * num_loc_classes_;
        float* bboxes_area = all_decode_bboxes_area + + c * num_priors_ +
                             i * num_priors_ * num_loc_classes_;
        for(int p = 0; p < num_priors_; ++p) {
          bboxes_area[p] = BBoxSizeOpt(bboxes[p]);
        }
      }
    }
    TIMER_END("GetLocPredictions");
    ...
    //Error case;
    BM_LOG(FATAL) << "Not enough inputs";
    return -1;
  }
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int c = 0; c < num_classes_; ++c) {
      ...
      const float* bboxes_area = all_decode_bboxes_area + label * num_priors_ +
                                 i * num_priors_ * num_loc_classes_;
      const float* scores = all_conf_scores + i * num_priors_ * num_classes_;
      if(conf_stride == num_classes_) scores = scores + c;
      else                            scores = scores + c * num_priors_;
      ApplyNMSFast_opt(bboxes, num_priors_, bboxes_area, scores,
          confidence_threshold_, nms_threshold_, eta_, top_k_,
          conf_stride, &(indices[c]));
  ...
  memset(top_data, 0, bmcpu_shape_count((*output_shapes_)[0]) * sizeof(Dtype));
  if (num_kept == 0) {
  ...
  for (int i = 0; i < num; ++i) {
    const NormalizedBBoxOpt* decode_bboxes = all_decode_bboxes + i * num_priors_ * num_loc_classes_;
    for (auto it = all_indices[i].begin(); it != all_indices[i].end(); ++it) {
      ...
      const NormalizedBBoxOpt* bboxes = decode_bboxes + loc_label * num_priors_ +
                                        i * num_priors_ * num_loc_classes_;
  ...
  TIMER_START;
  if(all_conf_scores != NULL && input_tensors_.size() >= 5)     delete[] all_conf_scores;
  if(all_decode_bboxes_area != NULL)                            delete[] all_decode_bboxes_area;
  if(input_tensors_.size() > 2 || (input_tensors_.size() == 2 && num_loc_classes_ != 1))
                                                                delete[] all_decode_bboxes;
  TIMER_END("Release memory");

#ifdef TIME_PROFILE
  struct timeval tpend;
  gettimeofday(&tpend,NULL);
  float time_spent = 1000.0f * (tpend. tv_sec - tpstart.tv_sec)
                     + (tpend.tv_usec - tpstart.tv_usec) / 1000.0f;
...
void cpu_anakin_detect_outlayer::setParam(void *param, int param_size)
{
  layer_param_ = param;
  ...
  objectness_score_            = ssd_detect_out_param->objectness_score_;
}

int cpu_anakin_detect_outlayer::reshape(
    ...
    vector<vector<int>>& output_shapes)
{
  ...
  cout << " anakin detect_output reshape "<< endl;
  return 0;
}

REGISTER_CPULAYER_CLASS(CPU_ANAKIN_DETECT_OUTPUT, cpu_anakin_detect_out)
