...
static void test_vpp_yuv2rgb() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        os << i;
        os << ".bmp";
        bm_image_write_to_bmp(dst[i], os.str().c_str());
    }

    bmcv_image_vpp_csc_matrix_convert(handle, 1, src, dst, CSC_YCbCr2RGB_BT709);
    bm_image_write_to_bmp(dst[0], "yuv2rgb_BT709.bmp");
...
static void test_vpp_yuv_crop_to_odd_num_height() {
    ...
    bm_image_create(handle,
                    ...
                    DATA_TYPE_EXT_1N_BYTE,
                    &dst);
    bm_image_alloc_dev_mem_heap_mask(dst, 6);
    bm_image_create(handle,
                    IMAGE_H / 2 - 1,
                    IMAGE_W / 4 - 1 ,
                    FORMAT_BGR_PACKED,
    ...
    bm_image_write_to_bmp(dst, "odd_yuv1.bmp");
    bmcv_image_vpp_convert(handle, 1, src, &dst_bgr, &rect2);
    bm_image_write_to_bmp(dst_bgr, "odd_yuv2.bmp");
...
static void test_vpp_compressed2rgb() {
    ...
    for (int i = 0; i < 4; i++) {
        bm_malloc_device_byte_heap(handle, mem + i, 1, plane_size[i]);
    ...
    bmcv_image_vpp_csc_matrix_convert(handle, 1, src, dst, CSC_YCbCr2RGB_BT709);
    bm_image_write_to_bmp(dst[0], "fbd_BT709.bmp");
