...
#if DEBUG_WARP
    FILE *fp = fopen("warp_test.rgb.i", "rb+");
    assert(fp != NULL);
    fseek(fp, 0, SEEK_END);
    int size = ftell(fp);
    fseek(fp, 0, SEEK_SET);
    fread(res->data, size, 1, fp);
...
u8 inline fetch_pixel(float x_idx, float y_idx, int c_idx, int width, int height, u8* image)
{
...
static bm_status_t bmcv_warp_perspective_tpu(
    ...
    if (loop_times % 3 == 1) {
        ...
        gettimeofday_(&t1);
        BM_CHECK_RET(
            bmcv_image_warp_perspective_with_coordinate(handle, image_n, coordinate_image, src_img, dst_img->data, use_bilinear));
        gettimeofday_(&t2);
    }
    if (loop_times % 3 == 2) {
