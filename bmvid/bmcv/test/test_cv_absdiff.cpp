...
static int absdiff_tpu(
    ...
    gettimeofday_(&t1);
    bmcv_image_absdiff(handle, input1_img, input2_img, output_img);
    gettimeofday_(&t2);
    cout << "Absdiff TPU using time: " << ((t2.tv_sec - t1.tv_sec) * 1000000 + t2.tv_usec - t1.tv_usec) << "us" << endl;
...
static int test_absdiff_random(
        int height,
        int width,
        int format) {
    struct timespec tp;
    ...
    absdiff_tpu(
        ...
        height, width,
        format);
    unsigned char md5_opencvOut[] = "6f576ff88006b6bf5c522bacb186e440";
...
int main(int argc, char* args[]) {
    ...
    for (int i = 0; i < loop; i++) {
        ret = test_absdiff_random(height, width, format);
        if (ret) {
            cout << "test absdiff failed" << endl;
            return ret;
        }
    }
