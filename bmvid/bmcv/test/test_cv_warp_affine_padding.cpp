...
static bm_status_t bmcv_warp_tpu(bm_handle_t handle,
    ...
    if (!use_opencv) {
        printf("normal mode\n");
        gettimeofday_(&t1);
        bmcv_image_warp_affine_padding(handle, image_n, matrix_image, src_img, dst_img->data, is_bilinear);
        gettimeofday_(&t2);
    }else{
        printf("opencv mode\n");
        gettimeofday_(&t1);
        bmcv_image_warp_affine_similar_to_opencv_padding(handle, image_n, matrix_image, src_img, dst_img->data, is_bilinear);
        gettimeofday_(&t2);
    }
...
static void test_cv_warp_random_padding(int trials) {
    for (int idx_trial = 0; idx_trial < trials; idx_trial++) {
        auto task = []() {
            ...
            // for (int i = 0; i < output_num; i++){
            //     trans_mat->data[0 + i * 6] = 1;
            //     trans_mat->data[1 + i * 6] = 0;
            //     trans_mat->data[2 + i * 6] = 0;
            //     trans_mat->data[3 + i * 6] = 0;
            //     trans_mat->data[4 + i * 6] = 1;
            //     trans_mat->data[5 + i * 6] = 0;
            // }
