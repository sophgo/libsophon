...
int main(int argc, char **argv) {
  ...
  if (argc != 13) {
    printf("usage: %d\n", argc);
    printf("%s src_w src_h fmt src_name start_x start_y crop_w crop_h dst_name loop_time dst_csv_name dev_id\n", argv[0]);
    printf("example:\n");
    printf("FORMAT_RGB24PACKED\n");
    printf("%s 1920 1080 10 rgb24.bin 0 0 1920 1080 rgb24.dst 1000 fill_rectangle.csv 0\n", argv[0]);
  ...
  for(i = 0;i < loop_time; i++){
    ...
    bmcv_image_fill_rectangle(handle, src, 1, &rect, 255, 255, 255);
  ...
  pixel_per_sec = rect.crop_w * rect.crop_h * fps / 1024 /1024;
  ...
  printf("fill rectangle %d*%d to dst image, %s\n",rect.crop_w,rect.crop_h,fmt_str);
  printf("bmcv_image_fill_rectangle:loop %d cycles, time_avg = %llu, fps %llu, %lluM pps\n\n",loop_time, time_avg, fps, pixel_per_sec);
