#include <iostream>
#include <vector>
#include "bmcv_api_ext.h"
#include "stdio.h"
#include "stdlib.h"
#include <sstream>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#ifdef __linux__
#include <sys/time.h>
#endif

extern void bm_read_bin(bm_image src, const char *input_name);
extern void bm_write_bin(bm_image dst, const char *output_name);
extern void format_to_str(bm_image_format_ext format, char* res);

int main(int argc, char **argv) {

  bm_handle_t handle = NULL;
  int src_h, src_w;
  bm_image_format_ext src_fmt;
  char *src_name, *dst_name, *dst_csv_name;
  bm_image src;
  bmcv_point_t rect;
  int length;
  unsigned int i = 0, loop_time = 0;
  unsigned long long time_single, time_total = 0, time_avg = 0;
  unsigned long long time_max = 0, time_min = 10000, fps = 0, pixel_per_sec = 0;
  int dev_id = 0;

#ifdef __linux__
  struct timeval tv_start;
  struct timeval tv_end;
  struct timeval timediff;
#endif

  if (argc != 12) {
    printf("usage: %d\n", argc);
    printf("%s src_w src_h fmt src_name start_x start_y crop_w crop_h dst_name loop_time dst_csv_name dev_id\n", argv[0]);
    printf("example:\n");
    printf("FORMAT_RGB24PACKED\n");
    printf("%s 1920 1080 10 rgb24.bin 0 0 128 rgb24.dst 1000 point.csv 0\n", argv[0]);

    return 0;
  }

  src_w = atoi(argv[1]);
  src_h = atoi(argv[2]);
  src_fmt = (bm_image_format_ext)atoi(argv[3]);
  src_name = argv[4];
  rect.x = atoi(argv[5]);
  rect.y = atoi(argv[6]);
  length = atoi(argv[7]);
  dst_name = argv[8];
  loop_time = atoi(argv[9]);
  dst_csv_name = argv[10];
  dev_id = atoi(argv[11]);

  bm_status_t ret = bm_dev_request(&handle, dev_id);
  if (ret != BM_SUCCESS) {
    printf("Create bm handle failed. ret = %d\n", ret);
    exit(-1);
  }

  bm_image_create(handle, src_h, src_w, src_fmt, DATA_TYPE_EXT_1N_BYTE, &src);
  bm_image_alloc_dev_mem(src, 1);
  bm_read_bin(src,src_name);

  for(i = 0;i < loop_time; i++){

#ifdef __linux__
    gettimeofday(&tv_start, NULL);
#endif

    bmcv_image_draw_point(handle, src, 1, &rect, length, 255, 255, 255);

#ifdef __linux__
    gettimeofday(&tv_end, NULL);
    timediff.tv_sec  = tv_end.tv_sec - tv_start.tv_sec;
    timediff.tv_usec = tv_end.tv_usec - tv_start.tv_usec;
    time_single = (unsigned int)(timediff.tv_sec * 1000000 + timediff.tv_usec);
#endif

    if(time_single>time_max){time_max = time_single;}
    if(time_single<time_min){time_min = time_single;}
    time_total = time_total + time_single;
  }
  time_avg = time_total / loop_time;
  fps = 1000000 *2 / time_avg;
  pixel_per_sec = length * length * fps / 1024 /1024;

  bm_write_bin(src, dst_name);
  bm_image_destroy(src);

  bm_dev_free(handle);


  char fmt_str[100];
  format_to_str(src.image_format, fmt_str);

  printf("point %d to dst image, %s\n",length,fmt_str);
  printf("bmcv_image_draw_point:loop %d cycles, time_avg = %llu, fps %llu, %lluM pps\n\n",loop_time, time_avg, fps, pixel_per_sec);

  bmlib_log("BMCV",BMLIB_LOG_TRACE, "loop %d cycles, time_max = %llu, time_min = %llu, time_avg = %llu\n",
    loop_time, time_max, time_min, time_avg);

  FILE *fp_csv = fopen(dst_csv_name, "ab+");
  fprintf(fp_csv, "%lld, %lld, %lldM\n",time_avg, fps, pixel_per_sec);
  fclose(fp_csv);

  return 0;
}

