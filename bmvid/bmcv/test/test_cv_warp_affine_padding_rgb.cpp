...
static int padding_r = 0;
static int padding_g = 0;
static int padding_b = 0;
static int flag = 0;
...
u8 fetch_pixel(float x_idx, float y_idx, int c_idx, int width, int height, u8* image, int pixel_value)
{
    float y_compare = y_idx; float x_compare = x_idx;
   ...
   if ((x_compare != x_idx) || (y_compare != y_idx)){
    u8 mix = (u8)pixel_value;
    return mix;
   }
        ...
        for (int y = 0; y < image_dh; y++) {
            for (int x = 0; x < image_dw; x++) {
                ...
                if (got != exp) {
                    ...
                    if (c == 0) {
                        mix_pixel =
                        fetch_pixel(sx, sy, c, image_sw, image_sh, src, padding_r);
                    } else if(c == 1) {
                        mix_pixel =
                        fetch_pixel(sx, sy, c, image_sw, image_sh, src, padding_g);
                    } else if(c == 2) {
                        mix_pixel =
                        fetch_pixel(sx, sy, c, image_sw, image_sh, src, padding_b);
                    } else {
...
static bm_status_t bmcv_affine_nearest_1n_ref(
    ...
    if (image_c == 1) {
        ...
        for (int y = 0; y < dst_h; y++) {
            for (int x = 0; x < dst_w; x++) {
                int x_compare = tensor_DX[y * dst_w + x];
                int sx = bm_min(x_compare, image_sw - 1);
                sx = bm_max(sx, 0);

                int y_compare = tensor_DY[y * dst_w + x];
                int sy = bm_min(y_compare, image_sh - 1);
                sy = bm_max(sy, 0);

                if ((x_compare != sx) || (y_compare != sy)){
                    db[y * dst_w_stride + x * w_stride] = 0;
                } else {
        ...
        for (int y = 0; y < dst_h; y++) {
            for (int x = 0; x < dst_w; x++) {
                ...
                if ((x_compare != sx) || (y_compare != sy)){
                    db[y * dst_w_stride + x * w_stride] = 0;
                    dg[y * dst_w_stride + x * w_stride] = 0;
                    dr[y * dst_w_stride + x * w_stride] = 0;
                } else {
                    db[y * dst_w_stride + x * w_stride] =
                        sb[sy * src_w_stride + sx * w_stride];
    ...
    if (image_c == 3) {
        ...
        for (int y = 0; y < dst_h; y++) {
            for (int x = 0; x < dst_w; x++) {
                ...
                db[y * dst_w_stride + x * w_stride] =
                    fetch_pixel(sx, sy, 0, image_sw, image_sh, sb, padding_r);
                dg[y * dst_w_stride + x * w_stride] =
                    fetch_pixel(sx, sy, 0, image_sw, image_sh, sg, padding_g);
                dr[y * dst_w_stride + x * w_stride] =
                    fetch_pixel(sx, sy, 0, image_sw, image_sh, sr, padding_b);
            }
        }
    }else {
        // warp in source image directly.
...
static bm_status_t bmcv_warp_tpu(bm_handle_t handle,
    ...
    if (!use_opencv) {
        ...
        printf("normal mode\n");
        gettimeofday_(&t1);
        bmcv_image_warp_affine_padding_rgb(handle, image_n, matrix_image, src_img, dst_img->data, padding_r, padding_g, padding_b, is_bilinear);
        gettimeofday_(&t2);
    }else{
        printf("opencv mode\n");
        gettimeofday_(&t1);
        bmcv_image_warp_affine_similar_to_opencv_padding_rgb(handle, image_n, matrix_image, src_img, dst_img->data, padding_r, padding_g, padding_b,is_bilinear);
        gettimeofday_(&t2);
    }
...
static void test_cv_warp_random_padding_rgb(int trials) {
    for (int idx_trial = 0; idx_trial < trials; idx_trial++) {
        auto task = []() {
            ...
            test_cv_warp_single_case(src_data, trans_mat, matrix_num, image_n, image_c,
                                            image_sh, image_sw, image_dh, image_dw, use_opencv, padding_r, padding_g, padding_b, is_bilinear);
        };
        pool->enqueue(task);
    }
    pool->wait_all_done();
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 2){
        ...
        padding_r = atoi(argv[6]);
        padding_g = atoi(argv[7]);
        padding_b = atoi(argv[8]);
    }
    ...
    for (int loop_idx = 0; loop_idx < test_loop_times; loop_idx++) {
        ...
        test_cv_warp_random_padding_rgb(2);
    }
