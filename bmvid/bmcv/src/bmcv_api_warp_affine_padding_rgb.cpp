...
static bm_status_t per_image_bilinear_padding_rgb(bm_handle_t handle,
                        int image_dh,
                        int image_dw,
                        bm_image input,
                        bm_image output,
    ...
    bm_image_get_device_mem(input, &tensor_input);
    bm_image_get_device_mem(output, &tensor_output);
    sg_api_cv_warp_bilinear_padding_rgb_t param;
    int image_sh = input.height;
    int image_sw = input.width;
    param.output_image_addr = bm_mem_get_device_addr(tensor_output);
    param.input_image_addr  = bm_mem_get_device_addr(tensor_input);
    ...
    param.input_format  = (bm_image_data_format_ext)input_format;
    param.output_format = (bm_image_data_format_ext)output_format;
    param.type = 1;   // 0: affine_bilinear
    param.padding_b = padding_b;
    param.padding_g = padding_g;
    param.padding_r = padding_r;
    ...
    ret = bm_tpu_kernel_launch(handle, "cv_api_warp_affine_bilinear_1684x_padding_rgb", &param, sizeof(param));
    if(ret != BM_SUCCESS){
        bmlib_log(BMCV_LOG_TAG, BMLIB_LOG_ERROR,
                "cv_api_warp_affine_bilinear_1684x_padding_rgb error, %s: %s: %d\n",
                filename(__FILE__), __func__, __LINE__);
        goto free_devmem;
    }
...
static bm_status_t per_image_nearest_padding(bm_handle_t handle,
 ...
 bm_status_t bmcv_image_warp_affine_padding_rgb_1684X(
    bm_handle_t handle,
    int image_num,
 ...
 bm_status_t bmcv_image_warp_affine_1684X_padding_rgb(
    bm_handle_t handle,
    int image_num,
    bmcv_affine_image_matrix matrix[4],
    int image_dh,
    ...
    for (int num = 0;num < image_num;num++){
        ...
        if(use_bilinear){
            bm_image_data_format_ext input_format = input[0].data_type;
            bm_image_data_format_ext output_format = output[0].data_type;
            ret = per_image_bilinear_padding_rgb(handle, image_dh, image_dw, input[num], output[num],
                    tensor_output[num], matrix[num], input_format, output_format, padding_r, padding_g, padding_b);
        }else{
    ...
    int use_bilinear)
{
    ...
    return bmcv_image_warp_affine_padding_rgb(handle,
            image_num,
            &matrix[0],
            input, output, padding_r, padding_g, padding_b, use_bilinear);
}
    ...
    int use_bilinear)
{
    ...
    if (chipid == BM1684X){
        return bmcv_image_warp_affine_1684X_padding_rgb(handle,
                image_num,
                &matrix[0],
                output->height,
                output->width,
