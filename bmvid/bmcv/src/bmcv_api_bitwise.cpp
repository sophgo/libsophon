...
static bm_status_t bmcv_bitwise_check(
        bm_handle_t handle,
        bm_image input1,
        bm_image input2,
        bm_image output) {
...
bm_status_t bmcv_image_bitwise(
        bm_handle_t handle,
        bm_image input1,
        bm_image input2,
        bm_image output,
    ...
    {
        case 0x1684:
            ret = bm_send_api(handle,  BM_API_ID_CV_BITWISE, (uint8_t*)&api, sizeof(api));
            if (BM_SUCCESS != ret) {
                bmlib_log("BITWISE", BMLIB_LOG_ERROR, "bitwise send api error\n");
        ...
        case BM1684X:
            ret = bm_tpu_kernel_launch(handle, "cv_bitwise", (u8 *)&api, sizeof(api));
            if (BM_SUCCESS != ret) {
                bmlib_log("BITWISE", BMLIB_LOG_ERROR, "bitwise sync api error\n");
                if (output_alloc_flag) {
                    for (int i = 0; i < channel; i++) {
            ...
            // tpu_kernel_launch_sync(handle, "cv_bitwise", &api, sizeof(api));
            break;
