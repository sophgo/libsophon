...
static bm_status_t bmcv_add_weighted_check(
        bm_handle_t handle,
        bm_image input1,
        bm_image input2,
        bm_image output) {
...
bm_status_t bmcv_image_add_weighted(
    ...
    {
        case 0x1684:
            ret = bm_send_api(handle, BM_API_ID_CV_ADD_WEIGHTED, (uint8_t*)&api, sizeof(api));
            if (BM_SUCCESS != ret) {
                bmlib_log("ADD_WEIGHTED", BMLIB_LOG_ERROR, "add_weighted send api error\n");
        ...
        case BM1684X:
            ret = bm_tpu_kernel_launch(handle, "cv_add_weighted", (u8 *)&api, sizeof(api));
            if (BM_SUCCESS != ret) {
                bmlib_log("ADD_WEIGHTED", BMLIB_LOG_ERROR, "add_weighted sync api error\n");
                if (output_alloc_flag) {
                    for (int i = 0; i < channel; i++) {
