...
bm_status_t bmcv_image_remap(bm_handle_t handle, bm_image input, bm_image output, bm_device_mem_t mapx_data_global_addr,
    ...
    bm_device_mem_t bilinear_y0x0_buffer_addr;
    bm_device_mem_t bilinear_y0x1_buffer_addr;
    bm_device_mem_t bilinear_y1x0_buffer_addr;
    bm_device_mem_t bilinear_y1x1_buffer_addr;
    bm_device_mem_t bilinear_flag_buffer_addr;
    ret = bm_malloc_device_byte(handle, &bilinear_y0x0_buffer_addr, output.width * output.height * channel * sizeof(unsigned int));
    if (BM_SUCCESS != ret) {
        bmlib_log("REMAP", BMLIB_LOG_ERROR, "bm_malloc_device_byte bilinear_y0x0_buffer_addr error\n");
        return ret;
    }
    ret = bm_malloc_device_byte(handle, &bilinear_y0x1_buffer_addr, output.width * output.height * channel * sizeof(unsigned int));
    if (BM_SUCCESS != ret) {
        bmlib_log("REMAP", BMLIB_LOG_ERROR, "bm_malloc_device_byte bilinear_y0x1_buffer_addr error\n");
        return ret;
    }
    ret = bm_malloc_device_byte(handle, &bilinear_y1x0_buffer_addr, output.width * output.height * channel * sizeof(unsigned int));
    if (BM_SUCCESS != ret) {
        bmlib_log("REMAP", BMLIB_LOG_ERROR, "bm_malloc_device_byte bilinear_y1x0_buffer_addr error\n");
        return ret;
    }
    ret = bm_malloc_device_byte(handle, &bilinear_y1x1_buffer_addr, output.width * output.height * channel * sizeof(unsigned int));
    if (BM_SUCCESS != ret) {
        bmlib_log("REMAP", BMLIB_LOG_ERROR, "bm_malloc_device_byte bilinear_y1x1_buffer_addr error\n");
        return ret;
    }
    ret = bm_malloc_device_byte(handle, &bilinear_flag_buffer_addr, output.width * output.height * channel * sizeof(unsigned char));
    if (BM_SUCCESS != ret) {
        bmlib_log("REMAP", BMLIB_LOG_ERROR, "bm_malloc_device_byte bilinear_flag_buffer_addr error\n");
        return ret;
    }
    ...
    api.bilinear_y0x0_buffer_addr = bm_mem_get_device_addr(bilinear_y0x0_buffer_addr);
    api.bilinear_y0x1_buffer_addr = bm_mem_get_device_addr(bilinear_y0x1_buffer_addr);
    api.bilinear_y1x0_buffer_addr = bm_mem_get_device_addr(bilinear_y1x0_buffer_addr);
    api.bilinear_y1x1_buffer_addr = bm_mem_get_device_addr(bilinear_y1x1_buffer_addr);
    api.bilinear_flag_buffer_addr = bm_mem_get_device_addr(bilinear_flag_buffer_addr);
    unsigned int chipid;
    ...
    bm_free_device(handle, bilinear_y0x0_buffer_addr);
    bm_free_device(handle, bilinear_y0x1_buffer_addr);
    bm_free_device(handle, bilinear_y1x0_buffer_addr);
    bm_free_device(handle, bilinear_y1x1_buffer_addr);
    bm_free_device(handle, bilinear_flag_buffer_addr);
    return ret;
}