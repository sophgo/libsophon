...
int I422ToNV12(const uint8_t* src_y,
               ...
               int width,
               int height) {
  return I422ToNV21(src_y, src_stride_y,
                    src_v, src_stride_v,
                    src_u, src_stride_u,
                    dst_y, dst_stride_y,
...
int I444ToNV12(const uint8_t* src_y,
               ...
               int width,
               int height) {
  return I444ToNV21(src_y, src_stride_y,
                    src_v, src_stride_v,
                    src_u, src_stride_u,
                    dst_y, dst_stride_y,
                    dst_uv, dst_stride_uv,
...
int I400ToNV12(const uint8_t* src_y,
               ...
               int width,
               int height) {
  return I400ToNV21(src_y, src_stride_y,
                    dst_y, dst_stride_y,
                    dst_uv, dst_stride_uv,
                    width, height);
}
...
#elif defined(HAS_MERGEUVROW_MSA)
  if (TestCpuFlag(kCpuHasMSA)) {
    MergeUVRow = MergeUVRow_Any_MSA;
    if (IS_ALIGNED(width_uv, 16)) {
      MergeUVRow = MergeUVRow_MSA;
...
#elif defined(HAS_MERGEUVROW_MMI)
  if (TestCpuFlag(kCpuHasMMI)) {
    MergeUVRow = MergeUVRow_Any_MMI;
    if (IS_ALIGNED(width_uv, 8)) {
      MergeUVRow = MergeUVRow_MMI;
    }
  }
#elif defined(HAS_MERGEUVROW_SSE2)
  if (TestCpuFlag(kCpuHasSSE2)) {
    MergeUVRow = MergeUVRow_Any_SSE2;
    if (IS_ALIGNED(width_uv, 16)) {
      MergeUVRow = MergeUVRow_SSE2;
...
#elif defined(HAS_MERGEUVROW_AVX2)
  if (TestCpuFlag(kCpuHasAVX2)) {
    MergeUVRow = MergeUVRow_Any_AVX2;
    if (IS_ALIGNED(width_uv, 32)) {
      MergeUVRow = MergeUVRow_AVX2;
    }
  }
#endif
...
#endif
    align_buffer_64(row_u, width_uv);
    align_buffer_64(row_v, width_uv);
...
#endif
      MergeUVRow(row_u, row_v, dst_uv, width_uv);
...
int RAWToNV12(const uint8_t* src_raw,
  ...
  void (*RAWToYRow)(const uint8_t* src_raw, uint8_t* dst_y, int width) =
      RAWToYRow_C;
#else
  void (*RAWToARGBRow)(const uint8_t* src_raw, uint8_t* dst_argb, int width) =
      RAWToARGBRow_C;
  void (*ARGBToUVRow)(const uint8_t* src_argb0, int src_stride_argb,
                      uint8_t* dst_u, uint8_t* dst_v, int width) =
      ARGBToUVRow_C;
