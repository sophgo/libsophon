...
void I444ToRAWRow_C(const uint8_t* src_y,
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    YuvPixel(src_y[0], u, v, rgb_buf + 2, rgb_buf + 1, rgb_buf + 0,
             yuvconstants);
    YuvPixel(src_y[1], u, v, rgb_buf + 5, rgb_buf + 4, rgb_buf + 3,
             yuvconstants);
    src_y += 2;
    src_u += 2;
    src_v += 2;
...
void I400ToRGB24Row_C(const uint8_t* src_y, uint8_t* rgb_buf, int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
    YPixel(src_y[0], rgb_buf + 0, rgb_buf + 1, rgb_buf + 2);
    YPixel(src_y[1], rgb_buf + 3, rgb_buf + 4, rgb_buf + 5);
    src_y += 2;
    rgb_buf += 6;  // Advance 2 pixels.
  }
  if (width & 1) {
...
void I400ToRAWRow_C(const uint8_t* src_y, uint8_t* rgb_buf, int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
    YPixel(src_y[0], rgb_buf + 2, rgb_buf + 1, rgb_buf + 0);
    YPixel(src_y[1], rgb_buf + 5, rgb_buf + 4, rgb_buf + 3);
    src_y += 2;
    rgb_buf += 6;  // Advance 2 pixels.
  }
  if (width & 1) {
