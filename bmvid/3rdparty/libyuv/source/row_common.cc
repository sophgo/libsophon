...
void RGBToRGB565Row_C(const uint8_t* src_rgb, uint8_t* dst_rgb, int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
    uint8_t b0 = src_rgb[0] >> 3;
    uint8_t g0 = src_rgb[1] >> 2;
    uint8_t r0 = src_rgb[2] >> 3;
    uint8_t b1 = src_rgb[3] >> 3;
    uint8_t g1 = src_rgb[4] >> 2;
    uint8_t r1 = src_rgb[5] >> 3;
    WRITEWORD(dst_rgb, b0 | (g0 << 5) | (r0 << 11) | (b1 << 16) | (g1 << 21) |
                           (r1 << 27));
    dst_rgb += 4;
    src_rgb += 6;
...
void RGBToBGR565Row_C(const uint8_t* src_rgb, uint8_t* dst_rgb, int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
    uint8_t r0 = src_rgb[0] >> 3;
    uint8_t g0 = src_rgb[1] >> 2;
    uint8_t b0 = src_rgb[2] >> 3;
    uint8_t r1 = src_rgb[3] >> 3;
    uint8_t g1 = src_rgb[4] >> 2;
    uint8_t b1 = src_rgb[5] >> 3;
    WRITEWORD(dst_rgb, b0 | (g0 << 5) | (r0 << 11) | (b1 << 16) | (g1 << 21) |
                           (r1 << 27));
    dst_rgb += 4;
    src_rgb += 6;
...
void RGB24ToUVJ444Row_C(const uint8_t* src_rgb24,
  ...
  for (x = 0; x < width; ++x) {
    ...
    uint8_t g = src_rgb24[1];
    uint8_t r = src_rgb24[2];
    dst_u[0] = RGBToUJ(r, g, b);
    dst_v[0] = RGBToVJ(r, g, b);
    src_rgb24 += 3;
    dst_u += 1;
    dst_v += 1;
  }
...
void RAWToJYVURow_C(const uint8_t* src_rgb24, uint8_t* dst_yuv, int width) {
  int x;
  for (x = 0; x < width; ++x) {
    uint8_t r = src_rgb24[0];
    uint8_t g = src_rgb24[1];
    uint8_t b = src_rgb24[2];
    dst_yuv[0] = RGBToYJ(r, g, b);
    dst_yuv[2] = RGBToUJ(r, g, b);
    dst_yuv[1] = RGBToVJ(r, g, b);
    src_rgb24 += 3;
    dst_yuv += 3;
  }
}
...
void RAWToYUVRow_C(const uint8_t* src_rgb24, uint8_t* dst_yuv, int width) {
  ...
  for (x = 0; x < width; ++x) {
    ...
    uint8_t b = src_rgb24[2];
    dst_yuv[0] = RGBToYJ(r, g, b);
    dst_yuv[1] = (0x8080+126*b-126*dst_yuv[0]) >> 8;
    dst_yuv[2] = (0x8080+224*r-224*dst_yuv[0]) >> 8;
    src_rgb24 += 3;
    dst_yuv += 3;
  }
}
...
void J400ToRGB24Row_C(const uint8_t* src_y, uint8_t* dst_rgb24, int width) {
  ...
  for (x = 0; x < width; ++x) {
    uint8_t y = src_y[0];
    dst_rgb24[2] = dst_rgb24[1] = dst_rgb24[0] = y;
    dst_rgb24 += 3;
    ++src_y;
  }
}
...
void I400ToRGB24Row_C(const uint8_t* src_y, uint8_t* rgb_buf, const struct YuvConstants* yuvconstants, int width) {
  ...
  for (x = 0; x < width - 1; x += 2) {
    YPixel(src_y[0], rgb_buf + 0, rgb_buf + 1, rgb_buf + 2, yuvconstants);
    YPixel(src_y[1], rgb_buf + 3, rgb_buf + 4, rgb_buf + 5, yuvconstants);
    src_y += 2;
    rgb_buf += 6;  // Advance 2 pixels.
  }
  if (width & 1) {
...
void I400ToRAWRow_C(const uint8_t* src_y, uint8_t* rgb_buf, const struct YuvConstants* yuvconstants, int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
    YPixel(src_y[0], rgb_buf + 2, rgb_buf + 1, rgb_buf + 0, yuvconstants);
    YPixel(src_y[1], rgb_buf + 5, rgb_buf + 4, rgb_buf + 3, yuvconstants);
    src_y += 2;
    rgb_buf += 6;  // Advance 2 pixels.
  }
  if (width & 1) {
 ...
 *   yj = (y-16)*255.0/219.0
 */
#define COEF0 42 /* floor((255-(235-16))*1.0/(235-16)*256+0.5) */
void YToYJRow_C(const uint8_t* src_y, uint8_t* dst_yj, int width) {
  for (int x = 0; x < width; x++) {
    uint8_t  ym16 = src_y[x] - 16;
    uint16_t t0   = ym16 * COEF0 + 128;
    uint8_t  t1   = t0 >> 8;
    uint8_t  t2   = ym16 + t1;
    dst_yj[x] = t2;
  }
 ...
 */
#define COEF1 35 /* floor((255-(240-16))*1.0/(240-16)*256+0.5) */
void UVToUVJRow_C(const uint8_t* src_uv, uint8_t* dst_uvj, int width) {
  for (int x = 0; x < width; x++) {
    uint8_t c     = src_uv[x];
    int8_t  cm128 = c - 128;
    int16_t t0    = cm128 * COEF1 + 128;
    int8_t  t1    = t0 >> 8;
    dst_uvj[x] = c + t1;
  }
}

void UVToUVJRow2_C(const uint8_t* src_uv, uint8_t* dst_uj, uint8_t* dst_vj,
                   int width) {
  int width_uv = (width+1) & (~1);
  for (int x = 0; x < width_uv; x+=2) {
    int xd = x>>1;

    uint8_t u     = src_uv[x];
    uint8_t v     = src_uv[x+1];

    int8_t  um128 = u - 128;
    int8_t  vm128 = v - 128;

    int16_t ut0   = um128 * COEF1 + 128;
    int16_t vt0   = vm128 * COEF1 + 128;
