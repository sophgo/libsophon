...
#define skip_bits(CTX, NUM) JpuGbuSkipBit(CTX, NUM)
...
typedef struct {
    ...
    int streamBufSize;
    BYTE *pBitStream;
    int BitStreamByteSize;  // actual size for jpeg data, used for software parser
    ...
    Uint32 huffMin[4][16];  // TODO uint16_t
    Uint32 huffMax[4][16];  // TODO uint16_t
    BYTE huffPtr[4][16];
...
extern "C" {
  ...
  //  JpgRet InitJpgInstancePool(void);
	...
	void JpuGbuSkipBit(jpu_getbit_context_t *ctx, int bit_num);
