...
unsigned int  heap_flag = BM_JPU_ALLOCATION_FLAG_DEFAULT;
           ...
           do
            {
                ...
                for (i=0; i < multiConfig.numMulti; i++)
                {
                    ...
                    if (atoi(skip_str) == 0)
                    {
                        fprintf(stderr, "skip_str %d err\n", atoi(skip_str));
                        return -1;
                    }

                    if (atoi(str_type) != 0 && atoi(str_type) != 1)
                    {
                        fprintf(stderr, "str_type %d err\n", atoi(str_type));
                        return -1;
                    }

                    if (atoi(str_device_index) < 0 || atoi(str_device_index) >=64)
                    {
                        fprintf(stderr, "sophon idx :%d error, must be 0~63\n", atoi(str_device_index));
                        return -1;
                    }
                    multiConfig.multiMode[i] = atoi(str_type);
                    if (multiConfig.multiMode[i]) //decoder
                    {
                        ...
                        multiConfig.decConfig[i].device_index = atoi(str_device_index);
                        if( 0 == jpu_slt_en)
                        {
                        ...
                        multiConfig.decConfig[i].Skip = atoi(skip_str);
                    }
                    else
                    {
                        ...
                        multiConfig.encConfig[i].Skip = atoi(skip_str);
...
static int parse_args(int argc, char **argv,  void* pConfig, CodecType codecType)
{
    ...
    if(codecType == DEC)
    {
        ...
        while ((opt = getopt(argc, argv, "t:i:s:f:w:h:o:r:n:g:")) != -1)
        {
            switch (opt)
            {
            case 't':
