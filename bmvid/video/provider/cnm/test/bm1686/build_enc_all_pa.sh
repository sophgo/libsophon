...
function build_vpu()
{
    ...
    make TEST_CASE=wave_enc NPU_EN=0 RUN_ENV=DDR DEBUG=1 PLATFORM=ASIC  CHIP=BM1686 SYSTEM_ID=${sy_id} CHANNEL_ID=${ch_id} MMU_PAGE=${mmu_page} REMAP_MMU=${remap} MULTI_CHAN=${multch} ENABLE_MMU=${enable_mmu} SUBTYPE=palladium clean
    make TEST_CASE=wave_enc NPU_EN=0 RUN_ENV=DDR DEBUG=1 PLATFORM=ASIC  CHIP=BM1686 SYSTEM_ID=${sy_id} CHANNEL_ID=${ch_id} MMU_PAGE=${mmu_page} REMAP_MMU=${remap} MULTI_CHAN=${multch} ENABLE_MMU=${enable_mmu} SUBTYPE=palladium MEM_START=0x410000000 MEM_SIZE=0x20000000

    (cp out/BM1686_wave_enc.elf run/BM1686_wave_enc_s${sy_id}c${ch_id}_${type}_nommu.elf)
    (cp out/BM1686_wave_enc.map run/BM1686_wave_enc_s${sy_id}c${ch_id}_${type}_nommu.map)
    (cp out/target.ld run/BM1686_wave_enc_s${sy_id}c${ch_id}_${type}_nommu.ld)
    (aarch64-elf-objdump -D run/BM1686_wave_enc_s${sy_id}c${ch_id}_${type}_nommu.elf > run/BM1686_wave_enc_s${sy_id}c${ch_id}_${type}_nommu.txt)
}
