...
WMMU_DEFINES := SYSTEM_ID=${SYSTEM_ID} CHANNEL_ID=${CHANNEL_ID} ENABLE_MMU=${ENABLE_MMU} REMAP_MMU=${REMAP_MMU} MULTI_CHAN=${MULTI_CHAN} MMU_PAGE=${MMU_PAGE} USE_MEMSET=1

ifeq ($(LIB_VID_EXIST), )
PHONY: vid_lib all
vid_lib: $(ROOT)/../../driver/Makefile
	make -C $(ROOT)/../../driver -f $^ clean
	make -C $(ROOT)/../../driver -f $^ CHIP=bm1684 PRODUCT=wave OS=none BUILD_ORG_TEST=yes DEBUG=1 PRODUCTFORM=soc SUBTYPE=${SUBTYPE} ${WMMU_DEFINES}
