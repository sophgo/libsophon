...
function build_vpu()
{
    ...
    make TEST_CASE=wave NPU_EN=0 RUN_ENV=DDR DEBUG=1 PLATFORM=ASIC  CHIP=BM1684 SYSTEM_ID=${sy_id} CHANNEL_ID=${ch_id} MMU_PAGE=${mmu_page} REMAP_MMU=${remap} MULTI_CHAN=${multch} ENABLE_MMU=${enable_mmu} clean
    make TEST_CASE=wave NPU_EN=0 RUN_ENV=DDR DEBUG=1 PLATFORM=ASIC  CHIP=BM1684 SYSTEM_ID=${sy_id} CHANNEL_ID=${ch_id} MMU_PAGE=${mmu_page} REMAP_MMU=${remap} MULTI_CHAN=${multch} ENABLE_MMU=${enable_mmu}
    
    make TEST_CASE=wave NPU_EN=0 RUN_ENV=DDR DEBUG=1 PLATFORM=ASIC  CHIP=BM1684  SYSTEM_ID=${sy_id} CHANNEL_ID=${ch_id} MMU_PAGE=${mmu_page} REMAP_MMU=${remap} MULTI_CHAN=${multch} ENABLE_MMU=${enable_mmu}
    
    (cp out/BM1684_wave.elf run/BM1684_wave_s${sy_id}c${ch_id}_${type}_mmu.elf)
    (aarch64-elf-objdump -D run/BM1684_wave_s${sy_id}c${ch_id}_${type}_mmu.elf > run/BM1684_wave_s${sy_id}c${ch_id}_${type}_mmu.txt)
}
