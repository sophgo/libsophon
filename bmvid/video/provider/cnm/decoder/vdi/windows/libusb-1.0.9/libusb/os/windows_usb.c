...
static char* sanitize_path(const char* path)
{
    ...
    if ((ret_path = (char*)malloc(size)) == NULL)
        return NULL;

    memset(ret_path, 0x00, size);
    ...
    HDEVINFO *dev_info, SP_DEVINFO_DATA *dev_info_data, const GUID* guid, unsigned _index)
{
    ...
    memset(dev_interface_details, 0x00, size);
...
static int htab_create(struct libusb_context *ctx, unsigned long nel)
{
    ...
    htab_table = (htab_entry*)malloc((htab_size + 1)*sizeof(htab_entry));
    if (htab_table == NULL) {
        usbi_err(ctx, "could not allocate space for hash table");
        return 0;
    }
    memset(htab_table, 0x00,(htab_size + 1)*sizeof(htab_entry));
    return 1;
}
...
static unsigned long htab_hash(char* str)
{
    ...
    memset(htab_table[idx].str, 0x00, safe_strlen(str)+1);
...
static int windows_assign_endpoints(struct libusb_device_handle *dev_handle, int iface, int altsetting)
{
    ...
    memset(priv->usb_interface[iface].endpoint, 0x00,if_desc->bNumEndpoints);
...
static int cache_config_descriptors(struct libusb_device *dev, HANDLE hub_handle, char* device_id)
{
    ...
    priv->config_descriptor = (unsigned char**) malloc(dev->num_configurations*sizeof(PUSB_CONFIGURATION_DESCRIPTOR));
    if (priv->config_descriptor == NULL)
        return LIBUSB_ERROR_NO_MEM;

    memset(priv->config_descriptor, 0x00, dev->num_configurations*sizeof(PUSB_CONFIGURATION_DESCRIPTOR));
    ...
    for (i=0, r=LIBUSB_SUCCESS; ; i++)
    {
        ...
        memset(cd_buf_actual, 0x00, size);
        ...
        memset(priv->config_descriptor[i], 0x00, cd_data->wTotalLength);
...
#define DEV_PASS 3
    ...
    unref_list = (libusb_device**) malloc(unref_size*sizeof(libusb_device*));
    if (unref_list == NULL) {
        return LIBUSB_ERROR_NO_MEM;
    }

    memset(unref_list, 0x00, unref_size*sizeof(libusb_device*));
        ...
        for (i = 0; ; i++) {
            ...
            case GEN_PASS:
                ...
                if (key != INVALID_HANDLE_VALUE) {
                    ...
                    if (s == ERROR_SUCCESS) {
                        ...
                        memset(if_guid, 0x00, sizeof(GUID));
            ...
            case DEV_PASS:
                ...
                default:
                    ...
                    priv->usb_interface[0].path = (char*) malloc(safe_strlen(priv->path)+1);
                    if (priv->usb_interface[0].path != NULL) {
                        memset(priv->usb_interface[0].path, 0x00, safe_strlen(priv->path)+1);
