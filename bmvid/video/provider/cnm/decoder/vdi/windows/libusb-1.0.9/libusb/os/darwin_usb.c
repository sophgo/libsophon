...
static int submit_iso_transfer(struct usbi_transfer *itransfer) {
  ...
  if (!tpriv->isoc_framelist) {
    ...
    tpriv->isoc_framelist = (IOUSBIsocFrame*) malloc (transfer->num_iso_packets*sizeof(IOUSBIsocFrame));
    if (!tpriv->isoc_framelist)
      return LIBUSB_ERROR_NO_MEM;
    memset(tpriv->isoc_framelist, 0x00, transfer->num_iso_packets*sizeof(IOUSBIsocFrame));
  }
