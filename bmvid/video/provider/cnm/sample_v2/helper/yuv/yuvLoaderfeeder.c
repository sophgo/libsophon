    ...
    int pic_width, int pic_height, int out_width, int out_height, int frm_format, int packedFormat)
{
    ...
    if ( packedFormat == NOT_PACKED )
    {
        if (frm_format == OUTPUT_PLANAR) {      // planar
            ...
            VLOG(ERR, "Error) Unknown frame format: %d\n", frm_format);
        }
    } else {/* packdformat */
...
#endif
            osal_fputc(tmp16&0xff, fp);
            osal_fputc((tmp16>>8)&0xff, fp);
    ...
    //***** 32bit
        ...
        for (y=0; y<height; y++) {
            for (x=0; x<width; x=x+3) {
                ...
                osal_fputc((tmp32>>24)&0xff, fp);
                osal_fputc((tmp32>>16)&0xff, fp);
                osal_fputc((tmp32>>8)&0xff, fp);
                osal_fputc(tmp32&0xff, fp);
            }
        }
    ...
    )
{
    ...
    Uint32      subFrameSyncSrcWriteMode = pEncInfo->subFrameSyncConfig.subFrameSyncSrcWriteMode;
    ...
    if ( bitdepth == 0 ) {
        ...
        if ( subFrameSyncEn && (subFrameSyncSrcWriteMode & REMAIN_SRC_DATA_WRITE) == 0)
            osal_fseek(yuvFp, -((long)srcFrameSize), SEEK_CUR);
