...
static int GetValue(osal_file_t fp, char *para, char *value)
{
	...
	while (1) {
		if (osal_fgets(lineStr, 256, fp) == NULL)
			return 0;
		osal_sscanf(lineStr, "%s %s", paraStr, value);
		if (paraStr[0] != ';') {
			if (osal_strcmp(para, paraStr) == 0)
				return 1;
		}
	}
} 
...
static int GetValues(osal_file_t fp, char *para, int *values, int num)
{
    ...
    while (1)
    {
        ...
        if (osal_fgets(line, sizeof(line)-1, fp) == NULL)
            return 0;

        // empty line
        if ((str = osal_strtok(line, " ")) == NULL)
            continue;

        if(osal_strcmp(str, para) != 0)
            continue;

        for (i=0; i<num; i++)
        {
            if ((str = osal_strtok(NULL, " ")) == NULL)
                return 1;
            if (!isdigit((Int32)str[0]))
                return 1;
            values[i] = osal_atoi(str);
        }
        return 1;
    }
}
...
int parseMp4CfgFile(ENC_CFG *pEncCfg, char *FileName)
{
	...
	if (GetValue(fp, "YUV_SRC_IMG", sValue) == 0)
		...
		osal_strcpy(pEncCfg->SrcFileName, sValue);
...
int parseAvcCfgFile(ENC_CFG *pEncCfg, char *FileName)
{
    ...
    if (GetValue(fp, "FRAME_NUMBER_ENCODED", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->NumFrame = osal_atoi(sValue);
    if (GetValue(fp, "PICTURE_WIDTH", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->PicX = osal_atoi(sValue);
    if (GetValue(fp, "PICTURE_HEIGHT", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->PicY = osal_atoi(sValue);
    if (GetValue(fp, "FRAME_RATE", sValue) == 0)
        goto __end_parseAvcCfgFile;
    {
        double frameRate;
    ...
    if (GetValue(fp, "CONSTRAINED_INTRA", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->ConstIntraPredFlag = osal_atoi(sValue);
    if (GetValue(fp, "DISABLE_DEBLK", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->DisableDeblk = osal_atoi(sValue);
    if (GetValue(fp, "DEBLK_ALPHA", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->DeblkOffsetA = osal_atoi(sValue);
    if (GetValue(fp, "DEBLK_BETA", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->DeblkOffsetB = osal_atoi(sValue);
    if (GetValue(fp, "CHROMA_QP_OFFSET", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->ChromaQpOffset = osal_atoi(sValue);
    ...
    {
        pEncCfg->level = osal_atoi(sValue);
    ...
    if (GetValue(fp, "PIC_QP_Y", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->PicQpY = osal_atoi(sValue);
    if (GetValue(fp, "GOP_PIC_NUMBER", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->GopPicNum = osal_atoi(sValue);
    if (GetValue(fp, "IDR_INTERVAL", sValue) == 0)
        pEncCfg->IDRInterval = 0;
    else
        pEncCfg->IDRInterval = osal_atoi(sValue);
    if (GetValue(fp, "SLICE_MODE", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->SliceMode = osal_atoi(sValue);
    if (GetValue(fp, "SLICE_SIZE_MODE", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->SliceSizeMode = osal_atoi(sValue);
    if (GetValue(fp, "SLICE_SIZE_NUMBER", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->SliceSizeNum = osal_atoi(sValue);	
    if (GetValue(fp, "AUD_ENABLE", sValue) == 0)
        pEncCfg->aud_en = 0;
    else
        pEncCfg->aud_en = osal_atoi(sValue);	
    ...
    */
    ...
    if (GetValue(fp, "WEIGHT_INTRA_COST", sValue) == 0)
        ...
        pEncCfg->intraCostWeight = osal_atoi(sValue);
    ...
    */
    ...
    if (GetValue(fp, "ME_USE_ZERO_PMV", sValue) == 0)
        ...
        pEncCfg->MeUseZeroPmv = osal_atoi(sValue);

    if (GetValue(fp, "ME_BLK_MODE_ENABLE", sValue) == 0)
        pEncCfg->MeBlkModeEnable = 0;
    else
        pEncCfg->MeBlkModeEnable = osal_atoi(sValue);

    if (GetValue(fp, "RATE_CONTROL_ENABLE", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->RcEnable = osal_atoi(sValue);
    if (GetValue(fp, "BIT_RATE_KBPS", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->RcBitRate = osal_atoi(sValue);
    if (GetValue(fp, "DELAY_IN_MS", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->RcInitDelay = osal_atoi(sValue);

    if (GetValue(fp, "VBV_BUFFER_SIZE", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->RcBufSize = osal_atoi(sValue);
    if (GetValue(fp, "INTRA_MB_REFRESH", sValue) == 0)
        goto __end_parseAvcCfgFile;
    pEncCfg->IntraRefreshNum = osal_atoi(sValue);
    ...
    if (pEncCfg->IntraRefreshNum > 0)
    {
        if (GetValue(fp, "CONSC_INTRA_REFRESH_EN", sValue) == 0)
            ...
            pEncCfg->ConscIntraRefreshEnable = osal_atoi(sValue);
    }

    if (GetValue(fp, "FRAME_SKIP_DISABLE", sValue) == 0)
        pEncCfg->frameSkipDisable = 0;
    else
        pEncCfg->frameSkipDisable = osal_atoi(sValue);

    if (GetValue(fp, "CONST_INTRAQP_ENABLE", sValue) == 0)
        pEncCfg->ConstantIntraQPEnable = 0;
    else
        pEncCfg->ConstantIntraQPEnable = osal_atoi(sValue);

    if (GetValue(fp, "RC_INTRA_QP", sValue) == 0)
        pEncCfg->RCIntraQP = 0;
    else
        pEncCfg->RCIntraQP = osal_atoi(sValue);
    if (GetValue(fp, "MAX_QP_SET_ENABLE", sValue) == 0)
        pEncCfg->MaxQpSetEnable= 0;
    else
        pEncCfg->MaxQpSetEnable = osal_atoi(sValue);

    if (GetValue(fp, "MAX_QP", sValue) == 0)
        pEncCfg->MaxQp = 0;
    else
        pEncCfg->MaxQp = osal_atoi(sValue);

    if (GetValue(fp, "MIN_QP_SET_ENABLE", sValue) == 0)
        pEncCfg->MinQpSetEnable= 0;
    else
        pEncCfg->MinQpSetEnable = osal_atoi(sValue);
    if (GetValue(fp, "MIN_QP", sValue) == 0)
        pEncCfg->MinQp = 0;
    else
        pEncCfg->MinQp = osal_atoi(sValue);

    if (GetValue(fp, "MAX_DELTA_QP_SET_ENABLE", sValue) == 0)
        pEncCfg->MaxDeltaQpSetEnable= 0;
    else
        pEncCfg->MaxDeltaQpSetEnable = osal_atoi(sValue);
    if (GetValue(fp, "MAX_DELTA_QP", sValue) == 0)
        pEncCfg->MaxDeltaQp = 0;
    else
        pEncCfg->MaxDeltaQp = osal_atoi(sValue);

    if (GetValue(fp, "MIN_DELTA_QP_SET_ENABLE", sValue) == 0)
        pEncCfg->MinDeltaQpSetEnable= 0;
    else
        pEncCfg->MinDeltaQpSetEnable = osal_atoi(sValue);
    if (GetValue(fp, "MIN_DELTA_QP", sValue) == 0)
        pEncCfg->MinDeltaQp = 0;
    else
        pEncCfg->MinDeltaQp = osal_atoi(sValue);

    if (GetValue(fp, "GAMMA_SET_ENABLE", sValue) == 0)
        pEncCfg->GammaSetEnable = 0;
    else
        pEncCfg->GammaSetEnable = osal_atoi(sValue);
    if (GetValue(fp, "GAMMA", sValue) == 0)
        pEncCfg->Gamma = 0;
    else
        pEncCfg->Gamma = osal_atoi(sValue);
    /* CODA960 features */
    if (GetValue(fp, "RC_INTERVAL_MODE", sValue) == 0)
        pEncCfg->rcIntervalMode = 0;
    else
        pEncCfg->rcIntervalMode = osal_atoi(sValue);

    if (GetValue(fp, "RC_MB_INTERVAL", sValue) == 0)
        pEncCfg->RcMBInterval = 0;
    else
        pEncCfg->RcMBInterval = osal_atoi(sValue);
    /***************************************/
    if (GetValue(fp, "RC_INTERVAL_MODE", sValue) == 0)
        pEncCfg->rcIntervalMode = 0;
    else
    ...
    if (GetValue(fp, "SEARCH_RANGE", sValue) == 0)
        ...
        pEncCfg->SearchRange = osal_atoi(sValue);
    ...
    )
{
    ...
    while (1) {
        ...
        token = osal_strtok(lineStr, ": "); // parameter name is separated by ' ' or ':'
        if( token != NULL ) {
            if ( osal_strcasecmp(para, token) == 0) { // check parameter name
                token = osal_strtok(NULL, ":\r\n");
                if ( token && osal_strlen(token) == 1 && osal_strncmp(token, " ", 1) == 0) //check space - ex) Frame1  : P  1  0  0
                    token = osal_strtok(NULL, ":\r\n");
                if ( token == NULL )
                    return -1;
                osal_memcpy( valueStr, token, osal_strlen(token) );
                while( valueStr[pos] == ' ' ) { // check space
                    pos++;
                }
                if ( valueStr[pos] == 0 )
                    return -1;//no value
                osal_strcpy(value, &valueStr[pos]);
                return 1;
            }
            else {
                continue;
    ...
    )
{
    ...
    numParsed = osal_sscanf(lineStr, "%c %d %d %d %d %d", 
    ...
    )
{
    ...
    numParsed = osal_sscanf(lineStr, "%c %d %d %d %d", 
        &sliceType, &gopPicParam->pocOffset, &gopPicParam->picQp, 
    ...
    )
{
    ...
    *roiQp = 0;
    ...
    numParsed = osal_sscanf(lineStr, "%d %d %d %d %d", 
    ...
    )
{
    ...
    fp = osal_fopen(FileName, "r");
    if (fp == NULL) {
        VLOG(ERR, "file open err : %s, errno\n", FileName/*, errno*/);
        return ret;
    }

    if (WAVE_GetStringValue(fp, "BitstreamFile", sValue) == 1)
        osal_strcpy(pEncCfg->BitStreamFileName, sValue);
...
#define FIXED_BIT_RATIO 49
    ...
    if (pEncCfg->waveCfg.intraPeriod == 1) {
        ...
        for (i = 0; pEncCfg->waveCfg.gopPresetIdx == PRESET_IDX_CUSTOM_GOP && i < pEncCfg->waveCfg.gopParam.customGopSize; i++) {
            ...
            osal_sprintf(tempStr, "Frame%d", i+1);
            if (WAVE_GetStringValue(fp, tempStr, sValue) != 1) {
    ...
    if (pEncCfg->waveCfg.roiEnable) {
        osal_sprintf(tempStr, "RoiFile");
        if (WAVE_GetStringValue(fp, tempStr, sValue) == 1) {
            osal_sscanf(sValue, "%s\n", pEncCfg->waveCfg.roiFileName);
        }
    }
    ...
    if (pEncCfg->waveCfg.roiEnable) {
        osal_sprintf(tempStr, "RoiQpMapFile");
        if (WAVE_GetStringValue(fp, tempStr, sValue) == 1) {
    ...
    if (pEncCfg->waveCfg.scalingListEnable) {
        osal_sprintf(tempStr, "ScalingListFile");
        if (WAVE_GetStringValue(fp, tempStr, sValue) == 1) {
    ...
    if (pEncCfg->waveCfg.customLambdaEnable) {
        osal_sprintf(tempStr, "CustomLambdaFile");
        if (WAVE_GetStringValue(fp, tempStr, sValue) == 1) {
    ...
    if (pEncCfg->waveCfg.customLambdaMapEnable) {
        osal_sprintf(tempStr, "LambdaMapFile");
        if (WAVE_GetStringValue(fp, tempStr, sValue) == 1) {
    ...
    if (pEncCfg->waveCfg.customModeMapFlag) {
        osal_sprintf(tempStr, "ModeMapFile");
        if (WAVE_GetStringValue(fp, tempStr, sValue) == 1) {
    ...
    if (pEncCfg->waveCfg.weightPredEnable & 0x1) {
        osal_sprintf(tempStr, "WpParamFile");
        if (WAVE_GetStringValue(fp, tempStr, sValue) == 1) {
    ...
    )
{
    ...
    // FixedBitRatio 0 ~ 7
    for (i=0; i<MAX_GOP_NUM; i++) {
        osal_sprintf(tempStr, "FixedBitRatio%d", i);
        if (WAVE_GetStringValue(fp, tempStr, sValue) == 1) {
            iValue = osal_atoi(sValue);
            if ( iValue >= waveCfgInfo[FIXED_BIT_RATIO].min && iValue <= waveCfgInfo[FIXED_BIT_RATIO].max )
                pEncCfg->waveCfg.fixedBitRatio[i] = iValue;
