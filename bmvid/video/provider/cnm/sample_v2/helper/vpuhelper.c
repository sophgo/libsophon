...
Int32 GetEncOpenParamDefault(EncOpenParam *pEncOP, TestEncConfig *pEncConfig)
{
    int bitFormat;
    Int32   productId;
    productId                       = VPU_GetProductId(pEncConfig->coreIdx/*pEncOP->coreIdx*/);
    ...
    pEncOP->frameRateInfo           = pEncConfig->fps == 0 ? 30 : pEncConfig->fps;
    ...
    pEncOP->rcIntraQp               = pEncConfig->rcIntraQp;       // disable == -1
    ...
    pEncOP->mbInterval              = 0;
    pEncConfig->picQpY              = pEncConfig->picQpY;
    ...
    if( bitFormat == STD_MPEG4 ) {
        ...
        pEncOP->srcBitDepth     = pEncConfig->bitdepth == 0 ? 8 : pEncConfig->bitdepth;
        param->losslessEnable   = 0;
        param->constIntraPredFlag = 0;
        param->useLongTerm  = 0;

        /* for CMD_ENC_SEQ_GOP_PARAM */
        param->gopPresetIdx     = pEncConfig->gopPresetIdx;
    ...
    if ( PRODUCT_ID_W_SERIES(productId) == TRUE ) {
        ...
        case STD_AVC:
            if (parseWaveEncCfgFile(pCfg, pEncConfig->cfgFileName, bitFormat) == 0)
                return 0;
            if (pEncCfg)
                osal_strcpy(pEncConfig->yuvFileName, pCfg->SrcFileName);
            else
                osal_sprintf(pEncConfig->yuvFileName,  "%s%s", yuvDir, pCfg->SrcFileName);
            if (pEncConfig->bitstreamFileName[0] == 0 && pCfg->BitStreamFileName[0] != 0)
                osal_sprintf(pEncConfig->bitstreamFileName, "%s", pCfg->BitStreamFileName);

            if ( pEncConfig->bitstreamFileName[0] == 0 )
                osal_sprintf(pEncConfig->bitstreamFileName, "%s", "output_stream.265");

            if (pCfg->waveCfg.roiEnable) {
                osal_strcpy(pEncConfig->roi_file_name, pCfg->waveCfg.roiFileName);
                if (!osal_strcmp(pCfg->waveCfg.roiQpMapFile, "0") || pCfg->waveCfg.roiQpMapFile[0] == 0) {
                    //invalid value exist or not exist
                }
                else {
                    //valid value exist
                    osal_strcpy(pEncConfig->roi_file_name, pCfg->waveCfg.roiQpMapFile);
                }
            }
            ...
            if (pCfg->waveCfg.scalingListEnable)
                osal_strcpy(pEncConfig->scaling_list_fileName, pCfg->waveCfg.scalingListFileName);

            if (pCfg->waveCfg.customLambdaEnable)
                osal_strcpy(pEncConfig->custom_lambda_fileName, pCfg->waveCfg.customLambdaFileName);

            // custom map
            if (pCfg->waveCfg.customLambdaMapEnable)
                osal_strcpy(pEncConfig->lambda_map_fileName, pCfg->waveCfg.customLambdaMapFileName);

            if (pCfg->waveCfg.customModeMapFlag)
                osal_strcpy(pEncConfig->mode_map_fileName, pCfg->waveCfg.customModeMapFileName);

            if (pCfg->waveCfg.weightPredEnable&1)
                osal_strcpy(pEncConfig->wp_param_fileName, pCfg->waveCfg.WpParamFileName);
