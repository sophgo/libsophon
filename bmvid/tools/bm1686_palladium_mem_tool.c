...
int main(int argc, char *argv[])
{
    ...
    case 0: // load
        ...
        for (i = 0; i < tb_num; i++)
        {
            ...
            strncpy(fn, argv[4], n);
            if (interleave)
                sprintf(fn+n, "_tb%d", i);
            else
                sprintf(fn+n, "_tb%d", (int)(address_backup >> 32));
    ...
    if (opt == 0 || opt == 2) // load.tcl
    {
        ...
        for (j = 0; j < tb_num; j++)
        {
            ...
            for (i = start_bank; i < start_bank + max_bank; i++)
            {
                ...
                switch(j){
                    case 0:
                        fprintf(fin, "memory -load %%readmemh {u_bm1684_chip.u_bm1684_core.u_ddr0a_x32.u_snps_ddr4_4ports_w128_top_0.u_jedec_lpddr4_16Gb_%d.lpddr4_chA.memcore} -file %s%s_tb%d_ba%d_mem0.h  -start 0x%lx -end 0x%lx\n",\
                                id, prefix, fn, j, id, (unsigned long)(address + offset), \
                                (unsigned long)address + offset + bank_size[id] - 1);
                        fprintf(fin, "memory -load %%readmemh {u_bm1684_chip.u_bm1684_core.u_ddr0a_x32.u_snps_ddr4_4ports_w128_top_0.u_jedec_lpddr4_16Gb_%d.lpddr4_chB.memcore} -file %s%s_tb%d_ba%d_mem1.h  -start 0x%lx -end 0x%lx\n",\
                                id, prefix, fn, j, id, (unsigned long)(address + offset), \
                                (unsigned long)address + offset + bank_size[id] - 1);
                         break;
                    case 1:
                        fprintf(fin, "memory -load %%readmemh {u_bm1684_chip.u_bm1684_core.u_ddr0b_x32.u_snps_ddr4_4ports_w128_top_0.u_jedec_lpddr4_16Gb_%d.lpddr4_chA.memcore} -file %s%s_tb%d_ba%d_mem0.h  -start 0x%lx -end 0x%lx\n",\
                                id, prefix, fn, j, id, (unsigned long)(address + offset), \
                                (unsigned long)address + offset + bank_size[id] - 1);
                        fprintf(fin, "memory -load %%readmemh {u_bm1684_chip.u_bm1684_core.u_ddr0b_x32.u_snps_ddr4_4ports_w128_top_0.u_jedec_lpddr4_16Gb_%d.lpddr4_chB.memcore} -file %s%s_tb%d_ba%d_mem1.h  -start 0x%lx -end 0x%lx\n",\
                                id, prefix, fn, j, id, (unsigned long)(address + offset), \
                                (unsigned long)address + offset + bank_size[id] - 1);
                         break;
                    case 2:
                        fprintf(fin, "memory -load %%readmemh {u_bm1684_chip.u_bm1684_core.u_ddr1_x32.u_snps_ddr4_4ports_w128_top_0.u_jedec_lpddr4_16Gb_%d.lpddr4_chA.memcore} -file %s%s_tb%d_ba%d_mem0.h  -start 0x%lx -end 0x%lx\n",\
                                id, prefix, fn, j, id, (unsigned long)(address + offset), \
                                (unsigned long)address + offset + bank_size[id] - 1);
                        fprintf(fin, "memory -load %%readmemh {u_bm1684_chip.u_bm1684_core.u_ddr1_x32.u_snps_ddr4_4ports_w128_top_0.u_jedec_lpddr4_16Gb_%d.lpddr4_chB.memcore} -file %s%s_tb%d_ba%d_mem1.h  -start 0x%lx -end 0x%lx\n",\
                                id, prefix, fn, j, id, (unsigned long)(address + offset), \
                                (unsigned long)address + offset + bank_size[id] - 1);
                         break;
                    case 3:
                        fprintf(fin, "memory -load %%readmemh {u_bm1684_chip.u_bm1684_core.u_ddr2_x32.u_snps_ddr4_4ports_w128_top_0.u_jedec_lpddr4_16Gb_%d.lpddr4_chA.memcore} -file %s%s_tb%d_ba%d_mem0.h  -start 0x%lx -end 0x%lx\n",\
                                id, prefix, fn, j, id, (unsigned long)(address + offset), \
                                (unsigned long)address + offset + bank_size[id] - 1);
                        fprintf(fin, "memory -load %%readmemh {u_bm1684_chip.u_bm1684_core.u_ddr2_x32.u_snps_ddr4_4ports_w128_top_0.u_jedec_lpddr4_16Gb_%d.lpddr4_chB.memcore} -file %s%s_tb%d_ba%d_mem1.h  -start 0x%lx -end 0x%lx\n",\
                                id, prefix, fn, j, id, (unsigned long)(address + offset), \
                                (unsigned long)address + offset + bank_size[id] - 1);
                         break;
                };
