...
#define BM_MEM_ADDR_NULL (0xfffffffffULL)
...
/* bm api message struct
	* represent the api information to be sent to bmcpu
*/
typedef struct bm_api_to_bmcpu {
	// bm_handle_t handle;
	int api_id;
	char api_data[MAX_MSG_SIZE];
	u32 api_size;
	key_t sem_key;
	pthread_t tid;
} bm_api_to_bmcpu_t;
...
/**
 * @name    bm_get_chipid
 * @brief   To get the chipid of the device. 
 * @ingroup bmlib_runtime
 *
 * @param [in] handle    The device handle
...
/**
 * @name    bm_pwr_ctrl
 * @brief   virt_addr to physical address
 * @ingroup bmlib_runtime
 *
 * @param [in] handle    The device handle
 * @param [in] virt_addr
 ...
 *          Other code  Fails.
 */
DECL_EXPORT bm_status_t bm_vAddr_to_pAddr(bm_handle_t handle, void *virt_addr,unsigned long *phy_addr);
