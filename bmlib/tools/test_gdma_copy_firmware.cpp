...
bm_device_mem_t alloc_on_device(DeviceMemAllocator *allocator, size_t elem_size,
  ...
  if (init_func) {
    float *ptr = reinterpret_cast<float *>(malloc(byte_size));
    if (!ptr) {
      destroy_mem(allocator);
  ...
  allocator->mem_need_free = reinterpret_cast<bm_device_mem_t *>(realloc(
...
bm_device_mem_t alloc_on_system(DeviceMemAllocator *allocator, size_t elem_size,
  ...
  if (init_func) {
    init_func(reinterpret_cast<float *>(ptr), elem_count);
  }
...
bm_device_mem_t map_output_to_device(DeviceMemAllocator *allocator,
  ...
  if (bm_mem_get_type(*raw_mem) == BM_MEM_TYPE_SYSTEM) {
    bm_device_mem_t new_mem = alloc_on_device(allocator, elem_size, NULL);
    allocator->post_copy_map.keys = reinterpret_cast<bm_device_mem_t *>(realloc(
        allocator->post_copy_map.keys,
        (allocator->post_copy_map.size + 1) * sizeof(bm_device_mem_t)));
    allocator->post_copy_map.values = reinterpret_cast<bm_device_mem_t *>(realloc(
...
int bm_mem_write_data_to_sys_ion(bm_handle_t handle, bm_device_mem_t *dmem,
  ...
  switch (dtype) {
  case BM_DTYPE_INT: {
    int *data = reinterpret_cast<int *>(mapped_memory);
  ...
  case BM_DTYPE_FLOAT: {
    float *data =  reinterpret_cast<float *>(mapped_memory);
    for (size_t i = 0; i < print_count; i++) {
      printf("Data[%zu]: %f\n", i, data[i]);
    }
  ...
  case BM_DTYPE_UINT32: {
    uint32_t *data = reinterpret_cast<uint32_t *>(mapped_memory);
    for (size_t i = 0; i < print_count; i++) {
      printf("Data[%zu]: %u\n", i, data[i]);
    }
...
typedef struct sg_api_memcpy {
  uint64_t src_global_offset;
  uint64_t dst_global_offset;
  int input_n;
  int src_nstride;
  int dst_nstride;
  int count;
...
bm_status_t test_sg_api_1d_memcpy(bm_handle_t handle,
  ...
  // api_mem_param.data_type = 1;
  // api_mem_param.w_bytes = shape_cnt;
  size_t sg_dtype_len = sizeof(uint8_t);

  // init src data
  uint8_t *input = reinterpret_cast<uint8_t *>(malloc(shape_cnt * sg_dtype_len));
  if (!input) {
  ...
  // init dst data
  uint32_t *output = reinterpret_cast<uint32_t *>(malloc(shape_cnt * sg_dtype_len));
  if (!output) {
  ...
  bm_memcpy_s2s(bm_mem_get_device_addr(output_device_mem), bm_mem_get_device_addr(input_device_mem), shape_cnt);
  ...
  void *virtual_addr = map_physical_memory(bm_mem_get_device_addr(output_device_mem),
...
bm_status_t test_sg_api_2d_memcpy(bm_handle_t handle,
                                  DeviceMemAllocator allocator) {
  // int (*f_ptr)(void *, unsigned int);
  size_t shape_cnt = 50 * 4;
  // f_ptr = (int (*)(void *, unsigned int))dlsym(lib_handle, "sg_api_2d_memcpy");

  sg_api_2d_memcpy api_mem_param;
  api_mem_param.data_type = 1;
  api_mem_param.w_bytes = 2;
  api_mem_param.h = 20;
  size_t sg_dtype_len = sizeof(uint8_t);
  ...
  // init dst data
  uint8_t *output = reinterpret_cast<uint8_t *>(malloc(shape_cnt * sg_dtype_len));
  if (!output) {
    fprintf(stderr, "Failed to allocate memory for output\n");
    free(input);
    return BM_ERR_FAILURE;
  ...
  bm_device_mem_t output_device_mem = map_output_to_device(
      &allocator, &output_mem, shape_cnt * sg_dtype_len, 0);
  api_mem_param.src_global_offset = bm_mem_get_device_addr(input_device_mem);
  api_mem_param.dst_global_offset = bm_mem_get_device_addr(output_device_mem) + 4;
  api_mem_param.stride_bytes_src = 4;
  api_mem_param.stride_bytes_dst = 2;

  printf("=====sg_api_2d_memcpy start=====\n");
  // int ret = f_ptr(&api_mem_param, sizeof(api_mem_param));
  int ret = bm_memcpy_s2s_2d(&api_mem_param);
  if (ret != 0) {
    printf("sg_api_1d_memcpy failed!\n");
  }
...
int main(int argc, char *argv[]) {
  ...
  ret_1 = test_sg_api_1d_memcpy(handle, allocator);
  if (ret_1 != 0) {
    std::cout << "test_sg_api_1d_memcpy fail" << std::endl;
  }
  ret_1 = test_sg_api_2d_memcpy(handle, allocator);
  if (ret_1 != 0) {
