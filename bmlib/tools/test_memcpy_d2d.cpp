...
void *test_gdma_d2d(void *arg)
{
	...
	bm_device_mem_u64_t src_device_buffer;
	bm_device_mem_u64_t dst_device_buffer;
	...
	if (src_device_addr == 0x0) {
		ret = bm_malloc_device_byte_heap_mask_u64(handle, &src_device_buffer, 3, transfer_size);
		if (ret != BM_SUCCESS) {
			printf("malloc device memory size = %d failed, ret = %d\n", transfer_size, ret);
			return NULL;
		}
	} else {
		src_device_buffer = bm_mem_from_device_u64(src_device_addr, transfer_size);
	}

	if (dst_device_addr == 0x0) {
		ret = bm_malloc_device_byte_heap_mask_u64(handle, &dst_device_buffer, 3, transfer_size);
		if (ret != BM_SUCCESS) {
			printf("malloc device memory size = %d failed, ret = %d\n", transfer_size, ret);
			return NULL;
		}
	} else {
		dst_device_buffer = bm_mem_from_device_u64(dst_device_addr, transfer_size);
	}

  	printf("bm_memcpy_s2d\n");
	ret = bm_memcpy_s2d_u64(handle, src_device_buffer, sys_send_buffer);
	if (ret != BM_SUCCESS) {
		if (sys_send_buffer)
	...
	if (core_id == 2) {
		for (i = 0; i < loop_num; i++) {
			ret = bm_memcpy_d2d_u64(handle, dst_device_buffer, 0, src_device_buffer, 0, transfer_size);
		}
	} else {
	...
	if (ret != BM_SUCCESS) {
		...
		printf("Src:%llx, Dst:%llx, size:0x%x cost:%dus core_id:0x%x, loop_num:%d success\n",
				bm_mem_get_device_addr_u64(src_device_buffer),
				bm_mem_get_device_addr_u64(dst_device_buffer),
				transfer_size,
				duration,
				core_id,
				loop_num);
	...
	ret = bm_memcpy_d2s_u64(handle, sys_receive_buffer, dst_device_buffer);
