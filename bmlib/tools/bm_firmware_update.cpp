...
bool is_valid_mcu(bm_handle_t handle, Bin_buffer *bin_buf)
{
    ...
    MD5Update1(&md_ctx, image, md_size);
    ...
    if (board_type == 11){
        app_efie = (struct efie *)(image + EFIT_START_SM5G);
        if (app_efie->offset + app_efie->length > PROGRAM_LIMIT_SM5G) {
            printf("wrong efie of app\n");
            return false;
        }
    }
    else if ((board_type == 33) || (board_type == 34)) {
...
bm_status_t bm1684_check_mcu_firmware(int board_type, Bin_buffer *bin_buf)
{
	u_int32_t EFIE_START;
	u_int32_t APP_START;
	u8* efie;
	const u_int8_t EFIE_TYPE_APP = 0;
	const u_int8_t OFF_OFFSET = 0;
	const u_int8_t LENGTH_OFFSET = 4;
    /* 4 bytes offset + 4 bytes length + 16 bytes checksum + 1 byte is_checked */
	const u_int8_t TYPE_OFFSET = 4 + 4 + 16 + 1;

    /*once expecting to support new model, detailed info of firmware SHOULD be added*/
	switch (board_type){
    case 11:
	default:
		EFIE_START = 24 * 1024;
	...
	efie= (u8*)((u8*)(bin_buf->buf) + EFIE_START);

	if(*(efie + TYPE_OFFSET) != EFIE_TYPE_APP){
		printf("application efie not found\n");
        return BM_ERR_DATA;
	}

	if(*(u_int32_t*)(efie + OFF_OFFSET) != APP_START){
		printf("declared offset and real offset are incompatible\n");
		return BM_ERR_DATA;
	}
...
{
    Bin_buffer helper_buf, app_buf, chksum_buf;
    unsigned char calc_cksum[16];

    int board_type = (int)((handle->misc_info.board_version & 0x0000FFFF) >> 8);

    /*set helper_buf to EFIT*/
    /*once expecting to support new model, detailed info of firmware SHOULD be added*/
    switch (board_type)
    {
    case 33:
    case 34:
      helper_buf.buf = bin_buf->buf + EFIT_START_SC7P;
	    helper_buf.size = sizeof(struct efie);
	    helper_buf.target_addr = EFIT_START_SC7P;
        break;
    case 11:
      helper_buf.buf = bin_buf->buf + EFIT_START_SM5G;
	    helper_buf.size = sizeof(struct efie);
	    helper_buf.target_addr = EFIT_START_SM5G;
        break;
    default:
      helper_buf.buf = bin_buf->buf + EFIT_START;
	    helper_buf.size = sizeof(struct efie);
	    helper_buf.target_addr = EFIT_START;
        break;
    }

    struct efie *app_efie = (struct efie *)(helper_buf.buf);
    print_efie(app_efie);
    ...
    /*for gdmcu, set helper_buf to FWINFO and check FWINFO*/
    /*once expecting to support new model, detailed info of firmware SHOULD be added*/
    if((board_type == 11) || (0)){
        helper_buf.buf = bin_buf->buf + FWINFO_START;
        helper_buf.size = FWINFO_SIZE;
        helper_buf.target_addr = FWINFO_START;

        if(bm1684_check_mcu_firmware(board_type, bin_buf)){
            printf("invalid firmware! upgrade interrupted.\n");
            return BM_ERR_FAILURE;
        }
    }
  ...
  /*
      On ST platform, helper_buf means EFIE_buf
      On GD platform, helper_buf means FWINFO_buf
      Except for APP zone, their another burn zones are different
  */
    #ifdef __linux__
    if (0 != ioctl(handle->dev_fd, BMDEV_PROGRAM_MCU, &helper_buf)) {
    #else
    if (0 != platform_ioctl(handle, BMDEV_PROGRAM_MCU, &helper_buf)) {
    #endif
        if((board_type == 11) || (0))
            printf("program fwinfo failed!\n");
        else
    ...
    if((board_type == 11) || (0)){
        printf("program fwinfo succeeds.\n");
        return BM_SUCCESS;
    }
    else
    ...
    /*  calculate checksum
        but checksum for gdmcu now is not supported
	*/
