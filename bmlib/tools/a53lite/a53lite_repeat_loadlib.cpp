...
bm_device_mem_t map_output_to_device(DeviceMemAllocator* allocator, const bm_device_mem_t* raw_mem, size_t elem_size, int is_inplace) {
    if (bm_mem_get_type(*raw_mem) == BM_MEM_TYPE_SYSTEM) {
        bm_device_mem_t new_mem = alloc_on_device(allocator, elem_size, NULL);
        allocator->post_copy_map.keys = reinterpret_cast<bm_device_mem_t*>(realloc(allocator->post_copy_map.keys, (allocator->post_copy_map.size + 1) * sizeof(bm_device_mem_t)));
        allocator->post_copy_map.values = reinterpret_cast<bm_device_mem_t*>(realloc(allocator->post_copy_map.values, (allocator->post_copy_map.size + 1) * sizeof(bm_device_mem_t)));
