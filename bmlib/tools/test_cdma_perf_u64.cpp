...
int test_cdma_stod_transfer(int chip_num, u64 transfer_size, unsigned long long dst_addr)
{
  bm_handle_t handle = NULL;
  bm_status_t ret = BM_SUCCESS;
  unsigned char *sys_send_buffer, *sys_recieve_buffer;
  ...
  if (consume > 0) {
    float bandwidth = (float)transfer_size / (1024.0*1024.0) / (consume / 1000000.0);
    printf("S2D sys:Transfer size:0x%llx byte. Cost time:%ld us, Write Bandwidth:%.2f MB/s\n",
            transfer_size,
            consume,
            bandwidth);
  }
  ...
  if (consume > 0) {
    float bandwidth = (float)transfer_size / (1024.0*1024.0) / (consume / 1000000.0);
    printf("S2D real:Transfer size:0x%llx byte. Cost time:%ld us, Write Bandwidth:%.2f MB/s\n",
            transfer_size,
            consume,
            bandwidth);
  }
  ...
  if (consume > 0) {
    float bandwidth = (float)transfer_size / (1024.0*1024.0) / (consume / 1000000.0);
    printf("D2S sys:Transfer size:0x%llx byte. Cost time:%ld us, Write Bandwidth:%.2f MB/s\n",
            transfer_size,
            consume,
            bandwidth);
  }
  ...
  if (consume > 0) {
    float bandwidth = (float)transfer_size / (1024.0*1024.0) / (consume / 1000000.0);
    printf("D2S real:Transfer size:0x%llx byte. Cost time:%ld us, Write Bandwidth:%.2f MB/s\n",
            transfer_size,
            consume,
            bandwidth);
  }
...
// dir = 0, s2d; dir = 1, d2s
int test_cmda_perf_mutithread(int thread_num, int dir, int dev_id, u64 size, int launch_num)
{
  #ifdef __linux__
