...
static const std::map<size_t, size_t> mars3_freq_pair =
{
  {375, 500},
  {500, 500},
  {750, 650},
...
static bm_status_t __set_engine_profile_param_common(bm_handle_t handle, int core, int engine_type, unsigned long long addr, unsigned long long size) {
  ...
  bm_status_t status = tpu_kernel_launch_async_from_core(handle, set_engine_param_func_id, (u8*)&param, sizeof(param), core);
  if (BM_SUCCESS != status) {
...
static bm_status_t __set_profile_enable_common(bm_handle_t handle, unsigned enable_bits, int core_id){
  if(!handle || !handle->profile) return BM_SUCCESS;
  ...
  bm_status_t status = tpu_kernel_launch_async_from_core(handle, set_profile_func_id, (void*)&profile_enable, api_buffer_size, core_id);
  if (BM_SUCCESS != status) {
...
bm_status_t mars3_final_init(bm_handle_t handle, bmlib_profile_t* profile, int core_id) {
  ...
  return __set_profile_enable_common(handle, enable_bits, core_id);
}

bm_status_t mars3_final_deinit(bm_handle_t handle, bmlib_profile_t* profile, FILE* fp, int core_id) {
  return __set_profile_enable_common(handle, 0, core_id);
}

void show_mars3_gdma_item(const mars3_GDMA_PROFILE_FORMAT* data, int len, float freq_MHz, int core_id, float time_offset=0){
  ...
  for(int i=0; i<real_print; i++){
    ...
    auto total_bytes = p.axi_d0_w_cntr + p.gif_mem_w_cntr;
...
void show_mars3_tiu_item(const mars3_TPU_PROFILE_FORMAT* data, int len, float freq_MHz, int core_id, float time_offset=0){
  if(len==0 || !data) return;
  int max_print = get_env_int_value(ENV_SHOW_SIZE, -1);
  bool show_raw = get_env_bool_value(ENV_SHOW_RAW_DATA, false);
  if(max_print<=0) return;
...
bm_status_t mars3_mcu_init(bm_handle_t handle, bmlib_profile_t* profile, int core_id){
  return BM_SUCCESS;
}

static bm_status_t __get_profile_data_common(
    bm_handle_t handle,
...
bm_status_t gdma_init_common(bm_handle_t handle, bmlib_profile_t* profile, int core_id) {
  ...
  auto ret = bm_memcpy_s2d(handle, profile->cores[core_id].gdma_buffer.mem, profile->cores[core_id].gdma_buffer.ptr);
  if (ret != BM_SUCCESS) {
    PROFILE_ERROR("init device buffer data failed, ret = %d\n", ret);
    exit(-1);
  }
  return __set_engine_profile_param_common(handle, core_id, PROFILE_ENGINE_GDMA, buffer_start_addr, buffer_size);
}

bm_status_t mars3_gdma_init(bm_handle_t handle, bmlib_profile_t* profile, int core_id) {
  return gdma_init_common(handle, profile, core_id);
}

bm_status_t tiu_init_common(bm_handle_t handle, bmlib_profile_t* profile, int core_id) {
  ...
  auto ret = bm_memcpy_s2d(handle, profile->cores[core_id].tiu_buffer.mem, profile->cores[core_id].tiu_buffer.ptr);
  if (ret != BM_SUCCESS) {
    PROFILE_ERROR("init device buffer data failed, ret = %d\n", ret);
    exit(-1);
  }
  return __set_engine_profile_param_common(handle, core_id, PROFILE_ENGINE_TIU, buffer_start_addr, buffer_size);
}

 bm_status_t mars3_tiu_init(bm_handle_t handle, bmlib_profile_t* profile, int core_id) {
  return tiu_init_common(handle, profile, core_id);
 }

bm_status_t mars3_mcu_deinit(bm_handle_t handle, bmlib_profile_t* profile, FILE* fp, int core_id) {
  for (int i=0; i<2; i++) {
    vector<u8> data;
    size_t offset = 0;
    size_t total_len = 0;
    ...
    while (1) {
      ...
      auto status = __get_profile_data_common(
               handle, profile,
...
bm_status_t mars3_tiu_deinit(bm_handle_t handle, bmlib_profile_t* profile, FILE* fp, int core_id) {
  ...
  auto tpu_data = (mars3_TPU_PROFILE_FORMAT *)profile->cores[core_id].tiu_buffer.ptr;
  u32 valid_len = 0;
  u32 tiu_record_len = profile->cores[core_id].tiu_buffer.size/sizeof(mars3_TPU_PROFILE_FORMAT);
  while (tpu_data[valid_len].inst_start_time != 0 && tpu_data[valid_len].inst_end_time != 0 && valid_len < tiu_record_len) valid_len++;
  ...
  if (tiu_freq_MHz==0) tiu_freq_MHz = 375;
  show_mars3_tiu_item(tpu_data, valid_len, tiu_freq_MHz, core_id);

  write_block(fp, BLOCK_MONITOR_BDC, valid_len * sizeof(tpu_data[0]), tpu_data);
  return BM_SUCCESS;
}

bm_status_t mars3_gdma_deinit(bm_handle_t handle, bmlib_profile_t* profile, FILE* fp, int core_id) {
  auto ret = bm_memcpy_d2s(handle, profile->cores[core_id].gdma_buffer.ptr, profile->cores[core_id].gdma_buffer.mem);
  if (ret != BM_SUCCESS) {
    PROFILE_ERROR("Get gdma monitor profile from device to system failed, ret = %d\n", ret);
    exit(-1);
  }
  auto gdma_data = (mars3_GDMA_PROFILE_FORMAT *)profile->cores[core_id].gdma_buffer.ptr;
  u32 valid_len = 0;
  u32 record_len = profile->cores[core_id].gdma_buffer.size/sizeof(mars3_GDMA_PROFILE_FORMAT);
  while (gdma_data[valid_len].inst_start_time != 0 && gdma_data[valid_len].inst_end_time != 0 && valid_len < record_len)
    valid_len++;
  ...
  // Note: GDMA and TIU in Mars3 have different frequence.
  int gdma_freq_MHz = mars3_freq_pair.at(tiu_freq_MHz);
  show_mars3_gdma_item(gdma_data, valid_len, gdma_freq_MHz, core_id);
  ...
  {                                                    \
    ...
    {                                                  \
      value,                                           \
          ...
          arch##_final_init,                       \
          arch##_final_deinit,                     \
          (u32)sizeof(arch##_TPU_PROFILE_FORMAT),  \
          (u32)sizeof(arch##_GDMA_PROFILE_FORMAT), \
    }                                                  \
  }

std::map<int, bm_arch_profile_info_t> _global_arch_profile_map = {
  ARCH_ITEM(mars3, CHIP_ID, 5)
};
