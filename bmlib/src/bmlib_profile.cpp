...
bm_status_t bm1684_final_init(bm_handle_t handle, bmlib_profile_t* profile, int core_id) { return BM_SUCCESS; }
bm_status_t bm1684_final_deinit(bm_handle_t handle, bmlib_profile_t* profile, FILE* fp, int core_id) { return BM_SUCCESS; }
...
bm_status_t bm1684_gdma_init(bm_handle_t handle, bmlib_profile_t* profile, int core_id){
  ...
  u32 ret = bm_memcpy_s2d(handle, profile->cores[core_id].gdma_aligned_mem, profile->cores[core_id].gdma_buffer.ptr);
  if (ret != BM_SUCCESS) {
...
bm_status_t bm1684_tiu_deinit(bm_handle_t handle, bmlib_profile_t* profile, FILE* fp, int core_id) {
  ...
  while (tpu_data[valid_len].inst_start_time != 0xFFFFFFFF && tpu_data[valid_len].inst_end_time != 0xFFFFFFFF && valid_len < tiu_record_len) valid_len++;
...
bm_status_t bm1684_gdma_deinit(bm_handle_t handle, bmlib_profile_t* profile, FILE* fp, int core_id) {
  ...
  auto ret = bm_memcpy_d2s(handle, profile->cores[core_id].gdma_buffer.ptr, profile->cores[core_id].gdma_aligned_mem);
  if (ret != BM_SUCCESS) {
    PROFILE_ERROR("Get monitor profile from device to system failed, ret = %d\n", ret);
    exit(-1);
  }
  u32 valid_len = fix_bm1684_gdma_monitor_data(profile->cores[core_id].gdma_buffer.ptr, bm_mem_get_device_size(profile->cores[core_id].gdma_aligned_mem));
  ...
  show_bm1684_gdma_item((BM1684_GDMA_PROFILE_FORMAT*)profile->cores[core_id].gdma_buffer.ptr, gdma_num, BM1684_GDMA_FREQ_MHZ);
...
////////////////////////////////////////////////////////
// for BM1686
bm_status_t bm1686_final_init(bm_handle_t handle, bmlib_profile_t* profile, int core_id){ return BM_SUCCESS; }
bm_status_t bm1686_final_deinit(bm_handle_t handle, bmlib_profile_t* profile, FILE* fp, int core_id) { return BM_SUCCESS; }

static bm_status_t __bm1686_set_profile_enable(bm_handle_t handle, bool enable, int core_id){
  if(!(handle && handle->profile && handle->profile->cores[core_id].enable_profile_func_id)) return BM_SUCCESS;
  u32 api_buffer_size = sizeof(u32);
  u32 profile_enable = enable;
  bm_status_t status = tpu_kernel_launch(handle, handle->profile->cores[core_id].enable_profile_func_id, &profile_enable, api_buffer_size);
  if (BM_SUCCESS != status) {
...
bm_status_t bm1686_gdma_deinit(bm_handle_t handle, bmlib_profile_t* profile, FILE* fp, int core_id) {
  ...
  while (gdma_data[valid_len].valid_data.inst_start_time != 0xFFFFFFFF && gdma_data[valid_len].valid_data.inst_end_time != 0xFFFFFFFF && valid_len < record_len)
    valid_len++;
...
static bm_status_t __bm1688_set_engine_profile_param(bm_handle_t handle, int core, int engine_type, unsigned long long addr, unsigned long long size){
...
void show_bm1688_tiu_item(const BM1688_TPU_PROFILE_FORMAT* data, int len, float freq_MHz, int core_id, float time_offset=0){
  if(len==0 || !data) return;
  int max_print = get_env_int_value(ENV_SHOW_SIZE, -1);
  bool show_raw = get_env_bool_value(ENV_SHOW_RAW_DATA, false);
  if(max_print<=0) return;
...
bm_status_t bm1688_gdma_init(bm_handle_t handle, bmlib_profile_t* profile, int core_id){
  ...
  return __bm1688_set_engine_profile_param(handle, core_id, PROFILE_ENGINE_GDMA, buffer_start_addr, buffer_size);
}
...
bm_status_t bm1688_tiu_init(bm_handle_t handle, bmlib_profile_t* profile, int core_id){
  ...
  return __bm1688_set_engine_profile_param(handle, core_id, PROFILE_ENGINE_TIU, buffer_start_addr, buffer_size);
}
