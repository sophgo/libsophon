...
static int bm_alloc_gmem(bm_handle_t ctx, bm_device_mem_t *pmem, int heap_id_mask) {
  ...
  #endif
  {
    ...
    if (0 != ret){
      bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_ERROR,
            "bm alloc gmem failed, ioclt ret = %d:%d\n", ret, __LINE__);
      return BM_ERR_FAILURE;
    }
...
bm_status_t bm_memcpy_s2d(bm_handle_t handle, bm_device_mem_t dst, void *src) {
  ...
  if(dst.u.device.dmabuf_fd==0){
    ...
    if(ret!=BM_SUCCESS){
      printf( "bm_malloc_device_byte failed! %d,size=%d\n",ret,bm_mem_get_size(dst));
      return BM_ERR_FAILURE;
    }
  }
  ret=bm_mem_write_data_to_ion(handle, &dst, src, bm_mem_get_size(dst));
  if (ret != BM_SUCCESS) {
    printf( "bm_mem_write_data_to_ion failed! %d\n",ret);
    return BM_ERR_FAILURE;
  } else{
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_DEBUG, "bm_mem_write_data_to_ion success! addr = %x\n",dst.u.device.device_addr);
  }
  return BM_SUCCESS;
