...
#ifdef __linux__
static struct rb_root g_mmap_root = RB_ROOT;
static pthread_mutex_t g_mmap_mutex = PTHREAD_MUTEX_INITIALIZER;
...
{
	struct rb_node *node = g_mmap_root.rb_node;
	long long result;
	...
	while (node) {
		...
		if (result < 0)
			...
			rb_erase(&data->node, &g_mmap_root);
			free(data);
			return;
		}
	}
...
static struct bm_mmap_record *mmap_record_search_locked(u64 vmem)
{
	struct rb_node *node = g_mmap_root.rb_node;
...
static bm_status_t mmap_record_track(u64 vmem_base, u64 vmem_size,
		bm_device_mem_u64_t *dmem)
{
	...
	pthread_mutex_lock(&g_mmap_mutex);
	ret = mmap_record_add_locked(record);
	pthread_mutex_unlock(&g_mmap_mutex);
	if (ret != BM_SUCCESS)
		free(record);
	return ret;
}

static bm_status_t mmap_record_track_device_mem(u64 vmem_base, u64 vmem_size,
		bm_device_mem_t *dmem)
{
	...
	return mmap_record_track(vmem_base, vmem_size, &dmem_u64);
}

static void mmap_record_untrack(u64 vmem_base)
{
	pthread_mutex_lock(&g_mmap_mutex);
	mmap_record_remove_locked(vmem_base);
	pthread_mutex_unlock(&g_mmap_mutex);
}
#endif
/*
                                   ...
                                   u64 *vmem)
{
	...
	if (MAP_FAILED != ret) {
		*vmem = (u64)ret;
		if (mmap_record_track_device_mem(*vmem, aligned_size, dmem) != BM_SUCCESS) {
                                   ...
                                   u64 *vmem)
{
	...
	if (MAP_FAILED != ret) {
		*vmem = (u64)ret;
		if (mmap_record_track(*vmem, aligned_size, dmem) != BM_SUCCESS) {
			(void)munmap(ret, aligned_size);
			return BM_ERR_FAILURE;
		}
		return BM_SUCCESS;
...
#ifdef __linux__
	pthread_mutex_lock(&g_mmap_mutex);
	record = mmap_record_search_locked(*vmem);
	if (!record) {
		pthread_mutex_unlock(&g_mmap_mutex);
		bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_ERROR,
