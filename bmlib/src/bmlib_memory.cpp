  ...
  if (bm_is_dynamic_loading(handle)) {
    ...
    for (unsigned int core_id=0; core_id<core_num; core_id++) {
      ...
      for (int i=0; i < sizeof(g_basic_func) / sizeof(g_basic_func[0]); i++) {
          g_basic_func[i].f_ids[core_id] = tpu_kernel_get_function_from_core(handle, bm_module, g_basic_func[i].func_name, core_id);
      }
      free(bm_module);
    }
  }
...
bm_status_t bm_get_carveout_heap_id(bm_handle_t ctx) {
  ...
  if (ret != 0) {
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_ERROR,
          "io query heap failed, ioclt ret = %d:%d\n", ret, __LINE__);
    return BM_ERR_FAILURE;
  }

  ctx->heap_cnt = 0;
  for (unsigned int i = 0; i < query.cnt; i++) {
    bmlib_log("bmlib_merory", BMLIB_LOG_INFO,
              "heap_data[%d].type: %d. %s\n", i,heap_data[i].type, heap_data[i].name);
...
bm_status_t bm_total_gmem(bm_handle_t ctx, u64 *total) {
  ...
  memset(&bm_heap_info, 0, sizeof(bm_heap_info));
  u64 total_size = 0;
  if (ctx->ion_fd) {
    ...
    // // }
    const char* filePath = "/sys/kernel/debug/ion/cvi_carveout_heap_dump/total_mem";
    FILE* ion_file = fopen(filePath, "r");
    if (!ion_file) {
        perror("can't open file\n");
        return BM_ERR_FAILURE;
    }
    char line[128];
    long alloc_mem_bytes = 0;
    if (fgets(line, sizeof(line), ion_file) != NULL) {
        alloc_mem_bytes = atol(line);
    }
    fclose(ion_file);
    *total = alloc_mem_bytes;
  } else {
    int ret;
...
bm_status_t bm_avail_gmem(bm_handle_t ctx, u64 *avail) {
  ...
  if (ctx->ion_fd) {
    ...
    // }
    const char* filePath = "/sys/kernel/debug/ion/cvi_carveout_heap_dump/alloc_mem";
    FILE* ion_file = fopen(filePath, "r");
    if (!ion_file) {
        perror("can't open file\n");
        return BM_ERR_FAILURE;
    ...
    fclose(ion_file);
    *avail = alloc_mem_bytes;
  } else {
  int ret;
  ret = platform_ioctl(ctx, BMDEV_AVAIL_GMEM, avail);
    if (0 != ret){
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_ERROR,
          "get avail gmem failed, ioclt ret = %d:%d\n", ret, __LINE__);
    return BM_ERR_FAILURE;
  }
  }
  return BM_SUCCESS;
...
static int bm_alloc_gmem(bm_handle_t ctx, bm_device_mem_t *pmem, int heap_id_mask) {
  ...
  if (ctx->ion_fd) {
    ...
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_ERROR, "%s:%d no ion_fd\n", __func__, __LINE__);
  }
  bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_DEBUG,
    "alloc_data.paddr:%llx, alloc_data.fd:%u, alloc_size:%llu, alloc_data.heap_id_mask:%u\n",
  ...
  pmem->u.device.mapped_memory = mmap(NULL, pmem->size, PROT_READ | PROT_WRITE, MAP_SHARED, pmem->u.device.dmabuf_fd, 0);
  bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_DEBUG,
  ...
  auto result = ht_addr_fd.insert(std::make_pair(pmem->u.device.dmabuf_fd, std::make_pair(pmem->u.device.device_addr, pmem->u.device.device_addr + pmem->size)));
  if (!result.second) {
    // Key already exists, update the value
    result.first->second = std::make_pair(pmem->u.device.device_addr, pmem->u.device.device_addr + pmem->size);
  }
  auto result1 = ht_addr_mapped_addr.insert(std::make_pair((uintptr_t)(pmem->u.device.mapped_memory), std::make_pair(pmem->u.device.device_addr, pmem->u.device.device_addr + pmem->size)));
  if (!result1.second) {
    // Key already exists, update the value
    result1.first->second = std::make_pair(pmem->u.device.device_addr, pmem->u.device.device_addr + pmem->size);
  }
  lock.unlock();
...
static int bm_alloc_gmem_u64(bm_handle_t ctx, bm_device_mem_u64_t *pmem, int heap_id_mask) {
  ...
  if (ctx->ion_fd) {
      if (((heap_id_mask >> 0) & 0x1) == 0x1) {
          pmem->flags.u.gmem_heapid = ctx->carveout_heap_id[0];
          alloc_data.heap_id_mask = (1 << ctx->carveout_heap_id[0]);
          ret = ioctl(ctx->ion_fd, ION_IOC_ALLOC, &alloc_data);
  ...
  if (fcntl(alloc_data.fd, F_GETFD) == -1) {
    if (errno == EBADF) {
        printf("alloc_data.fd is not valid\n");
    } else {
        perror("alloc_data.fd fcntl error");
    }
  }
...
void bm_set_device_mem(bm_device_mem_t *pmem, unsigned int size, u64 addr = 0) {
  ...
  std::unique_lock<std::mutex> lock(ht_rw_mutex);
  for (const auto& pair : ht_addr_fd) {
    const auto &key = pair.first;
    const auto &range = pair.second;
  ...
  for (const auto &pair : ht_addr_mapped_addr){
    ...
    const auto &range = pair.second;
    if (pmem->u.device.device_addr >= range.first && pmem->u.device.device_addr < range.second) {
        pmem->u.device.mapped_memory = (void *)key;
        break;
    }
  }
  lock.unlock();
...
bm_status_t bm_mem_mmap_device_mem(bm_handle_t handle, bm_device_mem_t *dmem,
  ...
  for (const auto &pair : ht_addr_mapped_addr) {
    const auto &key = pair.first;
    const auto &range = pair.second;
    if (dmem->u.device.device_addr >= range.first && dmem->u.device.device_addr < range.second) {
        dmem->u.device.mapped_memory = (void *)key;
        inner_offset = dmem->u.device.device_addr - range.first;
        break;
    }
  }
  lock.unlock();
...
bm_status_t bm_mem_mmap_device_mem_no_cache(bm_handle_t handle,
  ...
  ret	= mmap(NULL, aligned_size, PROT_READ | PROT_WRITE, MAP_SHARED,
              dmem->u.device.dmabuf_fd, bm_mem_get_device_addr(*dmem) | 0x1000000000);
...
bm_status_t bm_mem_mmap_device_mem_no_cache_u64(bm_handle_t handle,
  ...
  ret	= mmap(NULL, aligned_size, PROT_READ | PROT_WRITE, MAP_SHARED,
              dmem->u.device.dmabuf_fd, bm_mem_get_device_addr_u64(*dmem) | 0x1000000000);
...
bm_status_t bm_mem_invalidate_partial_device_mem_u64(bm_handle_t handle,
  ...
  dmem_32.u.device.mapped_memory = (void*)bm_mem_get_device_addr_u64(*dmem);
  bm_mem_invalidate_partial_device_mem(handle, &dmem_32, offset, len);
...
bm_status_t bm_mem_flush_partial_device_mem(bm_handle_t handle,
  ...
  bm_profile_record_mem_end(handle, bm_mem_op_type_t::FLUSH, dmem->u.device.device_addr+offset, len);
#else
  UNUSED(handle);
  UNUSED(dmem);
  UNUSED(offset);
...
bm_status_t bm_mem_flush_partial_device_mem_u64(bm_handle_t handle,
  ...
  dmem_32.size = (u32)bm_mem_get_device_size_u64(*dmem);
  dmem_32.u.device.mapped_memory = (void*)bm_mem_get_device_addr_u64(*dmem);
  bm_mem_flush_partial_device_mem(handle, &dmem_32, offset, len);
...
bm_status_t bm_memcpy_s2d(bm_handle_t handle, bm_device_mem_t dst, void *src) {
  ...
  int ret = 0;
  ret = bm_mem_write_data_to_ion(handle, &dst, src, bm_mem_get_size(dst));
  if (ret != BM_SUCCESS) {
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_ERROR,
      "bm_mem_write_data_to_ion failed! %d\n", ret);
    return BM_ERR_FAILURE;
  } else {
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_DEBUG,
      "bm_mem_write_data_to_ion success! addr = %x\n",
      dst.u.device.device_addr);
  }
...
bm_status_t bm_memcpy_s2d_partial_offset(bm_handle_t handle,
  ...
  ret = bm_mem_write_data_to_ion(handle, &dst, src, size, offset);
  if (ret != BM_SUCCESS) {
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_ERROR,
      "bm_mem_write_data_to_ion failed! %d\n", ret);
    return BM_ERR_FAILURE;
...
bm_status_t bm_memcpy_s2d_partial_offset_u64(bm_handle_t handle,
  ...
  ret = bm_mem_write_data_to_ion(handle, &target_dev_mem, src, size, offset);
  if (ret != BM_SUCCESS) {
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_ERROR,
      "bm_mem_write_data_to_ion failed! %d\n", ret);
    return BM_ERR_FAILURE;
...
bm_status_t bm_memcpy_d2s_partial_offset(bm_handle_t handle, void *dst,
  ...
  #endif
  bm_status_t read_status = bm_mem_read_data_from_ion(handle, &src, dst, size, offset);
  if (read_status != BM_SUCCESS) {
    fprintf(stderr, "Failed to read data from device memory, error code: %d\n", read_status);
    return read_status;
  } else {
...
bm_status_t bm_memcpy_d2s_partial_offset_u64(bm_handle_t handle, void *dst,
  ...
  bm_device_mem_t target_dev_mem =
      bm_mem_from_device(dev_mem_addr + offset, size);
  target_dev_mem.u.device.dmabuf_fd = src.u.device.dmabuf_fd;
  bm_status_t read_status = bm_mem_read_data_from_ion(handle, &target_dev_mem, dst, size, offset);
  if (read_status != BM_SUCCESS) {
    fprintf(stderr, "Failed to read data from device memory, error code: %d\n", read_status);
    return read_status;
  }
...
bm_status_t bm_memset_device_ext_to_core(bm_handle_t handle, void* value, int mode,
  ...
  ret = bm_mem_write_data_to_ion(handle, &mem, value, bm_mem_get_size(mem));
  return ret;
}
...
bm_status_t bm_memcpy_d2d_with_core(bm_handle_t handle, bm_device_mem_t dst,
      ...
      api_launch_func_t send_api;
      send_api.f_id = BM_API_ID_MEM_CPY;

      bm_api_memcpy_t api = {bm_mem_get_device_addr(src) + src_offset*4,
                              bm_mem_get_device_addr(dst) + dst_offset*4,
                              1,
                              src_nstride,
                              dst_nstride,
                              len};
      memcpy(send_api.param, &api, sizeof(api));
      send_api.size = sizeof(api);

      ret = bm_send_api_to_core(handle, BM_API_ID_A53LITE_LAUNCH_FUNC, (u8 *)(&send_api), sizeof(send_api), 0);
...
bm_status_t bm_memcpy_d2d_stride_with_core(bm_handle_t     handle,
  ...
  api_launch_func_t send_api;
  send_api.f_id =   BM_API_ID_MEMCPY_WSTRIDE;
...
bm_status_t bm_memcpy_d2d_byte_with_core(bm_handle_t handle, bm_device_mem_t dst,
                                ...
                                size_t src_offset, size_t len, int core_id) {
  bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_DEBUG, "enter %s\n", __func__);
  int max_n = 1;
  int src_nstride = bm_mem_get_device_size(src) / max_n / FLOAT_SIZE;
  int dst_nstride = bm_mem_get_device_size(dst) / max_n / FLOAT_SIZE;
  ...
  api_launch_func_t send_api;
  send_api.f_id =   BM_API_ID_MEMCPY_BYTE;
...
bm_status_t bm_mem_write_data_to_ion(
    ...
    size_t           size,
    uint64_t         offset) {
    int device_addr = dmem->u.device.device_addr;
    std::unique_lock<std::mutex> lock(ht_rw_mutex);
    uint64_t inner_offset = 0;
    for (const auto &pair : ht_addr_mapped_addr) {
      const auto &key = pair.first;
      ...
      if (dmem->u.device.device_addr >= range.first && dmem->u.device.device_addr < range.second) {
          dmem->u.device.mapped_memory = (void *)key;
          inner_offset = dmem->u.device.device_addr - range.first;
          device_addr = range.first;
          break;
      }
    }
    lock.unlock();
    // write data to ion buffer
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_DEBUG, "dmem->u.device.mapped_memory=%p, device_addr=%x, inner_offset=%d, size=%d\n", dmem->u.device.mapped_memory, device_addr, (int)inner_offset, (int)size);
    uint8_t* dst = (uint8_t*)(dmem->u.device.mapped_memory) + offset + inner_offset;

    if (memcpy(dst, data, size) == NULL) {
        perror("memcpy failed");
        return BM_ERR_PARAM;
    }
    bm_device_mem_t tmp_dmem;
    tmp_dmem.u.device.device_addr = device_addr;
    tmp_dmem.u.device.mapped_memory = dmem->u.device.mapped_memory;
    tmp_dmem.size = size;
    bm_mem_flush_partial_device_mem(handle, &tmp_dmem, 0, size);
...
bm_status_t bm_mem_read_data_from_ion(
    ...
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_DEBUG, "dmem->u.device.mapped_memory=%p, size = %d, inner_offset=%d\n", dmem->u.device.mapped_memory, size, inner_offset);
    uint8_t* dst = (uint8_t*)buffer;
    uint8_t* src = (uint8_t*)(dmem->u.device.mapped_memory) + offset + inner_offset;
    bm_device_mem_t tmp_dmem;
    tmp_dmem.u.device.device_addr = device_addr;
    tmp_dmem.u.device.mapped_memory = dmem->u.device.mapped_memory;
    tmp_dmem.size = size;
    bm_mem_invalidate_partial_device_mem(handle, &tmp_dmem, 0, size);
...
void* bm_get_ion_mem_vaddr(u64 device_addr)
{
    ...
    for (const auto &pair : ht_addr_mapped_addr) {
      const auto &key = pair.first;
      const auto &range = pair.second;
      if (device_addr >= range.first && device_addr < range.second) {
          map_memory_begin = (void *)key;
          inner_offset = device_addr - range.first;
      }
    }
    lock.unlock();

    if (map_memory_begin != NULL){
      bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_DEBUG, "device_addr = %lu, map_memory_begin = %p, inner_offset=%d\n",
        device_addr, map_memory_begin, inner_offset);
      return (void *)(map_memory_begin + inner_offset);
    } else {
