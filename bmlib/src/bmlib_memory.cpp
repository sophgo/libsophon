...
static std::unordered_map<__u32, std::pair<__u64, __u32>> ht_addr_fd = {};
...
bm_status_t bm_get_carveout_heap_id(bm_handle_t ctx) {
  ...
  for (unsigned int i = 0; i < query.cnt; i++) {
    // printf("i=%d,heap_data[i].type=%d,heap_data[i].name=%s,heap_data[i].heap_id=%d\n", i,heap_data[i].type, heap_data[i].name, heap_data[i].heap_id);
...
static bm_status_t bm_free_gmem(bm_handle_t ctx, bm_device_mem_t *pmem) {
  ...
  // ret=ioctl(ctx->ion_fd, ION_IOC_FREE, &handle_data);
  if (ret != 0) {
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_ERROR,
...
bm_status_t sg_mem_mmap_device_mem(bm_handle_t handle, sg_device_mem_t *dmem,
  ...
  if (MAP_FAILED != ret) {
    *vmem = (u64)ret;
    ...
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_ERROR,
          "sg_mem_get_device_addr mmap failed, ioclt ret = %d:%d\n", ret, __LINE__);
    return BM_ERR_FAILURE;
  }
#else
    #define GLOBAL_MEM_START_ADDR 0x100000000
...
bm_status_t bm_mem_mmap_device_mem_u64(bm_handle_t handle, bm_device_mem_u64_t *dmem,
  ...
  if (MAP_FAILED != ret) {
    *vmem = (u64)ret;
    ...
    bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_ERROR,
        "bm_mem_mmap_device_mem_u64 mmap failed, errno = %d (%s), size=%d, device_addr=%x, fd=%d\n", 
        errno, strerror(errno), aligned_size, 
        bm_mem_get_device_addr_u64(*dmem), 
...
bm_status_t sg_mem_mmap_device_mem_no_cache(bm_handle_t handle,
  ...
  ret	= mmap(NULL, aligned_size, PROT_READ | PROT_WRITE, MAP_SHARED,
              dmem->u.device.dmabuf_fd, sg_mem_get_device_addr(*dmem) | 0x1000000000);
...
bm_status_t bm_memcpy_s2d(bm_handle_t handle, bm_device_mem_t dst, void *src) {
  ...
  int ret = 0;
  if (dst.u.device.dmabuf_fd == 0) {
    ret = bm_malloc_device_byte(handle, &dst, bm_mem_get_size(dst));
    if (ret != BM_SUCCESS) {
      bmlib_log(BMLIB_MEMORY_LOG_TAG, BMLIB_LOG_ERROR,
        "bm_malloc_device_byte failed! %d,size=%d\n", ret, bm_mem_get_size(dst));
      return BM_ERR_FAILURE;
    }
  }
...
bm_status_t bm_memcpy_s2d_partial_offset(bm_handle_t handle,
  ...
  target_dev_mem.u.device.dmabuf_fd = dst.u.device.dmabuf_fd;
