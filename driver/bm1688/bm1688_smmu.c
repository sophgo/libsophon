...
/* 1682 not support soc mode smmu, 1684 has the same tlb table address for soc
 * and pcie mode
 */
 ...
 */
int bm1688_disable_iommu(struct iommu_ctrl *ctrl);
...
static int iommu_validate_entries_to_device(struct iommu_ctrl *ctrl, int start, int number)
{
	...
	bmdev_construct_cdma_arg(&cdma_arg, ctrl->host_entry_base_paddr + start * sizeof(struct bm1688_iommu_entry),
			bmdi->gmem_info.resmem_info.smmu_addr + start * sizeof(struct bm1688_iommu_entry),
			number * sizeof(struct bm1688_iommu_entry), HOST2CHIP, false, false);

	if (bm1688_cdma_transfer(bmdi, NULL, &cdma_arg, false)) {
		dma_free_coherent(bmdi->cinfo.device, number * sizeof(struct bm1688_iommu_entry),
...
static int iommu_invalidate_entries_to_device(struct iommu_ctrl *ctrl, int start, int number)
{
	struct bm_memcpy_info *memcpy_info = container_of(ctrl, struct bm_memcpy_info, iommuctl);
	struct bm_device_info *bmdi = container_of(memcpy_info, struct bm_device_info, memcpy_info);
	bm_cdma_arg cdma_arg;
	memset(ctrl->host_entry_base_vaddr + start * sizeof(struct bm1688_iommu_entry), 0, number * sizeof(struct bm1688_iommu_entry));
...
static int bm_setup_iommu_pages(struct iommu_ctrl *ctrl, struct bm_buffer_object *bo)
{
	...
	struct bm1688_iommu_entry *last_entry = NULL;
	...
	for_each_sg(bo->sgt.sgl, sg, bo->sgt.nents, i) {
		...
		for (j = 0; j < (end_aligned - start_aligned) / PAGE_SIZE; j++) {
			struct bm1688_iommu_entry *entry = ctrl->host_entry_base_vaddr + sizeof(struct bm1688_iommu_entry) * (bo->iommu.entry_start + entry_idx);
			dma_addr_t paddr = (start_aligned + j * PAGE_SIZE);
#ifndef SOC_MODE
			entry->paddr = paddr >> PAGE_SHIFT;
#else
			entry->paddr = ((paddr >> PAGE_SHIFT) | (0x3f << (36 - PAGE_SHIFT)));
#endif
			entry->valid = 1;
			last_entry = entry;
...
{
	struct bm1688_iommu_entry *entry = NULL;
	dma_addr_t paddr;
	int i = 0;

	for (i = 0; i < iommu->real_num; i++) {
		entry = ctrl->host_entry_base_vaddr + sizeof(struct bm1688_iommu_entry) * (iommu->entry_start + i);
...
static int bm_free_iommu_pages(struct  iommu_ctrl *ctrl, struct bm_buffer_object *bo)
{
	...
	for (i = 0; i < bo->iommu.real_num; i++) {
		struct bm1688_iommu_entry *entry = ctrl->host_entry_base_vaddr + sizeof(struct bm1688_iommu_entry) * (bo->iommu.entry_start + i);
...
static int bm_free_raw_iommu_pages(struct iommu_ctrl *ctrl, struct iommu_region *iommu)
{
	int ret = 0;
	int i;
	struct bm1688_iommu_entry *entry;
	for (i = 0; i < iommu->real_num; i++) {
		entry = ctrl->host_entry_base_vaddr + sizeof(struct bm1688_iommu_entry) * (iommu->entry_start + i);
		entry->valid = 0;
	}
...
/* Default supported for 1684, better to read capability from reigster */
...
int bm1688_init_iommu(struct iommu_ctrl *ctrl, struct device *dev)
{
	...
	ctrl->host_entry_base_vaddr = dma_alloc_coherent(bmdi->cinfo.device, entry_num * sizeof(struct bm1688_iommu_entry),
	...
	memset(ctrl->host_entry_base_vaddr, 0, entry_num * sizeof(struct bm1688_iommu_entry));
...
int bm1688_deinit_iommu(struct iommu_ctrl *ctrl)
{
	...
	if (ctrl->enable) {
		bm1688_disable_iommu(ctrl);
		iommu_invalidate_entries_to_device(ctrl, 0, ctrl->entry_num);
	}

	dma_free_coherent(bmdi->cinfo.device, SMMU_PAGE_ENTRY_NUM * sizeof(struct bm1688_iommu_entry),
			ctrl->host_entry_base_vaddr, ctrl->host_entry_base_paddr);
	ctrl->free_ptr = ctrl->alloc_ptr = ctrl->full = 0;
...
static int bmdev_enable_iommu(struct iommu_ctrl *ctrl)
{
	...
	PR_TRACE("Enable SMMU.\n");
	/*
	 * Workaround for bm1688, read the register once again after writing,
 ...
 */
int bm1688_iommu_irq(struct iommu_ctrl *ctrl)
{
	struct bm_memcpy_info *memcpy_info = container_of(ctrl, struct bm_memcpy_info, iommuctl);
	struct bm_device_info *bmdi = container_of(memcpy_info, struct bm_device_info, memcpy_info);
	u64 status = 0;
	...
	status = smmu_reg_read(bmdi, SMMU_FSR);
	if (status) {
		if (status & SMMU_FSR_WRITE_FAULT)
			dev_err(ctrl->device, "SMMU write fault: 0x%llx, addr 0x%llx\n",
					status, bm_read64(bmdi, bmdi->cinfo.bm_reg->smmu_base_addr + SMMU_FARW));
		if (status & SMMU_FSR_READ_FAULT)
			dev_err(ctrl->device, "SMMU read fault: 0x%llx, addr 0x%llx\n",
					status, bm_read64(bmdi, bmdi->cinfo.bm_reg->smmu_base_addr + SMMU_FARR));
		smmu_reg_write(bmdi, SMMU_ICR, 1);
		return -1;
	}
...
int bm1688_enable_smmu_transfer(struct bm_memcpy_info *memcpy_info, struct iommu_region *iommu_rgn_src, struct iommu_region *iommu_rgn_dst, struct bm_buffer_object **bo_buffer)
{
	...
	ret = bm_demand_iommu_entries(&memcpy_info->iommuctl, iommu_rgn_src, iommu_rgn_dst);
	if (ret != 0) {
		// how to recover???
...
int bm1688_disable_smmu_transfer(struct bm_memcpy_info *memcpy_info, struct iommu_region *iommu_rgn_src, struct iommu_region *iommu_rgn_dst, struct bm_buffer_object **bo_buffer)
{
	...
	bm1688_disable_iommu(&memcpy_info->iommuctl);
...
{
	PR_TRACE("bm1688 smmu reset\n");
	reset_control_assert(bmdi->cinfo.smmu);
	udelay(1000);
	reset_control_deassert(bmdi->cinfo.smmu);
}

void bm1688_smmu_clk_enable(struct bm_device_info *bmdi)
{
	PR_TRACE("bm1688 smmu clk is enable\n");
	clk_prepare_enable(bmdi->cinfo.smmu_clk);
}

void bm1688_smmu_clk_disable(struct bm_device_info *bmdi)
{
	PR_TRACE("bm1688 smmu clk is gating\n");
	clk_disable_unprepare(bmdi->cinfo.smmu_clk);
}

void bm1688_pcie_clk_enable(struct bm_device_info *bmdi)
{
	PR_TRACE("bm1688 pcie clk is enable\n");
	clk_prepare_enable(bmdi->cinfo.pcie_clk);
}

void bm1688_pcie_clk_disable(struct bm_device_info *bmdi)
{
	PR_TRACE("bm1688 pcie clk is gating\n");
	clk_disable_unprepare(bmdi->cinfo.pcie_clk);
}
#endif
