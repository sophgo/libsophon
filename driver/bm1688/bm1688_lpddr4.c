...
{
	bm1688_ddr_phy_reg_write16(bmdi, base + PHY_OFFSET + (offset << 1), value);
}

static u16 dwc_phy_apb_read16(struct bm_device_info *bmdi, u32 base, u32 offset)
{
	return bm1688_ddr_phy_reg_read16(bmdi, base + PHY_OFFSET + (offset << 1));
}
...
void bm1688_mask_train_result(struct bm_device_info *bmdi, u32 index, u32 phase, u32 result)
{
		...
		u32 train_2d, u32 result)
{
	...
	bm1688_mask_train_result(bmdi, index, train_2d, pass_or_fail);
}

void bm1688_dwc_read_msgblock_msg(struct bm_device_info *bmdi, u32 cfg_base, u32 train_2d)
{
	int mail;
...
static inline void dwc_exec_fw(struct bm_device_info *bmdi, int cfg_base, u32 train_2d)
{
	...
	bm1688_dwc_read_msgblock_msg(bmdi, cfg_base, train_2d);
...
static int bm1688_ddr_phy_init(struct bm_device_info *bmdi, u32 cfg_base, u32 ddr_index)
{
	u32 i;
	u32 read_data;
...
void bm1688_dwc_phy_enter_mission(struct bm_device_info *bmdi)
{
...
static void dwc_pre_setting(struct bm_device_info *bmdi, u32 rank, u32 freq)
{
	...
	for (i = 0; i < ARRAY_SIZE(bm1688_pre_cfg_rank); i++) {
		if (rank == bm1688_pre_cfg_rank[i].rank_num)
			cfg_rank = &bm1688_pre_cfg_rank[i];
	}

	for (i = 0; i < ARRAY_SIZE(bm1688_pre_cfg_freq); i++) {
		if (freq == bm1688_pre_cfg_freq[i].frequency)
			cfg_freq = &bm1688_pre_cfg_freq[i];
	}
	...
	if (bmdi->boot_info.ddr_ecc_enable == 0x1) {
		...
		if (bmdi->cinfo.chip_id == 0x1686) {
			cfg_ecc = bm1688x_cfg_ecc_off;
			ecc_array_size = ARRAY_SIZE(bm1688x_cfg_ecc_off);
		} else {
			cfg_ecc = cfg_ecc_off;
...
static void ddr_group_init(struct bm_device_info *bmdi, u32 group)
{
	if (group == GROUP0) {
		//phy init
		bm1688_ddr_phy_init(bmdi, DDR_CTRL0_A, 0);
		bm1688_ddr_phy_init(bmdi, DDR_CTRL0_B, 1);
		//1d train
		...
		bm1688_ddr_phy_init(bmdi, DDR_CTRL1, 2);
		bm1688_ddr_phy_init(bmdi, DDR_CTRL2, 3);
...
static void bm1688_get_freq_str(int ddr_freq, char *s_ddr_freq)
{
	char *s_freq = NULL;
...
int bm1688_lpddr4_init(struct bm_device_info *bmdi)
{
	...
	if (chip_mode == NORMAL_MODE) {
		...
		if ((BM1688_BOARD_TYPE(bmdi) == BOARD_TYPE_SM5M_P) && (BM1688_HW_VERSION(bmdi) == 0x30)) {
			freq = FREQ_3600M;
		} else {
			freq = FREQ_4000M;
		}
	...
	bm1688_disable_ddr_interleave(bmdi);
...
start:
	...
	bm1688_dwc_phy_enter_mission(bmdi);

	bm1688_get_freq_str(freq, s_ddr_freq);
