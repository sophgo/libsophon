...
#define DDR1_IRQ_ID 58
#define DDR2_IRQ_ID 59
...
void bm1688_ddr1_ecc_irq_handler(struct bm_device_info *bmdi)
{
...
void bm1688_ddr2_ecc_irq_handler(struct bm_device_info *bmdi)
{
...
{
	bmdrv_submodule_request_irq(bmdi, DDR0A_IRQ_ID, bm1688_ddr0a_ecc_irq_handler);
	bmdrv_submodule_request_irq(bmdi, DDR0B_IRQ_ID, bm1688_ddr0b_ecc_irq_handler);
	bmdrv_submodule_request_irq(bmdi, DDR1_IRQ_ID, bm1688_ddr1_ecc_irq_handler);
	bmdrv_submodule_request_irq(bmdi, DDR2_IRQ_ID, bm1688_ddr2_ecc_irq_handler);
}
...
void bm1688_ddr_phy_reg_write16(struct bm_device_info *bmdi, u32 addr, u32 value)
{
	...
	bm1688_pci_slider_bar4_config_device_addr(&bmdi->cinfo.bar_info, addr);
...
int bm1688_ddr_top_init(struct bm_device_info *bmdi)
{
	...
	if (bmdi->cinfo.platform == PALLADIUM) {
		bm1688_pld_ddr_top_init(bmdi);
		return 0;
	}
	if (bm1688_lpddr4_init(bmdi) < 0)
		return -1;
	return 0;
#endif
}
