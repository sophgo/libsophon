...
#ifndef SOC_MODE
static int bm1688_cat_message(char *cat, u32 len, u32 num, char *buffer, char *res_buffer)
{
...
/*v1.4-bm1688-v4.3.0-(release):g2a38cf7*/
static int bm1688_get_bootload_version_number(struct bm_device_info *bmdi)
{
...
int bm1688_get_bootload_version(struct bm_device_info *bmdi)
{
...
/* 	if ((BM1688_BOARD_TYPE(bmdi) == BOARD_TYPE_BM1688X_EVB) ||
	    (BM1688_BOARD_TYPE(bmdi) == BOARD_TYPE_SC7_PRO))
		bm1688_cat_message(BLv_cat_bm1686, len, 4, (char *)stagemem_s2d->v_addr, bmdi->cinfo.boot_loader_version[0]);
	else */
		bm1688_cat_message(BLv_cat, len, 4, (char *)stagemem_s2d->v_addr, bmdi->cinfo.boot_loader_version[0]);

	bm1688_cat_message(BL_cat, len, 6, (char *)stagemem_s2d->v_addr, bmdi->cinfo.boot_loader_version[1]);
	mutex_unlock(&memcpy_info->stagemem_s2d.stage_mutex);

/* 	if ((BM1688_BOARD_TYPE(bmdi) != BOARD_TYPE_BM1688X_EVB) &&
	    (BM1688_BOARD_TYPE(bmdi) != BOARD_TYPE_SC7_PRO)) {
		ret = bm1688_get_bootload_version_number(bmdi);
		if (ret < 0) {
