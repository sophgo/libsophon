    ...
    )
{
    ...
    if (tiled2Linear == TRUE) {
        ...
        default:
            JLOG(ERR, "%s:%d Unknown format:(%d)\n", __FUNCTION__, __LINE__, (int)fb->format);
            return 0;
        }
        if ((pConv=(Uint8*)vmalloc(tiledFrameSize)) == NULL) {
    ...
    if (yuvFp)
    {
        if( !drv_fwrite(pYuv, sizeof(Uint8), frameSize , yuvFp) )
        {
            JLOG(ERR, "Frame Data fwrite failed file handle is %p, size is %ld \n", yuvFp, frameSize );
            return 0;
        }
        //fflush(yuvFp);
    }
...
static int getTblElement(drv_file_t *fp, char *str)
{
    while (1) {
        if (drv_fread(str, 1,256, fp) == 0)
            return 0;
        if ((str[0] != ';') && (str[0] != '/'))
            break;
    }
...
static int parseHuffmanTable(drv_file_t *fp, EncMjpgParam *param, int prec)
{
    char sLine[258] = {0,};
    unsigned int h[8] = {0,};
    BYTE *huffBit;
...
static int parseQMatrix(drv_file_t *fp, EncMjpgParam *param)
{
    char sLine[256] = {0,};
    int i,j;
    unsigned int h[8] = {0,};
...
int jpgGetHuffTable(char *huffFileName, EncMjpgParam *param, int prec)
{
    ...
    drv_file_t *huffFp = NULL;
    if (huffFileName[0] != 0) {
        strcpy(huffFilePath, huffFileName);
        huffFp = drv_fopen(huffFilePath, "rt");
        if (huffFp == NULL) {
...
int jpgGetQMatrix(char *qMatFileName, EncMjpgParam *param)
{
    ...
    drv_file_t *qMatFp = NULL;
    int i;

    if ( qMatFileName[0] != 0 ) {
        strcpy(qMatFilePath, qMatFileName);
        qMatFp = drv_fopen(qMatFilePath, "rt" );
        if ( !qMatFp ) {
...
int getJpgEncOpenParamDefault(JpgEncOpenParam *pEncOP, EncConfigParam *pEncConfig)
{
    ...
    mjpgParam = vzalloc(sizeof(EncMjpgParam));
    if (mjpgParam == NULL)
        return 0;
    ...
    if (pEncOP->jpg12bit == TRUE){
        memcpy(&pEncOP->huffVal[4][0], &pEncOP->huffVal[0][0], MIN(sizeof(pEncOP->huffBits[4][0]), 4*256));
        memcpy(&pEncOP->huffBits[4][0], &pEncOP->huffBits[0][0], MIN(sizeof(pEncOP->huffBits[4][0]), 4*256));
    }
    vfree(mjpgParam);
...
BOOL GetJpgEncOpenParam(JpgEncOpenParam *pEncOP, EncConfigParam *pEncConfig)
{
    ...
    BOOL            ret = FALSE;
    EncMjpgParam*   mjpgParam = NULL;
    char*           huffmanTablePath = NULL;
    char*           QMatTabPath = NULL;
    char*           cfgPath = NULL;
    char*           srcYuvFileName = pEncConfig->yuvFileName;

    encCfg = vmalloc(sizeof(ENC_CFG));
    if (encCfg == NULL)
        goto end;

    mjpgParam = vmalloc(sizeof(EncMjpgParam));
    if (mjpgParam == NULL)
        goto end;

    huffmanTablePath = vmalloc(MAX_FILE_PATH);
    if (huffmanTablePath == NULL)
        goto end;

    QMatTabPath = vmalloc(MAX_FILE_PATH);
    if (QMatTabPath == NULL)
        goto end;

    cfgPath = vmalloc(MAX_FILE_PATH);
    if (cfgPath == NULL)
        goto end;
    ...
    if (pEncOP) {
        ...
        //So, copy them to EX1 to EX2 to modeling rest 2 tables
        if (pEncOP->jpg12bit){
            memcpy(&pEncOP->huffVal[4][0], &pEncOP->huffVal[0][0], MIN(pEncOP->huffVal[4][0], 4*256));
            memcpy(&pEncOP->huffBits[4][0], &pEncOP->huffBits[0][0], MIN(pEncOP->huffBits[4][0], 4*256));
        }
    }
    ret =  TRUE;
...
static int GetValue(drv_file_t *fp, char *para, char *value)
{
    char lineStr[256];
    char paraStr[256];
    drv_fseek(fp, 0, SEEK_SET);

    while (1) {
        if (drv_fread(lineStr, 1, 256, fp) == 0)
            return 0;
...
int parseJpgCfgFile(ENC_CFG *pEncCfg, char *FileName)
{
    ...
    Fp = drv_fopen(FileName, "rt");
    if (Fp == NULL) {
        pr_err("   > ERROR: File not exist <%s>\n", FileName);
        goto end;
    }
...
size_t StoreYuvImageBurstFormat_V20(int chromaStride, Uint8 * dst, int picWidth, int picHeight, Uint32 bitDepth,
    PhysicalAddress addrY, PhysicalAddress addrCb, PhysicalAddress addrCr, int stride, FrameFormat format, int endian, CbCrInterLeave interLeave, PackedFormat packed)
{
    size_t size;
    int y, nY = 0, nCb, nCr;
    PhysicalAddress addr;
    size_t lumaSize, chromaSize = 0, chromaWidth = 0, chromaHeight =0;
    Uint8 * puc;
...
void GetFrameBufStride(FrameFormat subsample, CbCrInterLeave cbcrIntlv, PackedFormat packed, BOOL scalerOn,
                              ...
                              Uint32* oLumaStride, Uint32* oLumaHeight, Uint32* oChromaStride, Uint32* oChromaHeight)
{
    ...
    //lStride = JPU_CEIL(128, width);   //To make luma, chroma 64ailgned addr
    lHeight = height;
    cHeight = (height+1)/2;
    ...
    if (packed == PACKED_FORMAT_NONE) {
        ...
        case FORMAT_420: cStride = (lStride/2)*chromaDouble; cHeight = (height+1)/2; break;
        case FORMAT_422: cStride = (lStride/2)*chromaDouble; cHeight = height;   break;
        case FORMAT_440: cStride = lStride*chromaDouble;     cHeight = (height+1)/2; break;
        ...
        case PACKED_FORMAT_422_VYUY: lStride = JPU_CEIL(32, lStride*2); cStride = 0; cHeight = 0; break;
    ...
    JLOG(INFO,"GetFrameBufStride bytePerPixel:%d\n", bytePerPixel);
...
int GetDPBBufSize(int framebufFormat, int picWidth, int picHeight, int picWidth_C, int interleave)
{
    ...
    case FORMAT_400:
        framebufSize = framebufWidth * framebufHeight;
        if (interleave != 0)
            pr_err("Warning: 400 does not have interleave mode ! \n" );
        break;
    }
...
BOOL ParseEncTestLongArgs(void* config, const char* argName, char* value)
{
    ...
    else if (strcmp(argName, "yuv") == 0) {
        strcpy(enc->yuvFileName, value);
    }
    else if (strcmp(argName, "width") == 0) {
        enc->picWidth = atoi(value);
    }
    else if (strcmp(argName, "height") == 0) {
        enc->picHeight = atoi(value);
    }
    else if (strcmp(argName, "stream-endian") == 0) {
