...
JpgRet GetJpgInstance(JpgInst ** ppInst)
{
    ...
    for (i = 0; i < MAX_JPEG_NUM_INSTANCE; i++) {
        ...
        if (!pJpgInst) {
            JLOG(ERR, "GetJpgInstance 0 NULL.\n");
    ...
    if (i == MAX_JPEG_NUM_INSTANCE) {
        *ppInst = 0;
        JLOG(ERR, "GetJpgInstance no valid instance[%d].\n", i);
        return JPG_RET_FAILURE;
    }
    ...
    if ((pJpgInst->JpgInfo=(void*)vmalloc(handleSize)) == NULL) {
        pJpgInst->inUse = 0;
    ...
    *ppInst = pJpgInst;
    ...
    if (jdi_open_instance(pJpgInst->instIndex) < 0) {
        pJpgInst->inUse = 0;
        JLOG(ERR, "jdi_open_instance open instance[%d] failed.\n", pJpgInst->instIndex);
        return JPG_RET_FAILURE;
    }
...
{
    if (pop == 0) {
        JLOG(ERR, "CheckJpgDecOpenParam NULL\n");
        return JPG_RET_INVALID_PARAM;
    }
    if (pop->bitstreamBuffer % 8) {
        JLOG(ERR, "CheckJpgDecOpenParam bitstreamBuffer 0x%llx\n",pop->bitstreamBuffer);
        return JPG_RET_INVALID_PARAM;
    }
    if (pop->bitstreamBufferSize % 1024 || pop->bitstreamBufferSize < 1024) {
        JLOG(ERR, "CheckJpgDecOpenParam bitstreamBufferSize %d\n",pop->bitstreamBufferSize);
        return JPG_RET_INVALID_PARAM;
    }
    ...
    if (pop->chromaInterleave != CBCR_SEPARATED &&
        ...
        JLOG(ERR, "CheckJpgDecOpenParam chromaInterleave %d\n",pop->chromaInterleave);
            return JPG_RET_INVALID_PARAM;
    }

    if (pop->rotation != 0 && pop->rotation != 90 && pop->rotation != 180 && pop->rotation != 270) {
        JLOG(ERR, "CheckJpgDecOpenParam rotation %d\n",pop->rotation);
        return JPG_RET_INVALID_PARAM;
    }
    if ((Uint32)pop->mirror > (Uint32)MIRDIR_HOR_VER) {
        JLOG(ERR, "CheckJpgDecOpenParam mirror %d\n",pop->mirror);
        return JPG_RET_INVALID_PARAM;
    }

    if (pop->outputFormat == FORMAT_400 || pop->outputFormat == FORMAT_440) {
        JLOG(ERR, "CheckJpgDecOpenParam outputFormat %d\n",pop->outputFormat);
        return JPG_RET_INVALID_PARAM;
    }

    if (pop->packedFormat > PACKED_FORMAT_444) {
        JLOG(ERR, "CheckJpgDecOpenParam packedFormat %d\n",pop->packedFormat);
        return JPG_RET_INVALID_PARAM;
    }
    if (pop->packedFormat != PACKED_FORMAT_NONE) {
        if (pop->chromaInterleave != CBCR_SEPARATED) {
            JLOG(ERR, "CheckJpgDecOpenParam packedFormat %d chromaInterleave %d\n",
                pop->packedFormat, pop->chromaInterleave);
            return JPG_RET_INVALID_PARAM;
        }
    }
...
int JpgDecHuffTabSetUp(JpgInst * pJpgInst, JpgDecInfo *jpg, int instRegIndex)
{
    ...
    // MIN Tables
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_CTRL_REG, 0x003);
    JLOG(INFO,"JpgDecHuffTabSetUp DC Luma\n");
    //DC Luma
    for(j=0; j<16; j++)
    {
        ...
        JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_DATA_REG, (((temp & 0xFFFF) << 16) | HuffData));    // 32-bit
    }

    JLOG(INFO,"JpgDecHuffTabSetUp DC Chroma\n");
    //DC Chroma
    for(j=0; j<16; j++)
    {
    ...
    JLOG(INFO,"JpgDecHuffTabSetUp AC Luma\n");
    //AC Luma
    for(j=0; j<16; j++)
    {
    ...
    JLOG(INFO,"JpgDecHuffTabSetUp AC Chroma\n");
    //AC Chroma
    for(j=0; j<16; j++)
    {
    ...
    JLOG(INFO,"JpgDecHuffTabSetUp MAX Tables\n");
    // MAX Tables
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_CTRL_REG, 0x403);
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_ADDR_REG, 0x440);
    ...
    for(j=0; j<16; j++)
    {
        ...
        temp = (HuffData & 0x8000) >> 15;
        temp = (temp << 15) | (temp << 14) | (temp << 13) | (temp << 12) | (temp << 11) | (temp << 10) | (temp << 9) | (temp << 8) | (temp << 7 ) | (temp << 6) | (temp <<5) | (temp<<4) | (temp<<3) | (temp<<2) | (temp<<1)| (temp) ;
        JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_DATA_REG, (((temp & 0xFFFF) << 16) | HuffData));
    }
    ...
    // PTR Tables
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_CTRL_REG, 0x803);
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_ADDR_REG, 0x880);
    ...
    for(j=0; j<16; j++)
    {
        ...
        JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_DATA_REG, (((temp & 0xFFFFFF) << 8) | HuffData));
    }
    //DC Chroma
    for(j=0; j<16; j++)
    {
    ...
    // VAL Tables
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_CTRL_REG, 0xC03);
    ...
    for (i=0; i<12-HuffLength; i++) {
        JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_DATA_REG, 0xFFFFFFFF);
    }
    ...
    // end SerPeriHuffTab
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_CTRL_REG, 0x000);
...
int JpgDecHuffTabSetUp_12b(JpgInst * pJpgInst, JpgDecInfo *jpg, int instRegIndex)
{
    ...
    JLOG(INFO,"JpgDecHuffTabSetUp_12b DC Luma\n");
    //DC Luma
    for(j=0; j<16; j++)
    {
        HuffData = jpg->huffMin[0][j];
    ...
    // MAX Tables
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_CTRL_REG, 0x403);
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_ADDR_REG, 0x480);
    ...
    // PTR Tables
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_CTRL_REG, 0x803);
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_ADDR_REG, 0x900);
...
int JpgDecQMatTabSetUp(JpgInst * pJpgInst,JpgDecInfo *jpg, int instRegIndex)
{
    ...
    // SetPeriQMatTab
    // Comp 0
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_QMAT_CTRL_REG, 0x03);
    ...
    for (i=0; i<64; i++) {
        ...
        JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_QMAT_DATA_REG, val);
    }
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_QMAT_CTRL_REG, 0x00);

    // Comp 1
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_QMAT_CTRL_REG, 0x43);
    ...
    // Comp 2
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_QMAT_CTRL_REG, 0x83);
...
int JpgDecGramSetup(JpgInst * pJpgInst, JpgDecInfo * jpg, int instRegIndex, int timeout)
{
    ...
    while(1)
    {
        dMibStatus = JpuReadInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_BBC_BUSY_REG);
        if (dMibStatus == 0)
            break;
        ...
        {
            JLOG(ERR, "jJpgDecGramSetup timeout:%d\n", timeout);
            return JPG_RET_FAILURE;
        }
    }
    ...
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_BBC_CUR_POS_REG, dExtBitBufCurPos);    // next unit page pointer
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_BBC_CTRL_REG, (jpg->streamEndian<< 1) | 1);

    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_GBU_WPTR_REG, jpg->wordPtr);
    ...
    if (jpg->pagePtr & 1) {
        JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_GBU_BBIR_REG, 0);
        JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_GBU_BBHR_REG, 0);
    }
    else {
    ...
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_GBU_CTRL_REG, 4);
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_GBU_FF_RPTR_REG, jpg->bitPtr);
...
int decode_sof_header(JpgDecInfo *jpg)
{
    ...
    if ((numComp == 3 && (hSampFact[1] != 1 || hSampFact[2] != 1 || vSampFact[1] != 1 || vSampFact[2] != 1))) {
        JLOG(ERR, "Not Supported Sampling Factor\n");
        return 0;
    }
...
static int wraparound_bistream_data(JpgDecInfo *jpg, int return_type)
{
    ...
    dst_size = ((src_size+(JPU_GBU_WRAP_SIZE-1))&~(JPU_GBU_WRAP_SIZE-1));


    JLOG(ERR,"wraparound_bistream_data return_type:%d\n", return_type);
    if ((data_size + dst_size) > jpg->streamBufSize) {
        JLOG(ERR, "stream buffer size[%d] is not enouph to do workaround size=%d \n", jpg->streamBufSize, (data_size + dst_size));
        return -4;
    }
    data = (BYTE *)vmalloc(data_size);

    if (data)
        JpuReadMem(jpg->streamBufStartAddr, data, data_size, jpg->streamEndian);

    src = (BYTE *)vmalloc(src_size);
    dst = (BYTE *)vmalloc(dst_size);
    if (dst && src)
    {
...
int JpegDecodeHeader(JpgInst *    pJpgInst, JpgDecInfo *jpg)
{
    ...
    JLOG(INFO,"JpegDecodeHeader frameOffset:%d streamWrPtr:%llx streamBufStartAddr:%llx\n",
        jpg->frameOffset,jpg->streamWrPtr, jpg->streamBufStartAddr);
    ret = 1;
    ...
    JLOG(INFO,"JpegDecodeHeader ecsPtr:%d wrOffset:%d size:%d\n",jpg->ecsPtr, wrOffset, size);
    if (!jpg->ecsPtr)
    {
        int nextOffset = 0;
        int soiOffset = 0;
        ...
        if (size < 0)
        {
            jpg->consumeByte = jpg->consumeByte - (jpg->streamBufSize - jpg->frameOffset);
            ret = -1;
            goto DONE_DEC_HEADER;
        }
    ...
    if (jpg->headerSize > 0 && (jpg->headerSize > (jpg->streamBufSize - jpg->frameOffset - JPU_GBU_WRAP_SIZE))) { // if header size is smaller than room of stream end. copy the buffer to bistream start.
...
DONE_DEC_HEADER:
    ...
    if (jpg->streamBufSize - (jpg->frameOffset+jpg->ecsPtr) < JPU_GBU_WRAP_SIZE)
        return wraparound_bistream_data(jpg, -1);

    // the bellow should be after checking wraparound
    if (wrOffset - (jpg->frameOffset+jpg->ecsPtr)  < JPU_GBU_WRAP_SIZE &&
        jpg->streamEndflag == 0) {
            return -1;
    }
...
JpgRet CheckJpgEncOpenParam(JpgEncOpenParam * pop, JPUCap* cap)
{
    ...
    if (pop == 0) {
        JLOG(ERR, "CheckJpgEncOpenParam invalid NULL params.\n");
        return JPG_RET_INVALID_PARAM;
    }
    ...
    JLOG(INFO, "CheckJpgEncOpenParam picWidth:%d picHeight:%d.\n", picWidth, picHeight);

    if (pop->bitstreamBuffer % 8) {
        JLOG(ERR, "CheckJpgEncOpenParam bitstreamBuffer:%llx\n",pop->bitstreamBuffer);
        return JPG_RET_INVALID_PARAM;
    }
    if ((pop->bitstreamBufferSize - (4096)) < 4096) {
        JLOG(ERR, "CheckJpgEncOpenParam bitstreamBufferSize:%d\n",pop->bitstreamBufferSize);
        return JPG_RET_INVALID_PARAM;
    }

    if ((pop->bitstreamBufferSize - (4096)) % 4096) {
        JLOG(ERR, "CheckJpgEncOpenParam 2 bitstreamBufferSize:%d\n",pop->bitstreamBufferSize);
        return JPG_RET_INVALID_PARAM;
    }
    ...
    if (pop->intrEnableBit == 0) {
        JLOG(ERR, "CheckJpgEncOpenParam intrEnableBit:%d\n",pop->intrEnableBit);
        return JPG_RET_INVALID_PARAM;
    }

    if (pop->jpg12bit == TRUE && cap->support12bit == FALSE) {
        JLOG(ERR, "CheckJpgEncOpenParam not suppport params.\n");
...
int JpgEncGenHuffTab(JpgEncInfo * pEncInfo, int tabNum)
{
    ...
    huffsize = vmalloc(256);
    if (huffsize == NULL)
        goto end;

    huffcode = vmalloc(256);
    if (huffcode == NULL)
        goto end;

    bitleng    = pEncInfo->huffBits[tabNum];
    huffval    = pEncInfo->huffVal[tabNum];
...
int JpgEncLoadHuffTab(JpgInst *pJpgInst, int instRegIndex)
{
    ...
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_CTRL_REG, 0x3);

    JLOG(INFO,"JpgEncLoadHuffTab update\n");
    for (j=0; j<4; j++)
    {
        ...
        for (i=0; i<256; i++)
        {
            ...
            JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_DATA_REG, huffData);
        }
    }
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_CTRL_REG, 0x0);
    return 1;
}
...
int JpgEncLoadHuffTab_12b(JpgInst *pJpgInst, int instRegIndex)
{
    ...
    JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_HUFF_CTRL_REG, 0x3);
    JLOG(INFO,"JpgEncLoadHuffTab_12b update\n");
...
int JpgEncLoadQMatTab(JpgInst *pJpgInst, int instRegIndex)
{
    ...
    for (comp=0; comp<3; comp++) {
        ...
        JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_QMAT_CTRL_REG, 0x3 + t);
        ...
        for (i=0; i<64; i++) {
            divisor = pEncInfo->qMatTab[quantID][i];
            if (qprec)
            ...
            if (qprec){
                JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_QMAT_DATA_REG, (int)(divisor & 0xFFFF));
                JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_QMAT_DATA_REG, (int)(quotient & 0xFFFFFFFF));
            }else
                JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_QMAT_DATA_REG, (int)(divisor<<20)|(int)(quotient & 0xFFFFF));

        }
        JpuWriteInstReg(pJpgInst->coreIndex, instRegIndex, MJPEG_QMAT_CTRL_REG, t);
    }
...
int JpgEncEncodeHeader(JpgEncHandle handle, JpgEncParamSet * para)
{
    ...
    if (para->quantMode == JPG_TBL_NORMAL)
    {
        ...
        for (i=0; i<64; i++) {
            if (pEncInfo->q_prec0 == TRUE) {
                Uint16 q = pEncInfo->qMatTab[0][i];
                PUT_BYTE(p, (q>>8)&0xff);
                PUT_BYTE(p, q&0xff);
            }
            else {
                PUT_BYTE(p, pEncInfo->qMatTab[0][i]);
            }
        }
        ...
        if (pEncInfo->format != FORMAT_400) {
            ...
            for (i=0; i<64; i++) {
                if (pEncInfo->q_prec1 == TRUE) {
                    Uint16 q = pEncInfo->qMatTab[1][i];
                    PUT_BYTE(p, (q>>8)&0xff);
                    PUT_BYTE(p, q&0xff);
                }
                else {
                    PUT_BYTE(p, pEncInfo->qMatTab[1][i]);
                }
            }
        }
    }
    ...
    if (para->huffMode == JPG_TBL_NORMAL)
    {
        ...
        for (i=0; i<16; i++) {
            PUT_BYTE(p, pEncInfo->huffBits[0][i]);
        }

        for (i=0; i<numHuffValDC; i++) {
            PUT_BYTE(p, pEncInfo->huffVal[0][i]);
        }
        ...
        for (i=0; i<16; i++) {
            PUT_BYTE(p, pEncInfo->huffBits[1][i]);
        }

        for (i=0; i<numHuffValAC; i++) {
            PUT_BYTE(p, pEncInfo->huffVal[1][i]);
        }
        ...
        if (pEncInfo->format != FORMAT_400) {
            ...
            for (i=0; i<16; i++) {
                PUT_BYTE(p, pEncInfo->huffBits[2][i]);
            }
            for (i=0; i<numHuffValDC; i++) {
                PUT_BYTE(p, pEncInfo->huffVal[2][i]);
            }
            ...
            for (i=0; i<16; i++) {
                PUT_BYTE(p, pEncInfo->huffBits[3][i]);
            }

            for (i=0; i<numHuffValAC; i++) {
                PUT_BYTE(p, pEncInfo->huffVal[3][i]);
            }
            if (pEncInfo->jpg12bit == TRUE) {
                ...
                for (i=0; i<16; i++) {
                    PUT_BYTE(p, pEncInfo->huffBits[4][i]);
                }
                for (i=0; i<numHuffValDC; i++) {
                    PUT_BYTE(p, pEncInfo->huffVal[4][i]);
                }
                ...
                for (i=0; i<16; i++) {
                    PUT_BYTE(p, pEncInfo->huffBits[5][i]);
                }

                for (i=0; i<numHuffValAC; i++) {
                    PUT_BYTE(p, pEncInfo->huffVal[5][i]);
                }
            }
        }
    }
    ...
    for (i=0; i<pEncInfo->compNum; i++) {
        PUT_BYTE(p, (i+1));
        PUT_BYTE(p, ((pEncInfo->pCInfoTab[i][1]<<4) & 0xF0) + (pEncInfo->pCInfoTab[i][2] & 0x0F));
        PUT_BYTE(p, pEncInfo->pCInfoTab[i][3]);
    }
...
void JpgEncSetQualityFactor(JpgEncHandle handle, Uint32 quality, BOOL useStdTable)
{
    ...
    */
    ...
    for (i = 0; i<64; i++) {
        pLumaQTable[i] = (useStdTable == TRUE) ? std_luminance_quant_tbl[jpeg_natural_order[i]] : (Int32)pEncInfo->qMatTab[DC_TABLE_INDEX0][i];
    }
    for (i = 0; i<64; i++) {
        pChromaQTable[i] = (useStdTable == TRUE) ? std_chrominance_quant_tbl[jpeg_natural_order[i]] : (Int32)pEncInfo->qMatTab[AC_TABLE_INDEX0][i];
    }
    ...
    */
    ...
    for (i = 0; i < 64; i++) {
        ...
        pEncInfo->qMatTab[DC_TABLE_INDEX0][i] = temp;
    }
    ...
    for (i = 0; i < 64; i++) {
        ...
        pEncInfo->qMatTab[AC_TABLE_INDEX0][i] = temp;
    }
    ...
    memset((void*)pEncInfo->qMatTab[DC_TABLE_INDEX0], 0x00, 64 * qSize);
    memcpy((void*)pEncInfo->qMatTab[DC_TABLE_INDEX0], (void*)lumaQ2, 64 * qSize);

    memset((void*)pEncInfo->qMatTab[AC_TABLE_INDEX0], 0x00, 64 * qSize);
    memcpy((void*)ppEncInfo->qMatTab[AC_TABLE_INDEX0], (void*)chromaBQ2, 64 * qSize);
#endif

    qTableSize = 64 * qSize;
    memcpy((void*)pEncInfo->qMatTab[DC_TABLE_INDEX1], (void*)pEncInfo->qMatTab[DC_TABLE_INDEX0], qTableSize);
    memcpy((void*)pEncInfo->qMatTab[AC_TABLE_INDEX1], (void*)pEncInfo->qMatTab[AC_TABLE_INDEX0], qTableSize);
}
...
int JpgEncGetQulityTable(JpgEncHandle handle, JpgQualityTable *pTable)
{
    ...
    for (i = 0; i < 64; i++) {
        pTable->yQt[i]  = (BYTE) (pEncInfo->qMatTab[DC_TABLE_INDEX0][i] & 0xFF);
        pTable->CbQt[i] = (BYTE) (pEncInfo->qMatTab[AC_TABLE_INDEX0][i] & 0xFF);
        pTable->CrQt[i] = 0; // not use
        pTable->mcuPerECS = 0; // not use
    }
...
int JpgEncSetQulityTable(JpgEncHandle handle, JpgQualityTable *pTable)
{
    ...
    for (i = 0; i < 64; i++) {
        pEncInfo->qMatTab[DC_TABLE_INDEX0][i] = pTable->yQt[i];
        pEncInfo->qMatTab[AC_TABLE_INDEX0][i] = pTable->CbQt[i];
    }
...
JpgInst *GetJpgPendingInstEx(JpgInst * inst)
{
    ...
    if (inst->instIndex >= MAX_JPEG_NUM_INSTANCE)
        return NULL;

    return jip->instPendingInst[inst->instIndex];
}
...
void SetJpgPendingInstEx(JpgInst *inst, Uint32 instIdx)
{
    ...
    if (instIdx >= MAX_JPEG_NUM_INSTANCE)
        return;
