...
#define WAVE_ENC_FBC50_LUMA_TABLE_SIZE(_w, _h)                   (VPU_ALIGN2048(VPU_ALIGN32(_w))*VPU_ALIGN4(_h)/64)
#define WAVE_ENC_FBC50_CHROMA_TABLE_SIZE(_w, _h)                 (VPU_ALIGN2048(VPU_ALIGN32(_w)/2)*VPU_ALIGN4(_h)/128)
#define WAVE_ENC_FBC50_LOSSLESS_LUMA_8BIT_FRAME_SIZE(_w, _h)     ((VPU_ALIGN32(_w)+127)/128*VPU_ALIGN4(_h)*128)
#define WAVE_ENC_FBC50_LOSSLESS_CHROMA_8BIT_FRAME_SIZE(_w, _h)   ((VPU_ALIGN32(_w)/2+127)/128*VPU_ALIGN4(_h)/2*128)
#define WAVE_ENC_FBC50_LOSSY_LUMA_FRAME_SIZE(_w, _h, _tx)        ((VPU_ALIGN32(_w)+127)/128*VPU_ALIGN4(_h)*VPU_ALIGN32(_tx))
#define WAVE_ENC_FBC50_LOSSY_CHROMA_FRAME_SIZE(_w, _h, _tx)      ((VPU_ALIGN32(_w)/2+127)/128*VPU_ALIGN4(_h)/2*VPU_ALIGN32(_tx))
...
#define VPU_ALIGN2(_x)              (((_x)+0x01)&~0x01)
...
This command must be issued through VPU_DecGiveCommand() before VPU_DecIssueSeqInit().
...
Then VPU_DecIsseuSeqInit() are called before frame buffer allocation for a new sequence.
@endverbatim
*/
...
*/
    ...
    DEC_SET_TARGET_SPATIAL_ID, /**< This command decodes only a frame whose spatial id is equal to or less than the given target spatial id. (AV1 only) */
...
*/
    ...
    ENC_SET_SEI_NAL_DATA, /**< This command sets variables in <<vpuapi_h_EncSeiNalData>> structure for SEI encoding.  */
    DEC_GET_QUEUE_STATUS, /**< This command returns the status of queued commands and the status of report queued commands for the current decode instance. */
    ENC_GET_QUEUE_STATUS, /**< This command returns the status of queued commands and the status of report queued commands for the current encode instance. */
    GET_BANDWIDTH_REPORT, /**< This command reports the amount of bytes which are transferred on AXI bus. (WAVE only) */
...
*/
 ...
 /**
    set gop period,
    if period = 0, only have P frame
    if period != 0, will create IDR frame first
    if period = -1, set old period
 */
    ENC_SET_PERIOD,
/**
@verbatim
 ...
 */
 typedef enum {
    ...
    FORMAT_YUV444_P10_32BIT_PACKED, /**< 10bit YUV444 A2Y10U10V10(32bit) CSC 1plane packed format */
    FORMAT_MAX,
} FrameBufferFormat;
...
NOTE: This is only for WAVE6 IP.
*/
typedef enum {
...
#define PRODUCT_ID_W6_SERIES(x)     (x == PRODUCT_ID_617 || x == PRODUCT_ID_627 || x == PRODUCT_ID_637)
#define PRODUCT_ID_W_SERIES(x)      (x == PRODUCT_ID_617 || x == PRODUCT_ID_627 || x == PRODUCT_ID_637 || x == PRODUCT_ID_521 || x == PRODUCT_ID_511 || x == PRODUCT_ID_517)
#define PRODUCT_ID_CODA_SERIES(x)  !PRODUCT_ID_W_SERIES(x)
...
/**
* @brief    This is an enumeration type for representing mode of temporal ID.
*/
typedef enum {
    TEMPORAL_ID_MODE_ABSOLUTE,    /**< Use the target temporal_id as absolute value. TARGET_DEC_TEMP_ID = TARGET_DEC_TEMP_ID_PLUS1 - 1. */
    TEMPORAL_ID_MODE_RELATIVE,    /**< Use the target temporal_id as relative value. TARGET_DEC_TEMP_ID = SPS_MAX_SUB_LAYER - REL_TARGET_TEMP_ID. */
} TemporalIdMode;
...
/**
* @brief    This is a data structure of AUX buffer.
*/
typedef struct {
    Uint32 index;         /**< The index of AUX buffer. */
    Uint32 size;          /**< The size of AUX buffer. */
    PhysicalAddress addr; /**< The physical address of AUX buffer. */
} AuxBuffer;

/**
* @brief    This is a data structure of AUX buffer information. It is used for parameter when AUX buffers are allocated on Host side.
*/
typedef struct {
    Uint32 num;          /**< The number of AUX buffers.*/
    AuxBuffer* bufArray; /**< The AUX buffer array. please refer the <<vpuapi_h_AuxBuffer>>. */
    AuxBufferType type;  /**< The type of AUX buffer. please refer the <<vpuapi_h_AuxBufferType>>. */
} AuxBufferInfo;

/**
* @brief This structure is used for setting external buffer information for each instance.
*/
typedef struct {
    PhysicalAddress tempBufBase;   /**< It indicates the start address of temp buffer. */
    Uint32          tempBufSize;   /**< It indicates the size of temp buffer. */
    PhysicalAddress secAxiBufBase; /**< It indicates the start address of secondary-axi buffer. */
    Uint32          secAxiBufSize; /**< It indicates the size of secondary-axi buffer. */
    PhysicalAddress workBufBase;   /**< It indicates the start address of work buffer. */
    Uint32          workBufSize;   /**< It indicates the size of work buffer. */
    PhysicalAddress arTblBufBase;  /**< It indicates the start address of AR table buffer. (Encoder only). */
} InstanceBuffer;
...
typedef struct {
    ...
    BOOL    supportOCSRAM;              /**< This indicates whether a product supports the On-Chip SRAM in VPU. */
} VpuAttr;

/**
* @brief    This is a data structure of VPU debug information.
*/
typedef struct {
    Uint32 regs[0xC0]; /**< The value of debug register. The meaning of each register is different for PRODUCT.*/
} VpuDebugInfo;
...
/**
* @brief    This is a data structure of DRAM information(CODA960 and BODA950 only) and CFRAME50 configuration(WAVE only)
...
/**
* @brief    This is a data structure of AUX buffer size information for decoder.
*/
typedef struct {
    Uint32 width;       /**< The width of picture. */
    Uint32 height;      /**< The height of picture. */
    AuxBufferType type; /**< The type of AUX buffer. please refer the <<vpuapi_h_AuxBufferType>>. */
} DecAuxBufferSizeInfo;

/**
* @brief    This is a data structure for reporting cycle information. (WAVE6 only)
*/
typedef struct {
    Uint32  hostCmdStartTick;    /**< Start Tick of DEC_PIC/ENC_PIC host command for the picture */
    Uint32  hostCmdEndTick;      /**< End tick of DEC_PIC/ENC_PIC host command for the picture (until host get the result for the picture) */
    Uint32  processingStartTick; /**< Start tick of processing hw block for the picture */
    Uint32  processingEndTick;   /**< End tick of processing hw block for the picture */
    Uint32  vpuStartTick;        /**< Start tick of decoding/encoding for the picture */
    Uint32  vpuEndTick;          /**< End tick of decoding/encoding for the picture */
    Uint32  frameCycle;          /**< The total cycle of host command for the picture */
    Uint32  processingCycle;     /**< The total cycle of processing for the picture */
    Uint32  vpuCycle;            /**< The total cycle of decoding/encoding for the picture except processing cycle */
    Uint32  preProcessingStartTick; /**< Start tick of pre-processing hw block for the picture */
    Uint32  preProcessingEndTick;   /**< End tick of pre-processing hw block for the picture */
    Uint32  preVpuStartTick;        /**< Start tick of pre-decoding/encoding for the picture */
    Uint32  preVpuEndTick;          /**< End tick of pre-decoding/encoding for the picture */
    Uint32  preProcessingCycle;     /**< The total cycle of pre-processing for the picture */
    Uint32  preVpuCycle;            /**< The total cycle of pre-decoding/encoding for the picture except pre-processing cycle */
} ReportCycle;
...
*/
    ...
    BOOL             isSecureInst; /**< It indicates whether the instance is secure instance or non-secure instance. (WAVE6 only). */
    Uint32           instPriority; /**< It indicates the priority value of instance. (0 ~ 255). (WAVE6 only). */
...
*/
    ...
    // Report Information
    Int32           maxTemporalLayers;      /**< It indicates the max number of temporal sub-layers. */
    Int32           maxSpatialLayers;       /**< It indicates the max number of spatial sub-layers for AV1. (AV1 only) */
/**
@verbatim
...
*/
    ...
    PhysicalAddress userDataBufAddr;/**< Refer to userDataBufAddr in <<vpuapi_h_DecOutputExtData>>. */
...
*/
    ...
    Uint32          vlcBufSize;   /**< The size of vlc buffer */
    Uint32          paramBufSize; /**< The size of param buffer */
} DecInitialInfo;
...
When this variable is not zero, each bit corresponds to the `H265_USERDATA_FLAG_XXX` and `H264_USERDATA_FLAG_XXX`.
...
// H264 USER_DATA(SPS & SEI) ENABLE FLAG
#define H264_USERDATA_FLAG_RESERVED_0         (0)
#define H264_USERDATA_FLAG_RESERVED_1         (1)
...
...
...
*/
    ...
    PhysicalAddress userDataBufAddr;    /**< This is the address of user data buffer. */
...
typedef struct {
    ...
    int spatialId;               /**< It indicates a spatial ID of the picture. */
} Av1Info;
...
*/
    ...
    Av1Info  av1Info;               /**< This is AV1 picture information. Refer to <<vpuapi_h_Av1Info>>. */
...
*/
    ...
    PhysicalAddress rdPtr;  /**< A stream buffer read pointer for the current decoder instance */
    PhysicalAddress wrPtr;  /**< A stream buffer write pointer for the current decoder instance */
/**
@verbatim
...
If it is 1, HOST application can get the new sequence information by calling VPU_DecIssueSeqInit().
...
*/
    ...
    ReportCycle cycle; /**< Refer to <<vpuapi_h_ReportCycle>>. (WAVE6 only) */
/**
@verbatim
 ...
 */
 typedef struct {
    ...
    FrameBuffer   framebufPool[MAX_REG_FRAME]; /**< This is an array of <<vpuapi_h_FrameBuffer>> which contains the information of each frame buffer. When WTL is enabled, the number of framebufPool would be [number of compressed frame buffer] x 2, and the starting index of frame buffer for WTL is framebufPool[number of compressed frame buffer]. */
} DecGetFramebufInfo;

/**
 * @brief   This is a data structure of queue command information. It is used for parameter when host issues DEC_GET_QUEUE_STATUS/ENC_GET_QUEUE_STATUS of GiveCommand. (WAVE only)
 */
typedef struct {
    Uint32  instanceQueueCount; /**< This variable indicates the number of queued commands of the instance. (WAVE5 only) */
    Uint32  reportQueueCount;   /**< This variable indicates the number of report queued commands of the instance. (WAVE5 only) */
    BOOL    instanceQueueFull;  /**< This variable indicates whether queued commands of the instance is full or not. */
    BOOL    reportQueueEmpty;   /**< This variable indicates whether report queued commands of the instance is empty or not. */
} QueueStatusInfo;
...
*/
typedef enum {
    C9_ENC_CHANGE_PARAM_GOP_NUM       = (1<<0),
    C9_ENC_CHANGE_PARAM_INTRA_QP      = (1<<1),
    C9_ENC_CHANGE_PARAM_BIT_RATE      = (1<<2),
    C9_ENC_CHANGE_PARAM_FRAME_RATE    = (1<<3),
    C9_ENC_CHANGE_PARAM_INTRA_REFRESH = (1<<4),
    C9_ENC_CHANGE_PARAM_SLICE_MODE    = (1<<5),
    C9_ENC_CHANGE_PARAM_MIN_MAX_QP    = (1<<6),
    C9_ENC_CHANGE_PARAM_HEC_MODE      = (1<<6),
    C9_ENC_CHANGE_PARAM_CABAC_MODE    = (1<<7),
    C9_ENC_CHANGE_PARAM_PPS_ID        = (1<<11),
    C9_ENC_CHANGE_PARAM_PIC_PARAM     = (1<<14),
} C9EncChangeParam;
...
*/
typedef struct {
    int customGopSize;                       /**< The size of custom GOP (1 ~ 8) */
    CustomGopPicParam picParam[MAX_GOP_NUM]; /**< Picture parameters of Nth picture in custom GOP */
} CustomGopParam;

/**
* @brief    This is a data structure for temporal layer parameters. (WAVE6 encoder only)
*/
typedef struct {
    Uint32 enChangeQp; /**< It enables to change temporal layer QP. */
    Uint32 qpI;        /**< A QP value of I Pictures. (0 ~ 51) */
    Uint32 qpP;        /**< A QP value of P Pictures. (0 ~ 51) */
    Uint32 qpB;        /**< A QP value of B Pictures. (0 ~ 51) */
} TemporalLayerParam;

/**
* @brief    This is a data structure for setting custom map options in H.265/HEVC encoder.
*/
typedef struct {
...
/**
 * @brief    This is a data structure for setting RGB to YUV CSC(color space conversion) parameter. (WAVE6 only)
 */
typedef struct {
/**
@verbatim
This variable defines the ARGB/AYUV format order.
...
/**
* @brief    This is a data structure of AUX buffer size information for encoder.
*/
typedef struct {
    Uint32 width;                    /**< The width of picture. */
    Uint32 height;                   /**< The height of picture. */
    AuxBufferType type;              /**< The type of AUX buffer. please refer the <<vpuapi_h_AuxBufferType>>. */
    MirrorDirection mirrorDirection; /**< The mirroring direction option. please refer the <<vpuapi_h_MirrorDirection>>. */
    Int32 rotationAngle;             /**< The rotate angle option. */
} EncAuxBufferSizeInfo;
...
/**
@verbatim
Conformance window information.

left   : A left offset of conformance window.
top    : A top offset of conformance window.
right  : A right offset of conformance window.
bottom : A bottom offset of conformance window.
@endverbatim
*/
    VpuRect confWin;
...
@* 1 : Each picture in GOP is allocated in fixedBitRatio of the total bit budget.
@endverbatim
*/
    int bitAllocMode;
/**
...
the fixed bit ratio 2:1:1. This is only valid when BitAllocMode is 1.
...
*/
    ...
    int             s2SearchRangeXDiv4; /**< X coordinate pixel value of S2 ME search range which is divided by 4 (min(4,abs(s2SearchCenterXDiv4)) ~ min(32,abs(s2SearchCenterXDiv4))). The default value is 32.*/
    int             s2SearchRangeYDiv4; /**< Y coordinate pixel value of S2 ME search range which is divided by 4 (min(4,abs(s2SearchCenterYDiv4)) ~ min(16,abs(s2SearchCenterYDiv4))). The default value is 16*/
    BOOL            enReportNalSize;   /**< It enables NAL size report. */
    PhysicalAddress reportNalSizeAddr; /**< It specifies the address of nal size report */
} EncWave5Param;

/**
* @brief    This is a data structure of encoder parameters for WAVE6.
*/
typedef struct {
/**
@verbatim
...
/**
@verbatim
A decoding refresh type of intra picture. (HEVC only)
...
@endverbatim
*/
    Uint32 decodingRefreshType;
/**
@verbatim
A period of IDR picture (0 ~ 65535). (AVC only)
...
/**
@verbatim
A period of intra picture (0 ~ 65535).
...
/**
@verbatim
A GOP structure preset option (1 ~ 16).
...
*/
    Uint32 gopPreset;
    BOOL   enLongTerm;                             /**< It enables long-term reference function. */
    Uint32 enRateControl;                          /**< It enables rate control. */
/**
@verbatim
...
*/
    Uint32 frameRate;
    Uint32 encBitrate;                             /**< A target bitrate for rate control. */
    Uint32 vbvBufferSize;                          /**< VBV buffer size in msec. */
    Uint32 cuLevelRateControl;                     /**< It enables CU level rate control. */
    Uint32 enHvsQp;                                /**< It enables CU QP adjustment for subjective quality enhancement. */
/**
@verbatim
It specifies the scale for CU QP derivation. (1 ~ 4).
Only available when enHvsQp is 1.
@endverbatim
*/
    Uint32 hvsQpScaleDiv2;
/**
@verbatim
It specifies maximum delta QP from CTU QP. (0 ~ 12).
Only valid when CULevelRateControl is 1.
@endverbatim
*/
    Uint32 maxDeltaQp;
/**
@verbatim
An initial QP. (-1 ~ 51).
If this value is smaller than 0 or larger than 51, firmware decides the initial QP.
@endverbatim
*/
    Int32 rcInitialQp;
    Uint32 rcUpdateSpeed;                          /**< RC update speed. */
    Uint32 maxBitrate;                             /**< A maximum bitrate for for rate control. */
/**
@verbatim
It specifies the rate control mode. (0 ~ 1).
It is valid when enRateControl is on.
...
*/
    Uint32 rcMode;
    Uint32 rcInitialLevel;                         /**< An initial rc buffer level. */
    Uint32 picRcMaxDqp;                            /**< Max delta QP for picture level RC. */
    Uint32 enBgDetect;                             /**< It enables background detection. */
/**
@verbatim
It specifies the threshold of max difference that is used in s2me block. (0 ~ 255).
...
/**
@verbatim
It specifies the threshold of mean difference that is used in s2me block. (0 ~ 255).
It is valid  when background detection is on.
@endverbatim
*/
    Uint32 bgThMeanDiff;
/**
@verbatim
It specifies the difference between the lambda QP value of background
and the lambda QP value of foreground. (-16 ~ 15).
@endverbatim
*/
    Int32 bgDeltaQp;
    Uint32 strongIntraSmoothing;                   /**< It enables strong intra smoothing. (HEVC only). */
    Uint32 constrainedIntraPred;                   /**< It enables constrained intra prediction. (HEVC/AVC only). */
    Uint32 intraNxN;                               /**< It enables intra NxN PU mode. (HEVC/AV1 only). */
    Uint32 intraTransSkip;                         /**< It enables transform skip for intra CU. (HEVC/AV1 only). */
/**
@verbatim
An intra refresh mode.
...
*/
    ...
    Uint32 enTemporalMVP;                          /**< It enables temporal motion vector prediction. (HEVC only). */
/**
@verbatim
It specifies a search center mode for motion estimation.

@* 0 : the ME search center is fixed as (0, 0).
@* 1 : the ME search center is decided by MVP (motion vertor prediction).
@endverbatim
*/
    Uint32 meCenter;
/**
@verbatim
It enables CABAC. (AVC only).

@* 0 : CAVLC is used for entropy coding
@* 1 : CABAC is used for entropy coding
@endverbatim
*/
    Uint32 enCABAC;
    Uint32 transform8x8;                           /**< It enables transform 8x8. (AVC only). */
    Uint32 lfCrossSliceBoundaryFlag;               /**< It enables the use of in-loop filtering across slice boundaries. (HEVC/AVC only). */
    Uint32 enDBK;                                  /**< It enables the in-loop deblocking filter. */
    Int32 betaOffsetDiv2;                          /**< BetaOffsetDiv2 for deblocking filter. (HEVC/AVC only). */
    Int32 tcOffsetDiv2;                            /**< TcOffsetDiv2 for deblocking filter. (HEVC/AVC only). */
    Uint32 lfSharpness;                            /**< Lf sharpness. (AV1 only). */
    Uint32 enSAO;                                  /**< It enables sample adaptive offset. (HEVC only). */
    Uint32 enCdef;                                 /**< It enables CDEF. (AV1 only). */
    Uint32 enWiener;                               /**< It enables wiener. (AV1 only). */
    Uint32 qp;                                     /**< A quantization parameter of intra picture (0 ~ 51). */
    Uint32 minQpI;                                 /**< A minimum QP for I picture. (0 ~ 51). */
    Uint32 maxQpI;                                 /**< A maximum QP for I picture. (0 ~ 51). */
    Uint32 minQpP;                                 /**< A minimum QP for P picture. (0 ~ 51). */
    Uint32 maxQpP;                                 /**< A maximum QP for P picture. (0 ~ 51). */
    Uint32 minQpB;                                 /**< A minimum QP for B picture. (0 ~ 51). */
    Uint32 maxQpB;                                 /**< A maximum QP for B picture. (0 ~ 51). */
    Int32 yDcQpDelta;                              /**< It specifies the AV1 Qindex offset for Luma DC. (-64 ~ 63). (AV1 only). */
    Int32 uDcQpDelta;                              /**< It specifies the AV1 Qindex offset for DC of Cb component. (-64 ~ 63). (AV1 only). */
    Int32 vDcQpDelta;                              /**< It specifies the AV1 Qindex offset for DC of Cr component. (-64 ~ 63). (AV1 only). */
    Int32 uAcQpDelta;                              /**< It specifies the AV1 Qindex offset for AC of Cb component. (-64 ~ 63). (AV1 only). */
    Int32 vAcQpDelta;                              /**< It specifies the AV1 Qindex offset for AC of Cr component. (-64 ~ 63). (AV1 only). */
    Int32 cbQpOffset;                              /**< QP offset for Cb color component. (HEVC/AVC only). */
    Int32 crQpOffset;                              /**< QP offset for Cr color component. (HEVC/AVC only). */
    Uint32 enScalingList;                          /**< It enables to apply default scaling list. */
    Uint32 enAdaptiveRound;                        /**< It enables adaptive rounding and ignore QRoundIntra and QRoundInter. */
    Uint32 qRoundIntra;                            /**< It specifies a customized quantization rounding offset for intra pictures. (0 ~ 255). */
    Uint32 qRoundInter;                            /**< It specifies a customized quantization rounding offset for inter pictures. (0 ~ 255). */
/**
@verbatim
It disables the transform coefficient clearing algorithm in P or B picture.

@* 0 : All-zero coefficient block is evaluated additionally in RDO
@* 1 : All-zero coefficient block is not evaluated in RDO
@endverbatim
*/
    Uint32 disableCoefClear;
    Int32 lambdaDqpIntra;                          /**< It specifies a lambda delta QP for intra pictures. (-32 ~ 31). */
    Int32 lambdaDqpInter;                          /**< It specifies a lambda delta QP for inter pictures. (-32 ~ 31). */
/**
@verbatim
A slice mode. (HEVC/AVC only).
...
/**
@verbatim
It specifies a size of slice. (0 ~ 262143). (HEVC/AVC only).
The number of CTU in case of SLICE_MODE = 1.
@endverbatim
*/
    Uint32 sliceArg;
    Uint32 enQpMap;                                /**< It enables custom QP map. */
    Uint32 enModeMap;                              /**< It enables custom mode map. */
    Uint32 enQRoundOffset;                         /**< It enables custom QRoundIntra and QRoundInter. */
    Uint32 enStillPicture;                         /**< It enables main/main10_still_profile encoding according to bit_depth. (HEVC only). */
/**
@verbatim
A level indicator.

@* 0 : VPU decides a proper level value.
@* Non-0 : host input value determines a level value as below:
@** For AVC, host inputs the level value 10.
@** For HEVC, host inputs the level value 30.

If you want to set the level value to 5 for AVC, set 5*10 = 50 as a host level value. For HEVC, you can set 5*30 = 150 as a host level value.
@endverbatim
*/
    Uint32 level;
/**
@verbatim
A tier indicator (HEVC only)
...
@* 0 : VPU decides whether the profile is main or main10 according to bit_depth.
@* 1 : main
@* 2 : main10
@* 3 : main still picture

In case of AV1, the profile is always "main". In case of AVC, the profile is decided by VPU according to "USE_ENTROPY_CODING_MODE" and "USE_TRANSFORM_8x8".
@endverbatim
*/
    Uint32 profile;                                /**< It enables custom QRoundIntra and QRoundInter. */
    Uint32 colTileNum;                             /**< It specifies the number of tile rows.  (1 ~ 2). (AV1 only). */
    Uint32 rowTileNum;                             /**< It specifies the number of tile columns. (1 ~ 16). (AV1 only). */
/**
@verbatim
Conformance window information

@* left   : A left offset of conformance window. (0 ~ 8192).
@* top    : A top offset of conformance window. (0 ~ 8192).
@* right  : A right offset of conformance window. (0 ~ 8192).
@* bottom : A bottom offset of conformance window. (0 ~ 8192).
@endverbatim
*/
    VpuRect confWin;
/**
@verbatim
It enables forced header encoding mode.
...
@endverbatim
*/
    Uint32 forcedIdrHeader;
    Uint32 enCustomLambda;                         /**< It enables custom lambda. */
    Uint32 customLambdaSSD[MAX_CUSTOM_LAMBDA_NUM]; /**< It specifies the custom lambda SSD value. (0 ~ 16383). */
    Uint32 customLambdaSAD[MAX_CUSTOM_LAMBDA_NUM]; /**< It specifies the custom lambda SAD value. (0 ~ 127). */
    CustomGopParam gopParam;                       /**< <<vpuapi_h_CustomGopParam>> */
/**
@verbatim
It specifies the QP data of each temporal layer which host wants to change.
If host disable the change option, temporal layer use the intra QP basically.
The maximum changeable temporal layer number is 4.
please refer <<vpuapi_h_TemporalLayerParam>>.
@endverbatim
*/
    TemporalLayerParam tempLayer[MAX_NUM_CHANGEABLE_TEMPORAL_LAYER];
/**
@verbatim
It specifies the number of temporal layer. (1 ~ 4).
This is only valid when gopPreset is 2 ~ 16.
@endverbatim
*/
    Uint32 tempLayerCnt;
    BOOL enReportMvHisto;                          /**< It enables MV histogram count report. */
    Uint32 reportMvHistoThreshold0;                /**< The value of MV threshold 0. */
    Uint32 reportMvHistoThreshold1;                /**< The value of MV threshold 1. */
    Uint32 reportMvHistoThreshold2;                /**< The value of MV threshold 2. */
    Uint32 reportMvHistoThreshold3;                /**< The value of MV threshold 3. */
/**
@verbatim
The endianess of the custom map buffer.
...
/**
* @brief This is an enumeration for encoder parameter change. (WAVE6 encoder only)
*/
typedef enum {
    // COMMON parameters which can be changed frame by frame.
    W6_ENC_CHANGE_PARAM_PPS                = (1<<0),
    W6_ENC_CHANGE_PARAM_GOP_PARAM          = (1<<1),
    W6_ENC_CHANGE_PARAM_INTRA_PARAM        = (1<<2),
    W6_ENC_CHANGE_PARAM_TEMPORAL_QP_PARAM  = (1<<4),
    W6_ENC_CHANGE_PARAM_RDO_PARAM          = (1<<5),
    W6_ENC_CHANGE_PARAM_SLICE_PARAM        = (1<<6),
    W6_ENC_CHANGE_PARAM_RC_TARGET_RATE     = (1<<10),
    W6_ENC_CHANGE_PARAM_RC_PARAM           = (1<<11),
    W6_ENC_CHANGE_PARAM_MIN_MAX_QP         = (1<<12),
    W6_ENC_CHANGE_PARAM_RC_MAX_BITRATE     = (1<<13),
    W6_ENC_CHANGE_PARAM_RC_VBV_BUFFER_SIZE = (1<<14),
    W6_ENC_CHANGE_PARAM_VUI_HRD_PARAM      = (1<<20),
    W6_ENC_CHANGE_PARAM_BG_PARAM           = (1<<22),
    W6_ENC_CHANGE_PARAM_QROUND_OFFSET      = (1<<26),
    W6_ENC_CHANGE_PARAM_QUANT_PARAM        = (1<<27),
} W6EncChangeParam;
...
*/
typedef struct {
    int enable_option;                  /**< A flag to decide which parameter will change, <<vpuapi_h_W5EncChangeParam>> */

    // W5_ENC_CHANGE_PARAM_PPS (lossless, WPP can't be changed while encoding)
    int constIntraPredFlag;             /**< It enables constrained intra prediction. */
    int lfCrossSliceBoundaryEnable;     /**< It enables filtering across slice boundaries for in-loop deblocking. */
    ...
    // W5_ENC_CHANGE_PARAM_INDEPEND_SLICE
    /**
    @verbatim
    A slice mode for independent slice
    ...
    */
    ...
    // W5_ENC_CHANGE_PARAM_DEPEND_SLICE
    /**
    @verbatim
    A slice mode for dependent slice
    ...
    */
    ...
    // W5_ENC_CHANGE_PARAM_RDO (MonoChrom, and RecommendEncParam
    ...
    // W5_ENC_CHANGE_PARAM_RC_FRAME_RATE
    int frameRate;                      /**< A frame rate indicator for rate control */

    // W5_ENC_CHANGE_PARAM_RC (rcEnable, cuLevelRc, bitAllocMode, RoiEnable,
    // RcInitQp can't be changed while encoding)
    ...
    // W5_ENC_CHANGE_PARAM_RC_MIN_MAX_QP
    int minQpI;                         /**< A minimum QP of I picture for rate control */
    int maxQpI;                         /**< A maximum QP of I picture for rate control */
    int hvsMaxDeltaQp;                  /**< A maximum delta QP for HVS */

    // W5_ENC_CHANGE_PARAM_RC_INTER_MIN_MAX_QP
    int minQpP;                         /**< A minimum QP of P picture for rate control */
    int minQpB;                         /**< A minimum QP of B picture for rate control */
    int maxQpP;                         /**< A maximum QP of P picture for rate control */
    int maxQpB;                         /**< A maximum QP of B picture for rate control */

    // W5_ENC_CHANGE_PARAM_RC_BIT_RATIO_LAYER
    /**
    @verbatim
    A fixed bit ratio (1 ~ 255) for each picture of GOP's bit
    allocation
    ...
    */
    ...
    // W5_ENC_CHANGE_PARAM_BG (bgDetectEnable can't be changed while encoding)
    int    s2fmeDisable;   /**< It disables s2me_fme (only for AVC encoder). */
    Uint32 bgThrDiff;      /**< It specifies the threshold of max difference that is used in s2me block. It is valid when background detection is on. */
    Uint32 bgThrMeanDiff;  /**< It specifies the threshold of mean difference that is used in s2me block. It is valid  when background detection is on. */
    Uint32 bgLambdaQp;     /**< It specifies the minimum lambda QP value to be used in the background area. */
    ...
    // W5_ENC_CHANGE_PARAM_CUSTOM_LAMBDA
    PhysicalAddress customLambdaAddr; /**< It specifies the address of custom lambda map.  */

    // W5_ENC_CHANGE_PARAM_INTRA_PARAM
    int intraQP;                        /**< A quantization parameter of intra picture */
    int intraPeriod;                    /**< A period of intra picture in GOP size */
    ...
    */
    ...
    // W5_ENC_CHANGE_PARAM_RC_WEIGHT
    Uint32 rcWeightBuf;   /**< It is the smoothing factor in the estimation. (Min: 1, Max: 255) This parameter indicates the speed of adjusting bitrate toward fullness of buffer as a reaction parameter. As it becomess larger, the bit-rate error promptly affects the target bit allocation of the following picture. */
    Uint32 rcWeightParam; /**< Adjusts the speed of updating parameters to the rate control model. If RcWeightFactor is set with a large value, the RC parameters are updated slowly. (Min: 1, Max: 31)*/

    // W5_ENC_CHANGE_PARAM_VUI_HRD_PARAM
...
/**
* @brief This is a data structure for WAVE6 encoding parameters that have changed.
*/
typedef struct {
    Uint32 enableOption; /**< A flag to decide which parameter will change, <<vpuapi_h_W6EncChangeParam>> */

    // W6_ENC_CHANGE_PARAM_PPS
    /**
    @verbatim
    It enables CABAC. (AVC only).
    ...
    */
    ...
    // W6_ENC_CHANGE_PARAM_INTRA_PARAM
    Uint32 qp; /**< A quantization parameter of intra picture (0 ~ 51). */
    /**
    @verbatim
    A decoding refresh type of intra picture. (HEVC only)
    ...
    */
    ...
    // W6_ENC_CHANGE_PARAM_TEMPORAL_QP_PARAM
    /**
    @verbatim
    It specifies the QP data of each temporal layer which host wants to change.
    If host disable the change option, temporal layer use the intra QP basically.
    ...
    */
    ...
    // W6_ENC_CHANGE_PARAM_SLICE_PARAM
    /**
    @verbatim
    A slice mode. (HEVC/AVC only).
    ...
    */
    ...
    // W6_ENC_CHANGE_PARAM_RC_TARGET_RATE
    Uint32 encBitrate; /**< A target bitrate for rate control. */

    // W6_ENC_CHANGE_PARAM_RC_PARAM
    Uint32 enRateControl;      /**< It enables rate control. */
    Uint32 cuLevelRateControl; /**< It enables CU level rate control. */
    /**
    @verbatim
    ...
    */
    ...
    // W6_ENC_CHANGE_PARAM_MIN_MAX_QP
    Uint32 minQpI; /**< A minimum QP for I picture. (0 ~ 51). */
    Uint32 maxQpI; /**< A maximum QP for I picture. (0 ~ 51). */
    Uint32 minQpP; /**< A minimum QP for P picture. (0 ~ 51). */
    Uint32 maxQpP; /**< A maximum QP for P picture. (0 ~ 51). */
    ...
    // W6_ENC_CHANGE_PARAM_RC_MAX_BITRATE
    Uint32 maxBitrate; /**< A maximum bitrate for for rate control. */

    // W6_ENC_CHANGE_PARAM_RC_VBV_BUFFER_SIZE
    Uint32 vbvBufferSize; /**< VBV buffer size in msec. */

    // W6_ENC_CHANGE_PARAM_VUI_HRD_PARAM
    Uint32          encodeHrdRbspInVPS; /**< A flag to encode the HRD syntax RBSP which is given by host into VPS. */
    Uint32          hrdRbspDataSize;    /**< The bit size of the HRD RBSP data. */
    PhysicalAddress hrdRbspDataAddr;    /**< The address of the HRD RBSP buffer. */
    Uint32          encodeVuiRbsp;      /**< A flag to encode the VUI syntax RBSP which is given by host. */
    Uint32          vuiRbspDataSize;    /**< The bit size of the VUI RBSP data. */
    PhysicalAddress vuiRbspDataAddr;    /**< The address of the VUI RBSP buffer. */
    Uint32          encodeAUD;          /**< A flag to encode the AUD NAL. */

    // W6_ENC_CHANGE_PARAM_BG_PARAM
    Uint32 enBgDetect; /**< It enables background detection. */
    /**
    @verbatim
    It specifies the threshold of max difference that is used in s2me block. (0 ~ 255).
    ...
    */
    ...
    // W6_ENC_CHANGE_PARAM_QROUND_OFFSET
    Uint32 qRoundIntra; /**< It specifies a customized quantization rounding offset for intra pictures. (0 ~ 255). */
    Uint32 qRoundInter; /**< It specifies a customized quantization rounding offset for inter pictures. (0 ~ 255). */

    // W6_ENC_CHANGE_PARAM_QUANT_PARAM
    Int32 yDcQpDelta;     /**< It specifies the AV1 Qindex offset for Luma DC. (-64 ~ 63). (AV1 only). */
    Int32 uDcQpDelta;     /**< It specifies the AV1 Qindex offset for DC of Cb component. (-64 ~ 63). (AV1 only). */
    Int32 vDcQpDelta;     /**< It specifies the AV1 Qindex offset for DC of Cr component. (-64 ~ 63). (AV1 only). */
    Int32 uAcQpDelta;     /**< It specifies the AV1 Qindex offset for AC of Cb component. (-64 ~ 63). (AV1 only). */
...
/**
* @brief This is a data structure for operating Subframe Sync(SFS) when SFS works in register-based mode.
*/
typedef struct {
...
/**
* @brief    This is a data structure for setting SEI NALs in encoder.
*/
typedef struct {
/**
@verbatim
...
*/
    ...
    Uint32 prefixSeiDataSize;         /**< The total byte size of the prefix SEI NAL. */
    PhysicalAddress prefixSeiNalAddr; /**< The base address of the prefix SEI NAL buffer. */
    Uint32 suffixSeiNalEnable;        /**< It enables to encode the suffix SEI NAL which is given by host. */
    Uint32 suffixSeiDataSize;         /**< The total byte size of the suffix SEI NAL. */
    PhysicalAddress suffixSeiNalAddr; /**< The base address of the suffix SEI NAL buffer. */
} EncSeiNalData;
...
*/
    ...
    union {
        ...
        EncWave5Param waveParam;  /**< <<vpuapi_h_EncWave5Param>> */
        EncWave6Param wave6Param; /**< <<vpuapi_h_EncWave6Param>> */
    } EncStdParam;
...
*/
    ...
    int             subFrameSyncEnable;     /**< It enable subframe synchronization for the instance. (WAVE5, WAVE6 only) */
    int             subFrameSyncMode;       /**< It sets <<vpuapi_h_SubFrameSyncMode>>. (WAVE5, WAVE6 only) */
    ...
    Uint32          vuiRbspDataSize;    /**< The bit size of the VUI RBSP data. */
    PhysicalAddress vuiRbspDataAddr;    /**< The address of the VUI RBSP buffer. */
    Uint32          encodeAUD;          /**< A flag to encode the AUD NAL. (WAVE6 only) */
    Uint32          extAddrVcpu; /**< AXI address extension of vCPU DMA. (WAVE6 only) */
    BOOL            isSecureInst; /**< It indicates whether the instance is secure instance or non-secure instance. (WAVE6 only). */
    Uint32          instPriority; /**< It indicates the priority value of instance. (0 ~ 255). (WAVE6 only). */
    InstanceBuffer  instBuffer; /**< <<vpuapi_h_InstanceBuffer>>. (WAVE6 only) */
    Uint32          cmdQueueDepth;
...
typedef struct {
    ...
    int encodeAUD;                  /**< A flag to encode AUD nal unit explicitly. (WAVE5 only) */
...
*/
    ...
    Uint32 wpPixSigmaY;  /**< Pixel variance of Y component for weighted prediction. (WAVE5 only) */
    Uint32 wpPixSigmaCb; /**< Pixel variance of Cb component for weighted prediction. (WAVE5 only) */
    Uint32 wpPixSigmaCr; /**< Pixel variance of Cr component for weighted prediction. (WAVE5 only) */
    ...
    int s2SearchCenterEnable; /**< It enables to set an MV search center. If not enabled, an MVP value is used for a search center. */
    int s2SearchCenterXDiv4;  /**< X relative coordinate from the search center divided by 4 (-32~32) */
    int s2SearchCenterYDiv4;  /**< Y relative coordinate from the search center divided by 4 (-16~16) */
...
*/
typedef struct {
    Uint32 lowerX0;  /**< L0 FME 32x32 mv_x sum lower. */
    Uint32 higherX0; /**< L0 FME 32x32 mv_x sum higher. */
    Uint32 lowerY0;  /**< L0 FME 32x32 mv_y sum lower. */
    Uint32 higherY0; /**< L0 FME 32x32 mv_y sum higher. */
    Uint32 lowerX1;  /**< L1 FME 32x32 mv_x sum lower. */
    Uint32 higherX1; /**< L1 FME 32x32 mv_x sum higher. */
    Uint32 lowerY1;  /**< L1 FME 32x32 mv_y sum lower. */
    Uint32 higherY1; /**< L1 FME 32x32 mv_y sum higher. */
} EncReportFmeSum;
...
*/
typedef struct {
    Uint32 cnt0; /**< The count number of MV histogram accumulated for class 0. */
    Uint32 cnt1; /**< The count number of MV histogram accumulated for class 1. */
    Uint32 cnt2; /**< The count number of MV histogram accumulated for class 2. */
    Uint32 cnt3; /**< The count number of MV histogram accumulated for class 3. */
    Uint32 cnt4; /**< The count number of MV histogram accumulated for class 4. */
} EncReportMvHisto;
...
*/
    ...
    EncReportFmeSum reportFmeSum; /**< <<vpuapi_h_EncReportFmeSum>>. (WAVE6 only) */
    EncReportMvHisto reportMvHisto; /**< <<vpuapi_h_EncReportMvHisto>>. (WAVE6 only) */
} EncOutputInfo;
...
*/
 typedef struct {
    ...
    BOOL    encodeAUD;          /**< A flag to encode AUD nal unit explicitly. (WAVE5 only) */
} EncHeaderParam;
...
typedef enum {
    ...
    PRESET_IDX_IBPBP_SINGLE     = 10, /**< gopsize = 2, with single reference (WAVE6 only) */
    PRESET_IDX_IBBBP_SINGLE     = 11, /**< gopsize = 4, with single reference (WAVE6 only) */
    PRESET_IDX_IPPPP_SINGLE     = 12, /**< Consecutive P, cyclic gopsize = 4, with single reference (WAVE6 only) */
    PRESET_IDX_IBBP             = 13, /**< gopsize = 3 (WAVE6 only) */
    PRESET_IDX_IBBP_SINGLE      = 14, /**< gopsize = 3, with single reference (WAVE6 only) */
    PRESET_IDX_IBBBBBBBP        = 15, /**< gopsize = 8 (WAVE6 only) */
    PRESET_IDX_IBBBBBBBP_SINGLE = 16, /**< gopsize = 8, with single reference (WAVE6 only) */
} GOP_PRESET_IDX;
...
/**
*  @brief   This function returns the stride of one frame buffer that is required for calculating frame buffer size.
*  @return  The stride of frame buffer
*/
int VPU_GetFrameBufStride(
    VpuHandle handle,     /**< [Input] A decoder/encoder handle obtained from VPU_DecOpen()/VPU_EncOpen() */
    int       width,      /**< [Input] The width of image  */
...
A pair of VPU_DecIssueSeqInit() and VPU_DecCompleteSeqInit() should be called
...
/**
* @brief    Before allocating AUX buffer, Host can request the size of AUX buffer.
This API calculate the AUX buffer size based on DecAuxBufferSizeInfo value. (WAVE6 only)
* @return
@verbatim
*RETCODE_SUCCESS* ::
Operation was done successfully, which means calculating aux buffer size successfully.

*RETCODE_FAILURE* ::
Operation was failed, which means there was an error for calculating aux buffer size.

*RETCODE_INVALID_HANDLE* ::
This means the given handle for the current API function call, pHandle, was invalid.
This return code might be caused if

* pHandle is not a handle which has been obtained by VPU_DecOpen().
* pHandle is a handle of an instance which has been closed already, etc.

*RETCODE_INVALID_PARAM* ::
The given argument parameter was invalid, which means it has a
null pointer, or given values for some member variables are improper values.
@endverbatim
*/
RetCode VPU_DecGetAuxBufSize(
    DecHandle handle,              /**< [Input] A decoder handle obtained from VPU_DecOpen(). */
    DecAuxBufferSizeInfo sizeInfo, /**< [Input] <<vpuapi_h_DecAuxBufferSizeInfo>> that holds size information of AUX buffer. */
    Uint32* size                   /**< [Output] The size of AUX buffer. */
    );

/**
* @brief    Before calling VPU_DecRegisterFrameBuffer(), Host should call this function
to store the AUX buffer information to API layer. Host should allocate enough aux buffer size
utilizing the VPU_DecGetAuxBufSize() function.
* @return
@verbatim
*RETCODE_SUCCESS* ::
Operation was done successfully, which means aux buffer information is successfully stored in API.

*RETCODE_FAILURE* ::
Operation was failed, which means there was an error for storing aux buffer information in API.
...
This means the size of AUX buffer is smaller than expected.
Host should allocate enough size refer the VPU_DecGetAuxBufSize() function.
@endverbatim
*/
RetCode VPU_DecRegisterAuxBuffer(
    DecHandle handle,           /**< [Input] A decoder handle obtained from VPU_DecOpen(). */
    AuxBufferInfo auxBufferInfo /**< [Input] <<vpuapi_h_AuxBufferInfo>> that holds information of allocated AUX buffers. */
    );

/**
 * @brief This function is used for registering frame buffers with the acquired
information from VPU_DecCompleteSeqInit(). The frame buffers pointed to by
bufArray are managed internally within VPU. These include reference
frames, reconstructed frame, etc. `num` must
not be less than `minFrameBufferCount` obtained by VPU_DecCompleteSeqInit().
* @return
@verbatim
*RETCODE_SUCCESS* ::
...
sequences between API functions. This function
should be called after successful calling VPU_DecIssueSeqInit().
...
the value   requested by VPU_DecCompleteSeqInit().
*RETCODE_INVALID_STRIDE* ::
...
/**
 * @brief This function is used for registering frame buffers with the acquired
information from VPU_DecCompleteSeqInit().
...
*RETCODE_INVALID_PARAM* ::
The given argument parameter, `fbcFb`, `linearFb`, was invalid, which means it has a null
pointer, or given values for some member variables are improper values.
*RETCODE_VPU_RESPONSE_TIMEOUT* ::
Operation has not recieved any response from VPU and has timed out.
@endverbatim
...
/**
* @brief    Before allocating AUX buffer, Host can request the size of AUX buffer.
This API calculate the AUX buffer size based on EncAuxBufferSizeInfo value. (WAVE6 only)
* @return
@verbatim
*RETCODE_SUCCESS* ::
Operation was done successfully, which means calculating aux buffer size successfully.
...
@endverbatim
*/
RetCode VPU_EncGetAuxBufSize(
    EncHandle handle,              /**< [Input] An encoder handle obtained from VPU_EncOpen().*/
    EncAuxBufferSizeInfo sizeInfo, /**< [Input] <<vpuapi_h_EncAuxBufferSizeInfo>> that holds size information of AUX buffer. */
    Uint32* size                   /**< [Output] The size of AUX buffer. */
    );

/**
* @brief    Before calling VPU_EncRegisterFrameBuffer(), Host should call this function
to store the AUX buffer information to API layer. Host should allocate enough aux buffer size
utilizing the VPU_EncGetAuxBufSize() function.
* @return
@verbatim
*RETCODE_SUCCESS* ::
Operation was done successfully, which means aux buffer information is successfully stored in API.
...
*RETCODE_INSUFFICIENT_RESOURCE* ::
This means the size of AUX buffer is smaller than expected.
Host should allocate enough size refer the VPU_EncGetAuxBufSize() function.
@endverbatim
*/
RetCode VPU_EncRegisterAuxBuffer(
    EncHandle handle,           /**< [Input] An encoder handle obtained from VPU_EncOpen(). */
    AuxBufferInfo auxBufferInfo /**< [Input] <<vpuapi_h_AuxBufferInfo>> that holds information of allocated AUX buffers. */
    );
