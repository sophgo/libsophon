...
void Coda9BitIssueCommand(Uint32 coreIdx, CodecInst *inst, int cmd)
{
    ...
    if (inst && inst->loggingEnable)
        vdi_log(coreIdx, instIdx, cmd, 1);
    VpuWriteReg(coreIdx, BIT_RUN_COMMAND, cmd);
}

static void SetupCoda9Properties(Uint32 coreIdx)
{
...
RetCode Coda9VpuInit(Uint32 coreIdx, void* firmware, Uint32 size)
{
    ...
    if (VpuReadReg(coreIdx, BIT_CUR_PC) == 0) {
        Uint32 i;
        for (i=0; i<64; i++) {
            VpuWriteReg(coreIdx, (i*4) + 0x100, 0x0);
        }
    }
...
RetCode Coda9VpuDecInitSeq(DecHandle handle)
{
    ...
    else { // pCodecInst->productId == PRODUCT_ID_960
        val = 0;
...
RetCode Coda9VpuDecGetResult(CodecInst* instance, DecOutputInfo* result)
{
    ...
    if (pCodecInst->loggingEnable)
        vdi_log(pCodecInst->coreIdx, pCodecInst->instIndex, PIC_RUN, 0);
    ...
    if( pCodecInst->codecMode == AVC_DEC)
    {
        ...
        result->avcVuiInfo.vuiTransferCharacteristics = (val>>8) & 0xff;
    ...
    if( pCodecInst->codecMode == MP2_DEC)
    {
        ...
        val = VpuReadReg(pCodecInst->coreIdx, RET_DEC_PIC_VUI_INFO_2);
        result->mp2ColorDescPresentFlag = ((val >> 31) & 0x01);
...
RetCode Coda9VpuDecGetSeqInfo(CodecInst* instance, DecInitialInfo* info)
{
    ...
    if (pCodecInst->loggingEnable) {
        vdi_log(pCodecInst->coreIdx, pCodecInst->instIndex, DEC_SEQ_INIT, 0);
    }
    ...
    if (pCodecInst->codecMode == MP2_DEC) {
        ...
        val = VpuReadReg(pCodecInst->coreIdx, RET_DEC_SEQ_VUI_INFO_2);
        info->mp2ColorDescPresentFlag = ((val >> 31) & 0x01);
...
RetCode Coda9VpuDecRegisterFramebuffer(CodecInst* instance)
{
        ...
        pCodecInst->codecMode == RV_DEC || pCodecInst->codecMode == AVS_DEC)
    {
        ...
        osal_memset(&vbBuffer, 0, sizeof(vpu_buffer_t));
    ...
    if (!ConfigSecAXICoda9(pCodecInst->coreIdx, pCodecInst->productId, pCodecInst->codecMode, &pDecInfo->secAxiInfo, pDecInfo->initialInfo.picWidth,
    ...
    if (vdi_wait_vpu_busy(pCodecInst->coreIdx, __VPU_BUSY_TIMEOUT, BIT_BUSY_FLAG) == -1) {
        if (pCodecInst->loggingEnable)
            vdi_log(pCodecInst->coreIdx, pCodecInst->instIndex, SET_FRAME_BUF, 2);
        return RETCODE_VPU_RESPONSE_TIMEOUT;
    }
    if (pCodecInst->loggingEnable)
        vdi_log(pCodecInst->coreIdx, pCodecInst->instIndex, SET_FRAME_BUF, 0);
...
RetCode Coda9VpuDecGiveCommand(CodecInst* instance, CodecCommand cmd, void* param)
{
    RetCode ret = RETCODE_SUCCESS;
    DecInfo* decInfo;
    ...
    case SET_SEC_AXI:
        {
            ...
            decInfo->secAxiInfo.u.coda9.useBitEnable  = secAxiUse.u.coda9.useBitEnable;
            decInfo->secAxiInfo.u.coda9.useIpEnable   = secAxiUse.u.coda9.useIpEnable;
            decInfo->secAxiInfo.u.coda9.useDbkYEnable = secAxiUse.u.coda9.useDbkYEnable;
            decInfo->secAxiInfo.u.coda9.useDbkCEnable = secAxiUse.u.coda9.useDbkCEnable;
            decInfo->secAxiInfo.u.coda9.useOvlEnable  = secAxiUse.u.coda9.useOvlEnable;
            decInfo->secAxiInfo.u.coda9.useBtpEnable  = secAxiUse.u.coda9.useBtpEnable;
...
RetCode Coda9VpuBuildUpEncParam(CodecInst* pCodecInst, EncOpenParam* param)
{
    RetCode  ret = RETCODE_SUCCESS;
    Int32    productId;
    EncInfo* pEncInfo = &pCodecInst->CodecInfo->encInfo;
    ...
    if ((ret=SetupEncCodecInstance(productId, pCodecInst)) != RETCODE_SUCCESS)
        return ret;
...
RetCode Coda9VpuEncSetup(CodecInst* pCodecInst)
{
    ...
    if (pCodecInst->productId == PRODUCT_ID_980) {
        if (pEncInfo->subFrameSyncConfig.subFrameSyncOn) {
            val =( pEncInfo->subFrameSyncConfig.curEncSourceIdx << 26 | //subFrameCurFrameIndex same as IPU buffer index , 5bit value.range 0, 1, 2
                (pEncInfo->openParam.picHeight / 8) << 16 | // frame height / 8, value, 10bit value.
                pEncInfo->subFrameSyncConfig.subFrameSyncMode << 15 |
                (pEncInfo->subFrameSyncConfig.sourceBufNumber-1) << 8 |      // Source buffer number - 1
                pEncInfo->subFrameSyncConfig.sourceBufIndexBase << 0);   // Base index of PRP source index, Start index of PRP source
//            VLOG(ERR, "ENC_SEQ_INIT subFrameSyncMode=%d, subFrameSyncOn=%d, sourceBufNumber=%d, sourceBufIndexBase=%d, val=%x\n",
//                pEncInfo->subFrameSyncConfig.subFrameSyncMode, pEncInfo->subFrameSyncConfig.subFrameSyncOn,
//                pEncInfo->subFrameSyncConfig.sourceBufNumber, pEncInfo->subFrameSyncConfig.sourceBufIndexBase, val);
            VpuWriteReg(pCodecInst->coreIdx, P_MBC_PIC_SUB_FRAME_CMD, 0); //SFS reset
            VpuWriteReg(pCodecInst->coreIdx, P_MBC_ENC_SUB_FRAME_SYNC, val);//0x12_8204
            VpuWriteReg(pCodecInst->coreIdx, P_MBC_PIC_SUB_FRAME_CMD, 1); //SFS enable
        }
    }
    ...
    if (vdi_wait_interrupt(pCodecInst->coreIdx, 0, __VPU_BUSY_TIMEOUT) == -1) {
        if (pCodecInst->loggingEnable)
            vdi_log(pCodecInst->coreIdx, pCodecInst->instIndex, ENC_SEQ_INIT, 2);
        return RETCODE_VPU_RESPONSE_TIMEOUT;
    }
    VpuWriteReg(pCodecInst->coreIdx, BIT_INT_REASON, 0);

    if (pCodecInst->loggingEnable)
        vdi_log(pCodecInst->coreIdx, pCodecInst->instIndex, ENC_SEQ_INIT, 0);
...
RetCode Coda9VpuEncParaChange(CodecInst* pCodecInst, ParamChange* param)
{
    EncInfo* pEncInfo = &pCodecInst->CodecInfo->encInfo;
    Int32    coreIdx  = pCodecInst->coreIdx;

    VpuWriteReg(coreIdx, CMD_ENC_PARAM_CHANGE_ENABLE, param->paraEnable);

    if (param->paraEnable & C9_ENC_CHANGE_PARAM_GOP_NUM) {
        VpuWriteReg(coreIdx, CMD_ENC_PARAM_CHANGE_GOP_NUM, param->NewGopNum);
        pEncInfo->openParam.gopSize = param->NewGopNum;
    }
    if (param->paraEnable & C9_ENC_CHANGE_PARAM_INTRA_QP) {
        VpuWriteReg(coreIdx, CMD_ENC_PARAM_CHANGE_INTRA_QP, param->NewIntraQp);
    }
    if (param->paraEnable & C9_ENC_CHANGE_PARAM_BIT_RATE) {
        VpuWriteReg(coreIdx, CMD_ENC_PARAM_CHANGE_BITRATE, param->NewBitrate);
        pEncInfo->openParam.bitRate = param->NewBitrate;
    }
    if (param->paraEnable & C9_ENC_CHANGE_PARAM_FRAME_RATE) {
        VpuWriteReg(coreIdx, CMD_ENC_PARAM_CHANGE_F_RATE, param->NewFrameRate);
        pEncInfo->openParam.frameRateInfo = (param->NewFrameRate & 0xffff);
    }
    if (param->paraEnable & C9_ENC_CHANGE_PARAM_INTRA_REFRESH) {
        VpuWriteReg(coreIdx, CMD_ENC_PARAM_CHANGE_INTRA_REFRESH, param->NewIntraRefresh);
        pEncInfo->openParam.intraRefreshNum = param->NewIntraRefresh;
    }

    Coda9BitIssueCommand(coreIdx, pCodecInst, ENC_CHANGE_PARAMETER);
    if (vdi_wait_vpu_busy(coreIdx, __VPU_BUSY_TIMEOUT, BIT_BUSY_FLAG) == -1) {
        if (pCodecInst->loggingEnable)
            vdi_log(coreIdx, pCodecInst->instIndex, ENC_CHANGE_PARAMETER, 2);
        return RETCODE_VPU_RESPONSE_TIMEOUT;
    }

    if (pCodecInst->loggingEnable)
        vdi_log(coreIdx, pCodecInst->instIndex, ENC_CHANGE_PARAMETER, 0);

    return RETCODE_SUCCESS;
}

RetCode Coda9VpuEncRegisterFramebuffer(CodecInst* pCodecInst)
{
    EncInfo*    pEncInfo;
    Int32       i, val;
    RetCode     ret;
    ...
    if (!ConfigSecAXICoda9(pCodecInst->coreIdx, pCodecInst->productId, pCodecInst->codecMode, &pEncInfo->secAxiInfo, stride, height, 0))
        return RETCODE_INSUFFICIENT_RESOURCE;
...
RetCode Coda9VpuEncode(CodecInst* pCodecInst, EncParam* param)
{
    ...
    if (pCodecInst->productId == PRODUCT_ID_980) {
        ...
        if (pEncInfo->subFrameSyncConfig.subFrameSyncOn) {
            VpuWriteReg(pCodecInst->coreIdx, P_MBC_PIC_RUN_IDX, 0);//pEncInfo->subFrameSyncConfig.sourceBufIndexBase);
        }
        VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PIC_SUB_FRAME_SYNC, val);
    }
...
RetCode Coda9VpuEncGetHeader(CodecInst* pCodecInst, EncHeaderParam* encHeaderParam)
{
    EncInfo* pEncInfo;
    EncOpenParam* encOP;
    PhysicalAddress rdPtr;
    ...
    if (vdi_wait_vpu_busy(pCodecInst->coreIdx, __VPU_BUSY_TIMEOUT, BIT_BUSY_FLAG) == -1) {
        if (pCodecInst->loggingEnable)
            vdi_log(pCodecInst->coreIdx, pCodecInst->instIndex, ENCODE_HEADER, 2);
        SetPendingInst(pCodecInst->coreIdx, 0);
        return RETCODE_VPU_RESPONSE_TIMEOUT;
    }
    if (pCodecInst->loggingEnable)
        vdi_log(pCodecInst->coreIdx, pCodecInst->instIndex, ENCODE_HEADER, 0);
...
RetCode Coda9VpuEncGiveCommand(CodecInst* pCodecInst, CodecCommand cmd, void* param)
{
    ...
    case ENC_CONFIG_SUB_FRAME_SYNC:
        {
            EncInfo* pEncInfo = &pCodecInst->CodecInfo->encInfo;
            EncSubFrameSyncConfig* subFrameSyncConfig = (EncSubFrameSyncConfig*)param;

            if (subFrameSyncConfig == NULL)
                return RETCODE_INVALID_PARAM;
...
//            VLOG(INFO, "ENC_CONFIG_SUB_FRAME_SYNC reg=%d, sfs=%d, sourceBufNumber=%d, sourceBufIndexBase=%d, curEncSourceIdx=%d\n",
//                pEncInfo->subFrameSyncConfig.subFrameSyncMode,
//                pEncInfo->subFrameSyncConfig.subFrameSyncOn,
//                pEncInfo->subFrameSyncConfig.sourceBufNumber,
//                pEncInfo->subFrameSyncConfig.sourceBufIndexBase,
//                pEncInfo->subFrameSyncConfig.curEncSourceIdx);
        }
        break;
    case ENC_SET_SUB_FRAME_SYNC:
        {
            EncSubFrameSyncState* pSubFrameSyncState = (EncSubFrameSyncState*)param;
            if (pSubFrameSyncState == NULL) {
                ret = RETCODE_INVALID_PARAM;
            }
            else {
//                VLOG(ERR, "Toogle ipuCurFrameIndex=%d, ipuNewFrame=%d, ipuEndOfRow=%d, val=%x\n",
//                    pSubFrameSyncState->ipuCurFrameIndex, pSubFrameSyncState->ipuNewFrame, pSubFrameSyncState->ipuEndOfRow,
//                    (pSubFrameSyncState->ipuCurFrameIndex<<2) | ((pSubFrameSyncState->ipuNewFrame&0x01)<<1) | (pSubFrameSyncState->ipuEndOfRow&0x01));
                VpuWriteReg(pCodecInst->coreIdx, P_MBC_SUB_FRAME_SYNC_IF,
                    (pSubFrameSyncState->ipuCurFrameIndex<<2) | ((pSubFrameSyncState->ipuNewFrame&0x01)<<1) | (pSubFrameSyncState->ipuEndOfRow&0x01));
            }
        }
        break;
...
RetCode Coda9VpuEncCheckOpenParam(EncOpenParam* pop)
{
    Int32 coreIdx = pop->coreIdx;
    VpuAttr* pAttr = &g_VpuCoreAttributes[coreIdx];
    Int32 picWidth = pop->picWidth;
    Int32 picHeight = pop->picHeight;
    ...
    if (pop->bitstreamFormat == STD_AVC) {
        if (pAttr->productId == PRODUCT_ID_980) {
