...
{
    int alignedWidth  = (codecMode == W_HEVC_ENC || codecMode == W_AV1_ENC) ? VPU_ALIGN32(srcWidth)  : VPU_ALIGN16(srcWidth);
    int alignedHeight = (codecMode == W_HEVC_ENC || codecMode == W_AV1_ENC) ? VPU_ALIGN32(srcHeight) : VPU_ALIGN16(srcHeight);
    int pad_right, pad_bot;
    int crop_right, crop_left, crop_top, crop_bot;
    int prp_mode = rotMode>>1;  // remove prp_enable bit

    if ((codecMode == W_HEVC_ENC || codecMode == W_AV1_ENC)
        && ((srcWidth%8) == 0 && (srcHeight%8) == 0)
    ...
    if (param->right > 0)
        crop_right = param->right + pad_right;
    else
        crop_right = pad_right;

    if (param->bottom > 0)
        crop_bot = param->bottom + pad_bot;
    else
        crop_bot = pad_bot;
    ...
    param->top    = crop_top;
    param->left   = crop_left;
    param->bottom = crop_bot;
    param->right  = crop_right;
    ...
    */
    ...
    {
        param->top    = crop_right;
        param->left   = crop_top;
        param->bottom = crop_left;
        param->right  = crop_bot;
    }
    else if(prp_mode == 2 || prp_mode ==12)
    {
        param->top    = crop_bot;
        param->left   = crop_right;
        param->bottom = crop_top;
        param->right  = crop_left;
    }
    else if(prp_mode == 3 || prp_mode ==13)
    {
        param->top    = crop_left;
        param->left   = crop_bot;
        param->bottom = crop_right;
        param->right  = crop_top;
    }
...
RetCode CheckEncInstanceValidity(EncHandle handle)
{
    ...
    if (pCodecInst->codecMode != MP4_ENC    &&
        ...
        pCodecInst->codecMode != W_AV1_ENC) {
        return RETCODE_INVALID_HANDLE;
    }
    return RETCODE_SUCCESS;
}
...
RetCode SetSliceMode(EncHandle handle, EncSliceMode *pSliceMode)
{
    ...
    VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PARAM_CHANGE_ENABLE, C9_ENC_CHANGE_PARAM_SLICE_MODE);
    VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PARAM_CHANGE_SLICE_MODE, data);

    Coda9BitIssueCommand(pCodecInst->coreIdx, pCodecInst, ENC_CHANGE_PARAMETER);
    if (vdi_wait_vpu_busy(pCodecInst->coreIdx, __VPU_BUSY_TIMEOUT, BIT_BUSY_FLAG) == -1) {
        if (pCodecInst->loggingEnable)
...
RetCode SetHecMode(EncHandle handle, int mode)
{
    ...
    VpuWriteReg(pCodecInst->coreIdx, CMD_ENC_PARAM_CHANGE_ENABLE, C9_ENC_CHANGE_PARAM_HEC_MODE);
    ...
    )
{
    ...
    if (mapType == LINEAR_FRAME_MAP || mapType == LINEAR_FIELD_MAP) {
        ...
        case FORMAT_RGB_P10_32BIT_PACKED:
        case FORMAT_YUV444_P10_32BIT_PACKED:
            lumaStride = VPU_ALIGN64(width * 4);
            break;
        default:
            break;
        }
        ...
        case FORMAT_VYUY_P10_32BIT_LSB:
            if (PRODUCT_ID_W6_SERIES(productId))
                lumaStride = VPU_ALIGN128(VPU_ALIGN16(width) * 5) / 4;
            else
    ...
    )
{
    ...
    if (mapType == LINEAR_FRAME_MAP || mapType == LINEAR_FIELD_MAP) {
        ...
        case FORMAT_VYUY:
            if (PRODUCT_ID_W6_SERIES(productId))
                size_dpb_chr = VPU_ALIGN32(stride/2)*height;
            else
...
int create_sw_uart_thread(unsigned long coreIdx, unsigned long productId)
{
    ...
    s_SwUartContext.status_reg         = PRODUCT_ID_W6_SERIES(productId) ? W6_SW_UART_STATUS  : W5_SW_UART_STATUS;
    s_SwUartContext.tx_data_reg        = PRODUCT_ID_W6_SERIES(productId) ? W6_SW_UART_TX_DATA : W5_SW_UART_TX_DATA;
