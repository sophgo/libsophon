...
void bm1688_bmdrv_smbus_set_default_value(struct pci_dev *pdev, struct bm_device_info *bmdi)
{
	u32 value = 0;
	int i = 0;
	if ((bmdi->cinfo.chip_id == 0x1684) || (bmdi->cinfo.chip_id == 0x1686)) {
		...
		dev_info_reg_write(bmdi, bmdi->cinfo.dev_info.hw_version_reg, BM1688_HW_VERSION(bmdi), sizeof(u8));
		dev_info_reg_write(bmdi, bmdi->cinfo.dev_info.board_name_reg, BM1688_BOARD_TYPE(bmdi), sizeof(u8));
		dev_info_reg_write(bmdi, bmdi->cinfo.dev_info.mcu_version_reg, BM1688_MCU_VERSION(bmdi), sizeof(u8));
