...
#ifdef DEBUG_DDR
#define PR_DDR(fmt, ...) pr_info(fmt, ##__VA_ARGS__) // to minimize print
#else
#define PR_DDR(fmt, ...)
#endif
...
{
	SGTPUV8_ddr_phy_reg_write16(bmdi, base + PHY_OFFSET + (offset << 1), value);
}

static u16 dwc_phy_apb_read16(struct bm_device_info *bmdi, u32 base, u32 offset)
{
	return SGTPUV8_ddr_phy_reg_read16(bmdi, base + PHY_OFFSET + (offset << 1));
}
...
static int dwc_get_mail(struct bm_device_info *bmdi, u32 cfg_base, int mode)
{
	...
	/* waiting a msg */
	while (dwc_phy_apb_read16(bmdi, cfg_base, UCT_SHANDOW_REG) & UCT_WRITE_PROT_SHADOW)
		;
...
static void dwc_decode_stream_msg(struct bm_device_info *bmdi, u32 cfg_base, int train_2d)
{
	...
	PR_DDR("%s str_stream_index 0x%08x\n",
				 train_2d ? "2D" : "1D", str_stream_index);
	i = 0;
	while (i < (str_stream_index & 0xffff))
	{
		args = dwc_get_mail(bmdi, cfg_base, STREAM_MSG);
		PR_DDR("args 0x%02x\n", args);
		i++;
	}
}
...
static void dwc_decode_major_msg(struct bm_device_info *bmdi, int msg)
{
	if (msg == FW_COMPLET_FAILED)
	{
		pr_info("dwc phy message major message type: 0x%x, %s\n", msg,
						...
						: msg == FW_COMPLET_FAILED ? "failed"
																			 : "unknown");
	}
}

void SGTPUV8_mask_train_result(struct bm_device_info *bmdi, u32 index, u32 phase, u32 result)
{
														 ...
														 u32 train_2d, u32 result)
{
	...
	SGTPUV8_mask_train_result(bmdi, index, train_2d, pass_or_fail);
}

void SGTPUV8_dwc_read_msgblock_msg(struct bm_device_info *bmdi, u32 cfg_base, u32 train_2d)
{
	int mail;
...
static inline void dwc_exec_fw(struct bm_device_info *bmdi, int cfg_base, u32 train_2d)
{
	...
	SGTPUV8_dwc_read_msgblock_msg(bmdi, cfg_base, train_2d);
...
static int ddr_phy_enter_mission(struct bm_device_info *bmdi, u32 cfg_base, u32 ddr_index)
{
	...
	PR_DDR("phy run firmware done\n");

	PR_DDR("Wait for impedance calibration\n");
...
static int SGTPUV8_ddr_phy_init(struct bm_device_info *bmdi, u32 cfg_base, u32 ddr_index)
{
	u32 i;
	u32 read_data;
...
void SGTPUV8_dwc_phy_enter_mission(struct bm_device_info *bmdi)
{
...
static void dwc_pre_setting(struct bm_device_info *bmdi, u32 rank, u32 freq)
{
	...
	for (i = 0; i < ARRAY_SIZE(SGTPUV8_pre_cfg_rank); i++)
	{
		if (rank == SGTPUV8_pre_cfg_rank[i].rank_num)
			cfg_rank = &SGTPUV8_pre_cfg_rank[i];
	}

	for (i = 0; i < ARRAY_SIZE(SGTPUV8_pre_cfg_freq); i++)
	{
		if (freq == SGTPUV8_pre_cfg_freq[i].frequency)
			cfg_freq = &SGTPUV8_pre_cfg_freq[i];
	}
	...
	if ((bmdi->boot_info.ddr_mode & 0xffff) == LPDDR4)
	{
		...
		chan_a_offset = (u8 *)&lpddr4x_train1d_dmem + mrs->train_chan_a;
		chan_b_offset = (u8 *)&lpddr4x_train1d_dmem + mrs->train_chan_b;
		...
		chan_a_offset = (u8 *)&lpddr4x_train2d_dmem + mrs->train_chan_a;
		chan_b_offset = (u8 *)&lpddr4x_train2d_dmem + mrs->train_chan_b;
...
static void ddr_group_init(struct bm_device_info *bmdi, u32 group)
{
	...
	{
		// phy init
		SGTPUV8_ddr_phy_init(bmdi, DDR_CTRL0_A, 0);
		SGTPUV8_ddr_phy_init(bmdi, DDR_CTRL0_B, 1);
		// 1d train
	...
	{
		SGTPUV8_ddr_phy_init(bmdi, DDR_CTRL1, 2);
		SGTPUV8_ddr_phy_init(bmdi, DDR_CTRL2, 3);
...
static int ddr_memset_by_cdma(struct bm_device_info *bmdi, u64 start_addr, u64 end_addr)
{
	...
	int num = div_u64((end_addr - start_addr), size);
...
static void SGTPUV8_get_freq_str(int ddr_freq, char *s_ddr_freq)
{
	char *s_freq = NULL;
...
int SGTPUV8_lpddr4_init(struct bm_device_info *bmdi)
{
	...
	{
		if ((SGTPUV8_BOARD_TYPE(bmdi) == BOARD_TYPE_SM5M_P) && (SGTPUV8_HW_VERSION(bmdi) == 0x30))
		{
			freq = FREQ_3600M;
		}
		else
	...
	SGTPUV8_disable_ddr_interleave(bmdi);
...
start:
	...
	SGTPUV8_dwc_phy_enter_mission(bmdi);

	SGTPUV8_get_freq_str(freq, s_ddr_freq);
	pr_info("lpddr%s(rank: %d + %d, mode: %s, freq: %s) init done\n",
					(bmdi->boot_info.ddr_mode & 0xffff) == LPDDR4 ? "4" : "4x",
					GROUP0_RANK(rank), GROUP1_RANK(rank),
					chip_mode == NORMAL_MODE ? "normal" : "fast", s_ddr_freq);
	msleep(1000);

	if (bmdi->boot_info.ddr_ecc_enable == 0x1)
	{
		ddr_memset_by_cdma(bmdi, 0x100000000,
											 div_u64((bmdi->boot_info.ddr_0a_size + bmdi->boot_info.ddr_0b_size), 8) * 7 + 0x100000000);
		ddr_memset_by_cdma(bmdi, 0x300000000, div_u64((bmdi->boot_info.ddr_1_size), 8) * 7 + 0x300000000);
		ddr_memset_by_cdma(bmdi, 0x400000000, div_u64((bmdi->boot_info.ddr_1_size), 8) * 7 + 0x400000000);
