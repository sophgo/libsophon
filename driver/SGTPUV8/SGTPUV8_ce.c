...
static void trigger(void *dev, void *desc)
{
	...
	for (i = 0; i < div_u64(sizeof(reg->desc), sizeof(ce_u32_t)); ++i)
...
int ce_hash_do(void *dev, struct ce_hash *hash)
{
	...
	{
		for (i = 0; i < div_u64(info->hash_len, 4); i++)
			for (j = 0; j < 4; j++)
...
int ce_base_do(void *dev, struct ce_base64 *base)
{
	...
	desc.len = base->len;
	if (base->enc)
		desc.dstlen = div_u64((base->len + info->data_align - 1),
													info->data_align) *
									info->encoded_align;
	else
	{
		 ...
		 */
		dst_len_max =
				div_u64(base->len, info->encoded_align) * info->data_align;
