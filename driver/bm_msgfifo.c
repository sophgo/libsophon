...
int bmdev_copy_to_msgfifo(struct bm_device_info *bmdi, bm_kapi_header_t *api_header_p, bm_api_t *bm_api_p, bm_kapi_opt_header_t *api_opt_header_p, u32 channel, bool api_from_userspace)
{
	u32 cur_wp;
	u32 next_wp;
	u32 idx, msg_buf;
	...
	if (1 == match) {
		...
		if (ret) {
			*data = api_entry->api_data;
			...
			PR_TRACE("pcie_cpu_mode get api data success\n");
			return 0;
		} else {
			if (bmdi->status_bmcpu == BMCPU_RUNNING)
				pr_err("pcie_cpu_mode get api data time out\n");
		}
	}
#endif
	return -1;
...
int bmdev_get_api_num_in_msgfifo(struct bm_device_info *bmdi, u32 channel, int core_id)
{
	...
	api_info = &bmdi->api_info[core_id][channel];
	mutex_lock(&api_info->api_fifo_mutex);
	num = kfifo_len(&api_info->api_fifo);
	mutex_unlock(&api_info->api_fifo_mutex);
	num = num / API_ENTRY_SIZE;
