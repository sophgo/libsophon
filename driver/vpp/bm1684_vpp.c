...
static struct vpp_reset_info bm_vpp_rst[VPP1684_CORE_MAX] = {
...
__maybe_unused static int vpp_reg_dump(struct bm_device_info *bmdi,int core_id)
{
	...
	pr_info("MAP_CONV_TIMEOUT_CNT: 0x%x\n",reg_value);


	bmdev_memcpy_d2s_internal(bmdi, (void *)&pdes_vpp, bmdi->gmem_info.resmem_info.vpp_addr +core_id * (512 << 10), sizeof(struct vpp_descriptor));

	pr_info("VPP_DES_HEAD: 0x%x\n",pdes_vpp.des_head);
	pr_info("VPP_NEXT_CMD_BASE: 0x%x\n",pdes_vpp.next_cmd_base);
	pr_info("MAP_CONV_OFF_BASE_Y: 0x%x\n",pdes_vpp.map_conv_off_base_y);
	pr_info("MAP_CONV_OFF_BASE_C: 0x%x\n",pdes_vpp.map_conv_off_base_c);
	pr_info("VPP_SRC_CTRL: 0x%x\n",pdes_vpp.src_ctrl);
	pr_info("MAP_CONV_OFF_STRIDE: 0x%x\n",pdes_vpp.map_conv_off_stride);
	pr_info("VPP_SRC_CROP_ST: 0x%x\n",pdes_vpp.src_crop_st);
	pr_info("VPP_SRC_CROP_SIZE: 0x%x\n",pdes_vpp.src_crop_size);
	pr_info("VPP_SRC_RY_BASE: 0x%x\n",pdes_vpp.src_ry_base);
	pr_info("VPP_SRC_GU_BASE: 0x%x\n",pdes_vpp.src_gu_base);
	pr_info("VPP_SRC_BV_BASE: 0x%x\n",pdes_vpp.src_bv_base);
	pr_info("VPP_SRC_STRIDE: 0x%x\n",pdes_vpp.src_stride);
	pr_info("VPP_SRC_RY_BASE_EXT: 0x%x\n",pdes_vpp.src_ry_base_ext);
	pr_info("VPP_SRC_GU_BASE_EXT: 0x%x\n",pdes_vpp.src_gu_base_ext);
	pr_info("VPP_SRC_BV_BASE_EXT: 0x%x\n",pdes_vpp.src_bv_base_ext);
	pr_info("VPP_DST_CTRL: 0x%x\n",pdes_vpp.dst_ctrl);
	pr_info("VPP_DST_CROP_ST: 0x%x\n",pdes_vpp.dst_crop_st);
	pr_info("VPP_DST_CROP_SIZE: 0x%x\n",pdes_vpp.dst_crop_size);
	pr_info("VPP_DST_RY_BASE: 0x%x\n",pdes_vpp.dst_ry_base);
	pr_info("VPP_DST_GU_BASE: 0x%x\n",pdes_vpp.dst_gu_base);
	pr_info("VPP_DST_BV_BASE: 0x%x\n",pdes_vpp.dst_bv_base);
	pr_info("VPP_DST_STRIDE: 0x%x\n",pdes_vpp.dst_stride);
	pr_info("VPP_DST_RY_BASE_EXT: 0x%x\n",pdes_vpp.dst_ry_base_ext);
	pr_info("VPP_DST_GU_BASE_EXT: 0x%x\n",pdes_vpp.dst_gu_base_ext);
	pr_info("VPP_DST_BV_BASE_EXT: 0x%x\n",pdes_vpp.dst_bv_base_ext);
	pr_info("VPP_SCL_CTRL: 0x%x\n",pdes_vpp.scl_ctrl);
	pr_info("VPP_SCL_INIT: 0x%x\n",pdes_vpp.scl_int);
	pr_info("VPP_SCL_X: 0x%x\n",pdes_vpp.scl_x);
	pr_info("VPP_SCL_Y: 0x%x\n",pdes_vpp.scl_y);
...
__maybe_unused static int vpp_regs_dump(struct bm_device_info *bmdi)
{
	...
	for (core_id = 0; core_id < VPP1684_CORE_MAX; core_id++)
...
static int vpp_soft_rst(struct bm_device_info *bmdi, int core_id)
{
	...
	{
		pr_err("vpp_soft_rst failed,core_id %d\n",core_id);
		return VPP_ERR;
	}
	/*vpp soft reset*/
...
/*deassert dma_stop and check dma_stop_active*/
	...
	pr_info("vpp_soft_rst done, core_id %d\n",core_id);

	return VPP_OK;
}

__maybe_unused static int vpp_cores_reset(struct bm_device_info *bmdi)
{
	int core_id;

	for (core_id = 0; core_id < VPP1684_CORE_MAX; core_id++)
		vpp_core_reset(bmdi, core_id);
...
static int vpp_check_hw_idle(int core_id, struct bm_device_info *bmdi)
{
	...
	while (1) {
		...
		if (count > 20000) { // 2 Sec
			pr_err("vpp is busy!!! status 0x%08x, core %d, pid %d, tgid %d, device %d\n",
...
static int vpp_setup_desc(int core_id, struct bm_device_info *bmdi,
			  struct vpp_batch *batch, u64 *vpp_desc_pa)
{
	...
	if (ret < 0) {
		pr_err("vpp_check_hw_idle failed! core_id %d, dev_index %d.\n", core_id, bmdi->dev_index);
		return ret;
	}
...
#if 0
	u32 cmd_base = vpp_reg_read(core_id, bmdi, VPP_CMD_BASE);
	u32 cmd_base_ext = vpp_reg_read(core_id, bmdi, VPP_CMD_BASE_EXT);
#endif
				...
				u64 *vpp_desc_pa)
{
	...
	struct vpp_descriptor *pdes_vpp = (struct vpp_descriptor *)kzalloc(sizeof(struct vpp_descriptor), GFP_USER);
	...
	for (idx = 0; idx < batch->num; idx++) {
		...
		/*  the first crop*/
		if (idx == (batch->num - 1)) {
			pdes_vpp->des_head = 0x300 + idx;
			pdes_vpp->next_cmd_base = 0x0;
		} else {
			pdes_vpp->des_head = 0x200 + idx;
			pdes_vpp->next_cmd_base = (unsigned int)(vpp_desc_pa[idx + 1] & 0xffffffff);
			pdes_vpp->des_head |= (unsigned int)(((vpp_desc_pa[idx + 1] >> 32) & 0x7) << 16);
		}

		if (cmd->csc_type == CSC_USER_DEFINED) {
			pdes_vpp->csc_coe00 = cmd->matrix.csc_coe00 & 0x1fff;
			pdes_vpp->csc_coe01 = cmd->matrix.csc_coe01  & 0x1fff;
			pdes_vpp->csc_coe02 = cmd->matrix.csc_coe02  & 0x1fff;
			pdes_vpp->csc_add0  = cmd->matrix.csc_add0  & 0x1fffff;
			pdes_vpp->csc_coe10 = cmd->matrix.csc_coe10  & 0x1fff;
			pdes_vpp->csc_coe11 = cmd->matrix.csc_coe11  & 0x1fff;
			pdes_vpp->csc_coe12 = cmd->matrix.csc_coe12  & 0x1fff;
			pdes_vpp->csc_add1  = cmd->matrix.csc_add1  & 0x1fffff;
			pdes_vpp->csc_coe20 = cmd->matrix.csc_coe20  & 0x1fff;
			pdes_vpp->csc_coe21 = cmd->matrix.csc_coe21  & 0x1fff;
			pdes_vpp->csc_coe22 = cmd->matrix.csc_coe22 & 0x1fff;
			pdes_vpp->csc_add2  = cmd->matrix.csc_add2 & 0x1fffff;
		} else {
		/*not change*/
			pdes_vpp->csc_coe00 = csc_matrix_list[csc_mode][0]  & 0x1fff;
			pdes_vpp->csc_coe01 = csc_matrix_list[csc_mode][1]  & 0x1fff;
			pdes_vpp->csc_coe02 = csc_matrix_list[csc_mode][2]  & 0x1fff;
			pdes_vpp->csc_add0  = csc_matrix_list[csc_mode][3]  & 0x1fffff;
			pdes_vpp->csc_coe10 = csc_matrix_list[csc_mode][4]  & 0x1fff;
			pdes_vpp->csc_coe11 = csc_matrix_list[csc_mode][5]  & 0x1fff;
			pdes_vpp->csc_coe12 = csc_matrix_list[csc_mode][6]  & 0x1fff;
			pdes_vpp->csc_add1  = csc_matrix_list[csc_mode][7]  & 0x1fffff;
			pdes_vpp->csc_coe20 = csc_matrix_list[csc_mode][8]  & 0x1fff;
			pdes_vpp->csc_coe21 = csc_matrix_list[csc_mode][9]  & 0x1fff;
			pdes_vpp->csc_coe22 = csc_matrix_list[csc_mode][10] & 0x1fff;
			pdes_vpp->csc_add2  = csc_matrix_list[csc_mode][11] & 0x1fffff;
		}

		/*src crop parameter*/
		pdes_vpp->src_ctrl = ((src_csc_en & 0x1) << 11)
			| ((cmd->src_plannar & 0x1) << 8)
			| ((cmd->src_endian & 0x1) << 5)
			| ((cmd->src_endian_a & 0x1) << 4)
			| (cmd->src_format & 0xf);

		pdes_vpp->src_crop_st = (cmd->src_axisX << 16) | cmd->src_axisY;
		pdes_vpp->src_crop_size = (cmd->src_cropW << 16) | cmd->src_cropH;
		pdes_vpp->src_stride = (cmd->src_stride << 16) | (srcUV & 0xffff);

		if ((cmd->src_fd0 == 0) && (cmd->src_addr0 != 0)) {/*src addr is pa*/
			pdes_vpp->src_ry_base = (unsigned int)(cmd->src_addr0 & 0xffffffff);
			pdes_vpp->src_gu_base = (unsigned int)(cmd->src_addr1 & 0xffffffff);
			pdes_vpp->src_bv_base = (unsigned int)(cmd->src_addr2 & 0xffffffff);

			pdes_vpp->src_ry_base_ext = (unsigned int)((cmd->src_addr0 >> 32) & 0x7);
			pdes_vpp->src_gu_base_ext = (unsigned int)((cmd->src_addr1 >> 32) & 0x7);
			pdes_vpp->src_bv_base_ext = (unsigned int)((cmd->src_addr2 >> 32) & 0x7);
		} else {/*src addr is not pa*/
			pr_err("%s, src addr0 is not physical addr\n", __func__);
		}

		/*dst crop parameter*/
		pdes_vpp->dst_ctrl = ((cmd->dst_plannar & 0x1) << 8)
			| ((cmd->dst_endian & 0x1) << 5)
			| ((cmd->dst_endian_a & 0x1) << 4)
			| (cmd->dst_format & 0xf);

		pdes_vpp->dst_crop_st = (cmd->dst_axisX << 16) | cmd->dst_axisY;
		pdes_vpp->dst_crop_size = (cmd->dst_cropW << 16) | cmd->dst_cropH;
		pdes_vpp->dst_stride = (cmd->dst_stride << 16) | (dstUV & 0xffff);
		...
		if ((cmd->dst_fd0 == 0) && (cmd->dst_addr0 != 0)) {/*dst addr is pa*/
			...
			if ((output_format == RGB24) && ((cmd->dst_plannar & 0x1) == 0x1)
				...
				pdes_vpp->dst_ry_base = lower_32_bits(dst_addr0);
				pdes_vpp->dst_gu_base = lower_32_bits(dst_addr1);
				pdes_vpp->dst_bv_base = lower_32_bits(dst_addr2);

				pdes_vpp->dst_ry_base_ext = (unsigned int)(upper_32_bits(dst_addr0) & 0x7);
				pdes_vpp->dst_gu_base_ext = (unsigned int)(upper_32_bits(dst_addr1) & 0x7);
				pdes_vpp->dst_bv_base_ext = (unsigned int)(upper_32_bits(dst_addr2) & 0x7);
			} else {
				pdes_vpp->dst_ry_base = (unsigned int)(cmd->dst_addr0 & 0xffffffff);
				pdes_vpp->dst_gu_base = (unsigned int)(cmd->dst_addr1 & 0xffffffff);
				pdes_vpp->dst_bv_base = (unsigned int)(cmd->dst_addr2 & 0xffffffff);

				pdes_vpp->dst_ry_base_ext = (unsigned int)((cmd->dst_addr0 >> 32) & 0x7);
				pdes_vpp->dst_gu_base_ext = (unsigned int)((cmd->dst_addr1 >> 32) & 0x7);
				pdes_vpp->dst_bv_base_ext = (unsigned int)((cmd->dst_addr2 >> 32) & 0x7);
			}
		...
		/*scl_ctrl parameter*/
		pdes_vpp->scl_ctrl = ((cmd->ver_filter_sel & 0xf) << 12)
			| ((cmd->hor_filter_sel & 0xf) << 8)
			| (scale_enable & 0x1);
		pdes_vpp->scl_int = ((cmd->scale_y_init & 0x3fff) << 16)
			| (cmd->scale_x_init & 0x3fff);

		pdes_vpp->scl_x = (unsigned int)(cmd->src_cropW * 16384 / cmd->dst_cropW);
		pdes_vpp->scl_y = (unsigned int)(cmd->src_cropH * 16384 / cmd->dst_cropH);
		...
		if (cmd->mapcon_enable == 1) {
			...
			pdes_vpp->map_conv_off_base_y = (u32)(map_conv_off_base_y & 0xfffffff0);

			map_conv_off_base_c = offset_base_c + (cmd->src_axisX / 2 / 256) * height * 2 +
				(cmd->src_axisY / 2 / 2) * 2 * 16;
			pdes_vpp->map_conv_off_base_c = (u32)(map_conv_off_base_c & 0xfffffff0);

			pdes_vpp->src_ctrl =
				((1 & 0x1) << 12) | (0x0 << 20) | (0x0 << 16) | pdes_vpp->src_ctrl;//little endian
//((1 & 0x1) << 12) | (0xf << 20) | (0xf << 16) | pdes_vpp->src_ctrl;//big endian

			map_pic_height_y = ALIGN(frm_h, 16) & 0x3fff;
			map_pic_height_c = ALIGN(frm_h / 2, 8) & 0x3fff;
			pdes_vpp->map_conv_off_stride = (map_pic_height_y << 16) | map_pic_height_c;
			pdes_vpp->src_crop_st = ((cmd->src_axisX & 0x1fff) << 16) | (cmd->src_axisY & 0x1fff);
			pdes_vpp->src_crop_size = ((cmd->src_cropW & 0x3fff) << 16) | (cmd->src_cropH & 0x3fff);
			frm_w = cmd->cols;
			comp_stride_y = ALIGN(ALIGN(frm_w, 16) * 4, 32);
			map_comp_base_y = comp_base_y + (cmd->src_axisY / 4)*comp_stride_y;
			pdes_vpp->src_ry_base = (u32)(map_comp_base_y & 0xfffffff0);

			comp_stride_c = ALIGN(ALIGN(frm_w / 2, 16) * 4, 32);
			map_comp_base_c = comp_base_c + (cmd->src_axisY / 2 / 2) * comp_stride_c;
			pdes_vpp->src_gu_base = (u32)(map_comp_base_c & 0xfffffff0);
			pdes_vpp->src_stride = ((comp_stride_y & 0xfffffff0) << 16) | (comp_stride_c & 0xfffffff0);
		...
		bmdev_memcpy_s2d_internal(bmdi, vpp_desc_pa[idx], (void *)pdes_vpp, sizeof(struct vpp_descriptor));
	}
	kfree(pdes_vpp);
...
int vpp_handle_setup(struct bm_device_info *bmdi, struct vpp_batch *batch)
{
	...
	if (vpp_desc_pa == NULL) {
		ret = VPP_ENOMEM;
		pr_err("vpp_desc_pa is NULL, dev_index  %d\n", bmdi->dev_index);
	...
	if (ret == VPP_OK) {
		ret1 = vpp_setup_desc(core_id, bmdi, batch, vpp_desc_pa);
		if (ret1 < 0){
			pr_err("vpp_setup_desc failed \n");
			kfree(vpp_desc_pa);
			vpp_free_core_id(bmdi, core_id);
			up(&bmdi->vppdrvctx.vpp_core_sem);
			return ret1;
		...
		ret1 = wait_event_timeout(bmdi->vppdrvctx.wq_vpp[core_id],
					  bmdi->vppdrvctx.got_event_vpp[core_id],
					  10*HZ);
		atomic_dec(&bmdi->vppdrvctx.s_vpp_usage_info.vpp_busy_status[core_id]);

		if (ret1 == 0) {
			pr_err("vpp timeout,ret  %d,core_id %d,current->pid %d,current->tgid %d,vpp_idle_bit_map %ld, dev_index  %d, vpp_desc_pa  %p\n",
					ret1, core_id, current->pid, current->tgid,
					bmdi->vppdrvctx.vpp_idle_bit_map,
					bmdi->dev_index, vpp_desc_pa);
			vpp_dump(batch);
			vpp_regs_dump(bmdi);
...
{
	int i;
	sema_init(&bmdi->vppdrvctx.vpp_core_sem, VPP1684_CORE_MAX);
	bmdi->vppdrvctx.vpp_idle_bit_map = 0;
	vpp_soft_rst(bmdi,0);
	vpp_soft_rst(bmdi,1);
	vpp_usage_info_init(bmdi);
	for (i = 0; i < VPP1684_CORE_MAX; i++)
