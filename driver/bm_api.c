...
int bmdrv_api_load_lib_process(struct bm_device_info *bmdi, bm_api_ext_t bm_api)
{
	...
	api_cpu_load_library_internal.process_handle = do_div(api_cpu_load_library_internal.process_handle, LIB_MAX_REC_CNT);
...
int bmdrv_api_dyn_load_lib_process(struct bm_device_info *bmdi, bm_api_ext_t *p_bm_api)
{
	...
	list_for_each_entry_safe(lib_temp, lib_next, &lib_info->lib_list, lib_list) {
		if (!memcmp(lib_temp->md5, api_cpu_load_library_internal.md5, MD5SUM_LEN) &&
				lib_temp->core_id == p_bm_api->core_id) {
...
int bmdrv_api_dyn_unload_lib_process(struct bm_device_info *bmdi, bm_api_ext_t *p_bm_api)
{
	...
	if (del_lib == 1) {
		...
		list_for_each_safe(pos, pos_next, &(bmdi->exec_func.func_list)) {
			if (!memcmp(((struct bmdrv_exec_func *)pos)->exec_func.md5,
										api_cpu_load_library_internal.md5, MD5SUM_LEN) &&
...
int ksend_api(struct bm_device_info *bmdi, struct file *file, unsigned char *msg, int core_id)
{
	...
	channel = 2;
	apinfo = &bmdi->api_info[core_id][2];
	...
	fifo_empty_number = div_u64(bm_api.api_size, sizeof(u32)) + div_u64(sizeof(bm_kapi_header_t), sizeof(u32));
	...
	api_header.api_size = div_u64(bm_api.api_size, sizeof(u32));
	api_header.api_handle = (uintptr_t)h_info->file;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 3, 0) || (LINUX_VERSION_CODE == KERNEL_VERSION(4, 18, 0) && CENTOS_KERNEL_FIX == 408))
	api_entry->sent_time_us = div_u64(ktime_get_boottime_ns(), 1000);
#else
	api_entry->sent_time_us = div_u64(ktime_get_boot_ns(), 1000);
#endif
...
int bmdrv_send_api(struct bm_device_info *bmdi, struct file *file, unsigned long arg, bool flag, bool api_from_userspace)
{
	...
	struct bm_api_info *apinfo, *apinfo_core0;
	pid_t api_pid;
	bm_api_ext_t bm_api, *bm_api_p, *bm_api_list;
	bm_kapi_header_t api_header;
	...
	channel = 2;
	apinfo = &bmdi->api_info[core_id][2];
	apinfo_core0 = &bmdi->api_info[0][2];
	...
	if (bm_api.api_id == 0x90000013) {
		...
		param_num = div_u64(bm_api.api_size, sizeof(tpu_launch_param_t));
	...
	for (i = 0; i < param_num; i++) {
		...
		if (2 == channel) {
			fifo_empty_number = div_u64(bm_api_p->api_size, sizeof(u32)) + div_u64(sizeof(bm_kapi_header_t), sizeof(u32));
		...
		api_header.api_size = div_u64(bm_api_p->api_size, sizeof(u32));
		api_header.api_handle = (uintptr_t)h_info->file;
		api_header.api_seq = thd_info->last_api_seq[core_id];
		api_header.duration = 0; /* not get from this area now */
		api_header.result = 0;
...
int bmdrv_handle_sync_api(struct bm_device_info *bmdi, struct file *file, unsigned long arg)
{
	...
	// bmdev_dump_msgfifo(bmdi, 2, core_id);
	// bmdev_dump_reg(bmdi, 2, core_id);
	return -EBUSY;
}
#else
int bmdrv_thread_sync_api(struct bm_device_info *bmdi, struct file *file, unsigned long arg)
	...
	for (core = 0; core < core_num; core++) {
		for (channel = 0; channel < 2; channel++) {
			...
			if (channel == 2) {
				...
				mutex_lock(&apinfo->api_fifo_mutex);
				// if (bmdev_msgfifo_empty(bmdi, 2, core)) {
				// 	mutex_unlock(&apinfo->api_fifo_mutex);
				// 	mutex_unlock(&apinfo->api_mutex);
				// 	return 0;
				// }
