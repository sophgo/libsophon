 ...
 */
int bm1684_disable_iommu(struct iommu_ctrl *ctrl);
...
static int iommu_validate_entries_to_device(struct iommu_ctrl *ctrl, int start, int number)
{
	...
	bmdev_construct_cdma_arg(&cdma_arg, ctrl->host_entry_base_paddr + start * sizeof(struct bm1684_iommu_entry),
			bmdi->gmem_info.resmem_info.smmu_addr + start * sizeof(struct bm1684_iommu_entry),
			number * sizeof(struct bm1684_iommu_entry), HOST2CHIP, false, false);

	if (bm1684_cdma_transfer(bmdi, NULL, &cdma_arg, false)) {
		dma_free_coherent(bmdi->cinfo.device, number * sizeof(struct bm1684_iommu_entry),
...
static int iommu_invalidate_entries_to_device(struct iommu_ctrl *ctrl, int start, int number)
{
	...
	struct bm_device_info *bmdi = container_of(memcpy_info, struct bm_device_info, memcpy_info);
	bm_cdma_arg cdma_arg;
	memset(ctrl->host_entry_base_vaddr + start * sizeof(struct bm1684_iommu_entry), 0, number * sizeof(struct bm1684_iommu_entry));
...
static int bm_setup_iommu_pages(struct iommu_ctrl *ctrl, struct bm_buffer_object *bo)
{
	...
	struct bm1684_iommu_entry *last_entry = NULL;
	...
	for_each_sg(bo->sgt.sgl, sg, bo->sgt.nents, i) {
		...
		for (j = 0; j < (end_aligned - start_aligned) / PAGE_SIZE; j++) {
			struct bm1684_iommu_entry *entry = ctrl->host_entry_base_vaddr + sizeof(struct bm1684_iommu_entry) * (bo->iommu.entry_start + entry_idx);
...
{
	struct bm1684_iommu_entry *entry = NULL;
	dma_addr_t paddr;
	int i = 0;

	for (i = 0; i < iommu->real_num; i++) {
		entry = ctrl->host_entry_base_vaddr + sizeof(struct bm1684_iommu_entry) * (iommu->entry_start + i);
...
static int bm_free_iommu_pages(struct  iommu_ctrl *ctrl, struct bm_buffer_object *bo)
{
	...
	for (i = 0; i < bo->iommu.real_num; i++) {
		struct bm1684_iommu_entry *entry = ctrl->host_entry_base_vaddr + sizeof(struct bm1684_iommu_entry) * (bo->iommu.entry_start + i);
...
static int bm_free_raw_iommu_pages(struct iommu_ctrl *ctrl, struct iommu_region *iommu)
{
	int ret = 0;
	int i;
	struct bm1684_iommu_entry *entry;
	for (i = 0; i < iommu->real_num; i++) {
		entry = ctrl->host_entry_base_vaddr + sizeof(struct bm1684_iommu_entry) * (iommu->entry_start + i);
		entry->valid = 0;
	}
...
int bm1684_init_iommu(struct iommu_ctrl *ctrl, struct device *dev)
{
	...
	ctrl->host_entry_base_vaddr = dma_alloc_coherent(bmdi->cinfo.device, entry_num * sizeof(struct bm1684_iommu_entry),
	...
	memset(ctrl->host_entry_base_vaddr, 0, entry_num * sizeof(struct bm1684_iommu_entry));
...
int bm1684_deinit_iommu(struct iommu_ctrl *ctrl)
{
	...
	if (ctrl->enable) {
		bm1684_disable_iommu(ctrl);
		iommu_invalidate_entries_to_device(ctrl, 0, ctrl->entry_num);
	}

	dma_free_coherent(bmdi->cinfo.device, SMMU_PAGE_ENTRY_NUM * sizeof(struct bm1684_iommu_entry),
			ctrl->host_entry_base_vaddr, ctrl->host_entry_base_paddr);
	ctrl->free_ptr = ctrl->alloc_ptr = ctrl->full = 0;
...
static int bmdev_enable_iommu(struct iommu_ctrl *ctrl)
{
	...
	/*
	 * Workaround for bm1684, read the register once again after writing,
...
int bm1684_disable_smmu_transfer(struct bm_memcpy_info *memcpy_info, struct iommu_region *iommu_rgn_src, struct iommu_region *iommu_rgn_dst, struct bm_buffer_object **bo_buffer)
{
	...
	bm1684_disable_iommu(&memcpy_info->iommuctl);
