...
void bm1684_enable_gdma_perf_monitor(struct bm_device_info *bmdi, struct bm_perf_monitor *perf_monitor)
{
	...
	if (bmdi->cinfo.chip_id == 0x1684) {
		...
		gdma_reg_write(bmdi, 0x108, (start_addr >> 7));
		gdma_reg_write(bmdi, 0x10c, (end_addr >> 7));
		gdma_reg_write(bmdi, 0x100, gdma_reg_read(bmdi, 0x100) | (1 << 2));
	}
}
...
void bm1684_disable_gdma_perf_monitor(struct bm_device_info *bmdi)
{
	if (bmdi->cinfo.chip_id == 0x1684) {
		...
		gdma_reg_write(bmdi, 0x100, gdma_reg_read(bmdi, 0x100) & ~(1 << 2));
		gdma_reg_write(bmdi, 0x108, 0x0);
		gdma_reg_write(bmdi, 0x10c, 0x0);
	}
}
