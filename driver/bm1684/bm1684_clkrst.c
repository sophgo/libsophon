...
{
	BM_MODULE_ID module = (BM_MODULE_ID)0;
...
int bm1686_modules_clk_init(struct bm_device_info* bmdi)
{
	struct device *dev = &bmdi->cinfo.pdev->dev;
	struct chip_info *cinfo =  &bmdi->cinfo;
	int ret = 0;

	cinfo->hau_ngs_clk = devm_clk_get(dev, "clk_gate_hau_ngs");
	if (IS_ERR(cinfo->hau_ngs_clk)) {
		ret = PTR_ERR(cinfo->hau_ngs_clk);
		dev_err(dev, "failed to retrieve hau_ngs clk");
		return ret;
	}
	cinfo->tpu_only_clk = devm_clk_get(dev, "clk_gate_tpu_for_tpu_only");
	if (IS_ERR(cinfo->tpu_only_clk)) {
		ret = PTR_ERR(cinfo->tpu_only_clk);
		dev_err(dev, "failed to retrieve tpu_only_clk");
		return ret;
	}
	return 0;
}
...
void bm1686_modules_clk_deinit(struct bm_device_info* bmdi)
{
	struct device *dev = &bmdi->cinfo.pdev->dev;
	struct chip_info *cinfo =  &bmdi->cinfo;

	devm_clk_put(dev, cinfo->hau_ngs_clk);
	devm_clk_put(dev, cinfo->tpu_only_clk);
}
...
void bm1686_modules_clk_enable(struct bm_device_info* bmdi)
{
	bm1684_hau_ngs_clk_enable(bmdi);
	bm1684_tpu_only_clk_enable(bmdi);
}
...
void bm1686_modules_clk_disable(struct bm_device_info* bmdi)
{
	bm1684_hau_ngs_clk_disable(bmdi);
	bm1684_tpu_only_clk_disable(bmdi);
}
#endif
