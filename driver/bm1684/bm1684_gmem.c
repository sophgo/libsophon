...
int bmdrv_bm1684_parse_reserved_mem_info(struct bm_device_info *bmdi)
{
	...
	if (cinfo->a53_enable == 0x0 || (BM1684_BOARD_TYPE(bmdi) == BOARD_TYPE_SC5_PLUS && BM1684_HW_VERSION(bmdi) == 0x3))
	...
	if (gmem_mode == GMEM_NORMAL) {
		...
		if (BM1684_BOARD_TYPE(bmdi) == BOARD_TYPE_SC5_PLUS && BM1684_HW_VERSION(bmdi) == 0x3)
			resmem_info->npureserved_size[1] = 0xA0000000;
		else
		...
		if (BM1684_BOARD_TYPE(bmdi) == BOARD_TYPE_SC5_PLUS && BM1684_HW_VERSION(bmdi) == 0x3)
			resmem_info->npureserved_size[2] = 0x60000000;
		else
