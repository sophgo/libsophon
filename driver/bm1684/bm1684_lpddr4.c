...
static void bm1684_get_freq_str(int ddr_freq, char *s_ddr_freq)
{
	char *s_freq = NULL;
...
int bm1684_lpddr4_init(struct bm_device_info *bmdi)
{
	...
	if (chip_mode == NORMAL_MODE) {
		...
		if ((BM1684_BOARD_TYPE(bmdi) == BOARD_TYPE_SM5M_P) && (BM1684_HW_VERSION(bmdi) == 0x30 || BM1684_HW_VERSION(bmdi) == 0x31)) {
			freq = FREQ_3600M;
		} else {
			freq = FREQ_4000M;
		}
...
start:
	...
	bm1684_get_freq_str(freq, s_ddr_freq);
