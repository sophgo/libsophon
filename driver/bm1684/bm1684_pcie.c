...
void bm1684_pcie_calculate_cdma_max_payload(struct bm_device_info *bmdi)
{
	...
	if (bmcd != NULL) {
		temp_value = bmcd->cdma_max_payload;
		if (temp_value < temp_low)
			temp_low =temp_value;
	}
...
static struct bm_bar_info bm_mode_chose_layout_bm1684[] = {
{
		.bar0_len = 0x400000,
		.bar0_dev_start = 0x5f800000,
...
int bm1684_setup_bar_dev_layout(struct bm_device_info *bmdi, BAR_LAYOUT_TYPE type)
{
	...
	if (type == SETUP_BAR_DEV_LAYOUT) {
		...
		if (bmdi->cinfo.chip_id == 0x1684) {
			bar_layout = bm_mode_chose_layout_bm1684;
		} else if (bmdi->cinfo.chip_id == 0x1686) {
...
void bmdrv_pcie_set_function2_iatu_config(struct pci_dev *pdev, struct bm_device_info *bmdi)
{
	...
	REG_WRITE32(bari->bar0_vaddr, 0x8bc, (REG_READ32(bari->bar0_vaddr, 0x8bc) | 0x1) );
	pcie_timeout_config = REG_READ32(bari->bar0_vaddr + 0x10000, 0x98);
	pcie_timeout_config &= ~(0xf);
	pcie_timeout_config |= (0xa);
	REG_WRITE32(bari->bar0_vaddr + 0x10000, 0x98, pcie_timeout_config);
	REG_WRITE32(bari->bar0_vaddr, 0x8bc, (REG_READ32(bari->bar0_vaddr, 0x8bc) & (~0x1)));
...
void bmdrv_pcie_set_function3_iatu_config(struct pci_dev *pdev, struct bm_device_info *bmdi)
{
	...
	REG_WRITE32(bari->bar0_vaddr, 0x8bc, (REG_READ32(bari->bar0_vaddr, 0x8bc) | 0x1) );
	pcie_timeout_config = REG_READ32(bari->bar0_vaddr + 0x20000, 0x98);
	pcie_timeout_config &= ~(0xf);
	pcie_timeout_config |= (0xa);
	REG_WRITE32(bari->bar0_vaddr + 0x20000, 0x98, pcie_timeout_config);
...
int config_iatu_for_function_x(struct pci_dev *pdev, struct bm_device_info *bmdi, struct bm_bar_info *bari)
{
	...
	bm1684_get_pcie_func_index(bmdi);
	if (bmdi->cinfo.pcie_func_index > 0)
		function_num = bmdi->cinfo.pcie_func_index;
	else
		function_num = (pdev->devfn & 0x7);
